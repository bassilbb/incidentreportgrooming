...
$history_list = NULL; // Initialize page object first

class chistory_list extends chistory {
	...
	// Page object name
	var $PageObjName = 'history_list';

	// Grid form hidden field names
	var $FormName = 'fhistorylist';
	...
	function __construct() {
		...
		$this->AddUrl = "historyadd.php";
		$this->InlineAddUrl = $this->PageUrl() . "a=add";
		$this->GridAddUrl = $this->PageUrl() . "a=gridadd";
		$this->GridEditUrl = $this->PageUrl() . "a=gridedit";
		$this->MultiDeleteUrl = "historydelete.php";
		$this->MultiUpdateUrl = "historyupdate.php";
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fhistorylistsrch";
	...
	function GetFilterList() {
		...
		if (EW_SEARCH_FILTER_OPTION == "Server" && isset($UserProfile))
			$sSavedFilterList = $UserProfile->GetSearchFilters(CurrentUserName(), "fhistorylistsrch");
		...
		$sFilterList = ew_Concat($sFilterList, $this->operational_status->AdvancedSearch->ToJson(), ","); // Field operational_status
	...
	function RestoreFilterList() {
		...
		$this->operational_status->AdvancedSearch->SearchValue = @$filter["x_operational_status"];
		$this->operational_status->AdvancedSearch->SearchOperator = @$filter["z_operational_status"];
		$this->operational_status->AdvancedSearch->SearchCondition = @$filter["v_operational_status"];
		$this->operational_status->AdvancedSearch->SearchValue2 = @$filter["y_operational_status"];
		$this->operational_status->AdvancedSearch->SearchOperator2 = @$filter["w_operational_status"];
		$this->operational_status->AdvancedSearch->Save();
	...
	function AdvancedSearchWhere($Default = FALSE) {
		...
		$this->BuildSearchSql($sWhere, $this->operational_status, $Default, FALSE); // operational_status
		...
		if (!$Default && $this->Command == "search") {
			...
			$this->operational_status->AdvancedSearch->Save(); // operational_status
	...
	function CheckSearchParms() {
		...
		if ($this->status->AdvancedSearch->IssetSession())
			return TRUE;
		if ($this->operational_status->AdvancedSearch->IssetSession())
			return TRUE;
	...
	function ResetAdvancedSearchParms() {
		...
		$this->operational_status->AdvancedSearch->UnsetSession();
	...
	function RestoreSearchParms() {
		...
		$this->operational_status->AdvancedSearch->Load();
	...
	function SetupSortOrder() {
		...
		if (@$_GET["order"] <> "") {
			...
			$this->UpdateSort($this->operational_status); // operational_status
	...
	function ResetCmd() {
		...
		if (substr($this->Command,0,5) == "reset") {
			...
			if ($this->Command == "resetsort") {
				...
				$this->status->setSort("");
				$this->operational_status->setSort("");
	...
	function RenderListOptions() {
		...
		if ($Security->CanView()) {
			if (ew_IsMobile())
				...
				$oListOpt->Body = "<a class=\"ewRowLink ewView\" title=\"" . $viewcaption . "\" data-table=\"history\" data-caption=\"" . $viewcaption . "\" href=\"javascript:void(0);\" onclick=\"ew_ModalDialogShow({lnk:this,url:'" . ew_HtmlEncode($this->ViewUrl) . "',btn:null});\">" . $Language->Phrase("ViewLink") . "</a>";
		} else {
			$oListOpt->Body = "";
		}
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fhistorylistsrch\" href=\"#\">" . $Language->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fhistorylistsrch\" href=\"#\">" . $Language->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->ListActions->Items as $listaction) {
				if ($listaction->Select == EW_ACTION_MULTIPLE) {
					...
					$item->Body = "<a class=\"ewAction ewListAction\" title=\"" . ew_HtmlEncode($caption) . "\" data-caption=\"" . ew_HtmlEncode($caption) . "\" href=\"\" onclick=\"ew_SubmitAction(event,jQuery.extend({f:document.fhistorylist}," . $listaction->ToJson(TRUE) . "));return false;\">" . $icon . "</a>";
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $Language->Phrase("SearchPanel") . "\" data-caption=\"" . $Language->Phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fhistorylistsrch\">" . $Language->Phrase("SearchLink") . "</button>";
		$item->Visible = TRUE;
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewHighlight active\" title=\"" . $Language->Phrase("Highlight") . "\" data-caption=\"" . $Language->Phrase("Highlight") . "\" data-toggle=\"button\" data-form=\"fhistorylistsrch\" data-name=\"" . $this->HighlightName() . "\">" . $Language->Phrase("HighlightBtn") . "</button>";
	...
	function LoadSearchValues() {
		...
		// operational_status
		$this->operational_status->AdvancedSearch->SearchValue = @$_GET["x_operational_status"];
		if ($this->operational_status->AdvancedSearch->SearchValue <> "" && $this->Command == "") $this->Command = "search";
		$this->operational_status->AdvancedSearch->SearchOperator = @$_GET["z_operational_status"];
	...
	function RenderRow() {
		...
		$this->last_updated_by->ViewCustomAttributes = "";
			...
			if ($this->Export == "")
				$this->incident_id->ViewValue = $this->HighlightValue($this->incident_id);
			...
			if ($this->Export == "")
				$this->staffid->ViewValue = $this->HighlightValue($this->staffid);
			...
			if ($this->Export == "")
				$this->incident_description->ViewValue = $this->HighlightValue($this->incident_description);
			...
			$this->datetime_initiated->EditAttrs["class"] = "form-control";
			$this->datetime_initiated->EditCustomAttributes = "";
			$this->datetime_initiated->EditValue2 = ew_HtmlEncode(ew_FormatDateTime(ew_UnFormatDateTime($this->datetime_initiated->AdvancedSearch->SearchValue2, 14), 14));
			$this->datetime_initiated->PlaceHolder = ew_RemoveHtml($this->datetime_initiated->FldCaption());
			...
			if (trim(strval($this->operational_status->AdvancedSearch->SearchValue)) == "") {
				$sFilterWrk = "0=1";
			} else {
				$sFilterWrk = "`code`" . ew_SearchString("=", $this->operational_status->AdvancedSearch->SearchValue, EW_DATATYPE_NUMBER, "");
			}
			$sSqlWrk = "SELECT `code`, `description` AS `DispFld`, '' AS `Disp2Fld`, '' AS `Disp3Fld`, '' AS `Disp4Fld`, '' AS `SelectFilterFld`, '' AS `SelectFilterFld2`, '' AS `SelectFilterFld3`, '' AS `SelectFilterFld4` FROM `operational_status`";
			$sWhereWrk = "";
			$this->operational_status->LookupFilters = array("dx1" => '`description`');
			ew_AddFilter($sWhereWrk, $sFilterWrk);
			$this->Lookup_Selecting($this->operational_status, $sWhereWrk); // Call Lookup Selecting
			...
			if ($rswrk && !$rswrk->EOF) { // Lookup values found
				$arwrk = array();
				$arwrk[1] = ew_HtmlEncode($rswrk->fields('DispFld'));
				$this->operational_status->AdvancedSearch->ViewValue = $this->operational_status->DisplayValue($arwrk);
			} else {
				$this->operational_status->AdvancedSearch->ViewValue = $Language->Phrase("PleaseSelect");
			}
			$arwrk = ($rswrk) ? $rswrk->GetRows() : array();
			if ($rswrk) $rswrk->Close();
			$this->operational_status->EditValue = $arwrk;
	...
	function ValidateSearch() {
		...
		if (!ew_CheckShortEuroDate($this->datetime_initiated->AdvancedSearch->SearchValue2)) {
			ew_AddMessage($gsSearchError, $this->datetime_initiated->FldErrMsg());
		}
	...
	function SetupExportOptions() {
		...
		$url = "";
		$item->Body = "<button id=\"emf_history\" class=\"ewExportLink ewEmail\" title=\"" . $Language->Phrase("ExportToEmailText") . "\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_history',hdr:ewLanguage.Phrase('ExportToEmailText'),f:document.fhistorylist,sel:false" . $url . "});\">" . $Language->Phrase("ExportToEmail") . "</button>";
		$item->Visible = FALSE;
	...
	function SetupLookupFilters($fld, $pageId = null) {
		...
		case "x_status":
			...
			if ($sSqlWrk <> "")
				$fld->LookupFilters["s"] .= $sSqlWrk;
			break;
		case "x_operational_status":
			$sSqlWrk = "";
				$sSqlWrk = "SELECT `code` AS `LinkFld`, `description` AS `DispFld`, '' AS `Disp2Fld`, '' AS `Disp3Fld`, '' AS `Disp4Fld` FROM `operational_status`";
				$sWhereWrk = "{filter}";
				$fld->LookupFilters = array("dx1" => '`description`');
			$fld->LookupFilters += array("s" => $sSqlWrk, "d" => "", "f0" => '`code` IN ({filter_value})', "t0" => "3", "fn0" => "");
			$sSqlWrk = "";
...
// Create page object
if (!isset($history_list)) $history_list = new chistory_list();

// Page init
$history_list->Page_Init();

// Page main
$history_list->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$history_list->Page_Render();
?>
<?php include_once "header.php" ?>
<?php if ($history->Export == "") { ?>
<script type="text/javascript">

// Form object
var CurrentPageID = EW_PAGE_ID = "list";
var CurrentForm = fhistorylist = new ew_Form("fhistorylist", "list");
fhistorylist.FormKeyCountName = '<?php echo $history_list->FormKeyCountName ?>';

// Form_CustomValidate event
fhistorylist.Form_CustomValidate = 
...
fhistorylist.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fhistorylist.Lists["x_staffid"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_staffno","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"users"};
fhistorylist.Lists["x_staffid"].Data = "<?php echo $history_list->staffid->LookupFilterQuery(FALSE, "list") ?>";
fhistorylist.AutoSuggests["x_staffid"] = <?php echo json_encode(array("data" => "ajax=autosuggest&" . $history_list->staffid->LookupFilterQuery(TRUE, "list"))) ?>;
fhistorylist.Lists["x_staff_id"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_firstname","x_lastname","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"users"};
fhistorylist.Lists["x_staff_id"].Data = "<?php echo $history_list->staff_id->LookupFilterQuery(FALSE, "list") ?>";
fhistorylist.Lists["x_branch"] = {"LinkField":"x_branch_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_branch_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"branch"};
fhistorylist.Lists["x_branch"].Data = "<?php echo $history_list->branch->LookupFilterQuery(FALSE, "list") ?>";
fhistorylist.Lists["x_departments"] = {"LinkField":"x_code_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_description","","",""],"ParentFields":[],"ChildFields":["x_category"],"FilterFields":[],"Options":[],"Template":"","LinkTable":"departments"};
fhistorylist.Lists["x_departments"].Data = "<?php echo $history_list->departments->LookupFilterQuery(FALSE, "list") ?>";
fhistorylist.Lists["x_category"] = {"LinkField":"x_category_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_description","","",""],"ParentFields":[],"ChildFields":["x_sub_category"],"FilterFields":[],"Options":[],"Template":"","LinkTable":"category"};
fhistorylist.Lists["x_category"].Data = "<?php echo $history_list->category->LookupFilterQuery(FALSE, "list") ?>";
fhistorylist.Lists["x_sub_category"] = {"LinkField":"x_sub_category_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_sub_category_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"sub_category"};
fhistorylist.Lists["x_sub_category"].Data = "<?php echo $history_list->sub_category->LookupFilterQuery(FALSE, "list") ?>";
fhistorylist.Lists["x_status"] = {"LinkField":"x_code","Ajax":true,"AutoFill":false,"DisplayFields":["x_description","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"status"};
fhistorylist.Lists["x_status"].Data = "<?php echo $history_list->status->LookupFilterQuery(FALSE, "list") ?>";
fhistorylist.Lists["x_operational_status"] = {"LinkField":"x_code","Ajax":true,"AutoFill":false,"DisplayFields":["x_description","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"operational_status"};
fhistorylist.Lists["x_operational_status"].Data = "<?php echo $history_list->operational_status->LookupFilterQuery(FALSE, "list") ?>";
fhistorylist.Lists["x_last_updated_by"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_firstname","x_lastname","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"users"};
fhistorylist.Lists["x_last_updated_by"].Data = "<?php echo $history_list->last_updated_by->LookupFilterQuery(FALSE, "list") ?>";
fhistorylist.AutoSuggests["x_last_updated_by"] = <?php echo json_encode(array("data" => "ajax=autosuggest&" . $history_list->last_updated_by->LookupFilterQuery(TRUE, "list"))) ?>;

// Form object for search
var CurrentSearchForm = fhistorylistsrch = new ew_Form("fhistorylistsrch");

// Validate function for search
fhistorylistsrch.Validate = function(fobj) {
	...
	if (elm && !ew_CheckShortEuroDate(elm.value))
		return this.OnError(elm, "<?php echo ew_JsEncode2($history->datetime_initiated->FldErrMsg()) ?>");
...
// Form_CustomValidate event
fhistorylistsrch.Form_CustomValidate = 
 function(fobj) { // DO NOT CHANGE THIS LINE!
...
// Use JavaScript validation or not
fhistorylistsrch.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fhistorylistsrch.Lists["x_staff_id"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_firstname","x_lastname","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"users"};
fhistorylistsrch.Lists["x_staff_id"].Data = "<?php echo $history_list->staff_id->LookupFilterQuery(FALSE, "extbs") ?>";
fhistorylistsrch.Lists["x_status"] = {"LinkField":"x_code","Ajax":true,"AutoFill":false,"DisplayFields":["x_description","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"status"};
fhistorylistsrch.Lists["x_status"].Data = "<?php echo $history_list->status->LookupFilterQuery(FALSE, "extbs") ?>";
fhistorylistsrch.Lists["x_operational_status"] = {"LinkField":"x_code","Ajax":true,"AutoFill":false,"DisplayFields":["x_description","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"operational_status"};
fhistorylistsrch.Lists["x_operational_status"].Data = "<?php echo $history_list->operational_status->LookupFilterQuery(FALSE, "extbs") ?>";
</script>
<script type="text/javascript">
...
<?php if ($history->Export == "") { ?>
<div class="ewToolbar">
<?php if ($history_list->TotalRecs > 0 && $history_list->ExportOptions->Visible()) { ?>
<?php $history_list->ExportOptions->Render("body") ?>
<?php } ?>
<?php if ($history_list->SearchOptions->Visible()) { ?>
<?php $history_list->SearchOptions->Render("body") ?>
<?php } ?>
<?php if ($history_list->FilterOptions->Visible()) { ?>
<?php $history_list->FilterOptions->Render("body") ?>
<?php } ?>
<div class="clearfix"></div>
</div>
<?php } ?>
<?php
	$bSelectLimit = $history_list->UseSelectLimit;
	if ($bSelectLimit) {
		if ($history_list->TotalRecs <= 0)
			$history_list->TotalRecs = $history->ListRecordCount();
	} else {
		if (!$history_list->Recordset && ($history_list->Recordset = $history_list->LoadRecordset()))
			$history_list->TotalRecs = $history_list->Recordset->RecordCount();
	}
	$history_list->StartRec = 1;
	if ($history_list->DisplayRecs <= 0 || ($history->Export <> "" && $history->ExportAll)) // Display all records
		$history_list->DisplayRecs = $history_list->TotalRecs;
	if (!($history->Export <> "" && $history->ExportAll))
		$history_list->SetupStartRec(); // Set up start record position
	if ($bSelectLimit)
		$history_list->Recordset = $history_list->LoadRecordset($history_list->StartRec-1, $history_list->DisplayRecs);

	// Set no record found message
	if ($history->CurrentAction == "" && $history_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$history_list->setWarningMessage(ew_DeniedMsg());
		if ($history_list->SearchWhere == "0=101")
			$history_list->setWarningMessage($Language->Phrase("EnterSearchCriteria"));
		else
			$history_list->setWarningMessage($Language->Phrase("NoRecord"));
	}

	// Audit trail on search
	if ($history_list->AuditTrailOnSearch && $history_list->Command == "search" && !$history_list->RestoreSearch) {
		$searchparm = ew_ServerVar("QUERY_STRING");
		$searchsql = $history_list->getSessionWhere();
		$history_list->WriteAuditTrailOnSearch($searchparm, $searchsql);
	}
$history_list->RenderOtherOptions();
?>
<?php if ($Security->CanSearch()) { ?>
<?php if ($history->Export == "" && $history->CurrentAction == "") { ?>
<form name="fhistorylistsrch" id="fhistorylistsrch" class="form-inline ewForm ewExtSearchForm" action="<?php echo ew_CurrentPage() ?>">
<?php $SearchPanelClass = ($history_list->SearchWhere <> "") ? " in" : " in"; ?>
<div id="fhistorylistsrch_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
<input type="hidden" name="cmd" value="search">
<input type="hidden" name="t" value="history">
	<div class="ewBasicSearch">
<?php
if ($gsSearchError == "")
	$history_list->LoadAdvancedSearch(); // Load advanced search

// Render for search
$history->RowType = EW_ROWTYPE_SEARCH;

// Render row
$history->ResetAttrs();
$history_list->RenderRow();
?>
<div id="xsr_1" class="ewRow">
<?php if ($history->datetime_initiated->Visible) { // datetime_initiated ?>
	<div id="xsc_datetime_initiated" class="ewCell form-group">
		<label for="x_datetime_initiated" class="ewSearchCaption ewLabel"><?php echo $history->datetime_initiated->FldCaption() ?></label>
...
<input type="text" data-table="history" data-field="x_datetime_initiated" data-format="14" name="x_datetime_initiated" id="x_datetime_initiated" size="18" placeholder="<?php echo ew_HtmlEncode($history->datetime_initiated->getPlaceHolder()) ?>" value="<?php echo $history->datetime_initiated->EditValue ?>"<?php echo $history->datetime_initiated->EditAttributes() ?>>
<?php if (!$history->datetime_initiated->ReadOnly && !$history->datetime_initiated->Disabled && !isset($history->datetime_initiated->EditAttrs["readonly"]) && !isset($history->datetime_initiated->EditAttrs["disabled"])) { ?>
<script type="text/javascript">
ew_CreateDateTimePicker("fhistorylistsrch", "x_datetime_initiated", {"ignoreReadonly":true,"useCurrent":false,"format":14});
</script>
<?php } ?>
</span>
...
<input type="text" data-table="history" data-field="x_datetime_initiated" data-format="14" name="y_datetime_initiated" id="y_datetime_initiated" size="18" placeholder="<?php echo ew_HtmlEncode($history->datetime_initiated->getPlaceHolder()) ?>" value="<?php echo $history->datetime_initiated->EditValue2 ?>"<?php echo $history->datetime_initiated->EditAttributes() ?>>
<?php if (!$history->datetime_initiated->ReadOnly && !$history->datetime_initiated->Disabled && !isset($history->datetime_initiated->EditAttrs["readonly"]) && !isset($history->datetime_initiated->EditAttrs["disabled"])) { ?>
<script type="text/javascript">
ew_CreateDateTimePicker("fhistorylistsrch", "y_datetime_initiated", {"ignoreReadonly":true,"useCurrent":false,"format":14});
</script>
<?php } ?>
</span>
	</div>
...
<?php if ($history->incident_id->Visible) { // incident_id ?>
	<div id="xsc_incident_id" class="ewCell form-group">
		<label for="x_incident_id" class="ewSearchCaption ewLabel"><?php echo $history->incident_id->FldCaption() ?></label>
		<span class="ewSearchOperator"><?php echo $Language->Phrase("LIKE") ?><input type="hidden" name="z_incident_id" id="z_incident_id" value="LIKE"></span>
		<span class="ewSearchField">
<input type="text" data-table="history" data-field="x_incident_id" name="x_incident_id" id="x_incident_id" size="18" placeholder="<?php echo ew_HtmlEncode($history->incident_id->getPlaceHolder()) ?>" value="<?php echo $history->incident_id->EditValue ?>"<?php echo $history->incident_id->EditAttributes() ?>>
</span>
	</div>
<?php } ?>
</div>
...
<?php if ($history->staff_id->Visible) { // staff_id ?>
	<div id="xsc_staff_id" class="ewCell form-group">
		<label for="x_staff_id" class="ewSearchCaption ewLabel"><?php echo $history->staff_id->FldCaption() ?></label>
		<span class="ewSearchOperator"><?php echo $Language->Phrase("=") ?><input type="hidden" name="z_staff_id" id="z_staff_id" value="="></span>
		<span class="ewSearchField">
<span class="ewLookupList">
	<span onclick="jQuery(this).parent().next().click();" tabindex="-1" class="form-control ewLookupText" id="lu_x_staff_id"><?php echo (strval($history->staff_id->AdvancedSearch->ViewValue) == "" ? $Language->Phrase("PleaseSelect") : $history->staff_id->AdvancedSearch->ViewValue); ?></span>
</span>
<button type="button" title="<?php echo ew_HtmlEncode(str_replace("%s", ew_RemoveHtml($history->staff_id->FldCaption()), $Language->Phrase("LookupLink", TRUE))) ?>" onclick="ew_ModalLookupShow({lnk:this,el:'x_staff_id',m:0,n:10});" class="ewLookupBtn btn btn-default btn-sm"<?php echo (($history->staff_id->ReadOnly || $history->staff_id->Disabled) ? " disabled" : "")?>><span class="glyphicon glyphicon-search ewIcon"></span></button>
<input type="hidden" data-table="history" data-field="x_staff_id" data-multiple="0" data-lookup="1" data-value-separator="<?php echo $history->staff_id->DisplayValueSeparatorAttribute() ?>" name="x_staff_id" id="x_staff_id" value="<?php echo $history->staff_id->AdvancedSearch->SearchValue ?>"<?php echo $history->staff_id->EditAttributes() ?>>
</span>
	</div>
<?php } ?>
</div>
...
<?php if ($history->status->Visible) { // status ?>
	<div id="xsc_status" class="ewCell form-group">
		<label for="x_status" class="ewSearchCaption ewLabel"><?php echo $history->status->FldCaption() ?></label>
		<span class="ewSearchOperator"><?php echo $Language->Phrase("=") ?><input type="hidden" name="z_status" id="z_status" value="="></span>
		<span class="ewSearchField">
<span class="ewLookupList">
	<span onclick="jQuery(this).parent().next().click();" tabindex="-1" class="form-control ewLookupText" id="lu_x_status"><?php echo (strval($history->status->AdvancedSearch->ViewValue) == "" ? $Language->Phrase("PleaseSelect") : $history->status->AdvancedSearch->ViewValue); ?></span>
</span>
<button type="button" title="<?php echo ew_HtmlEncode(str_replace("%s", ew_RemoveHtml($history->status->FldCaption()), $Language->Phrase("LookupLink", TRUE))) ?>" onclick="ew_ModalLookupShow({lnk:this,el:'x_status',m:0,n:10});" class="ewLookupBtn btn btn-default btn-sm"<?php echo (($history->status->ReadOnly || $history->status->Disabled) ? " disabled" : "")?>><span class="glyphicon glyphicon-search ewIcon"></span></button>
<input type="hidden" data-table="history" data-field="x_status" data-multiple="0" data-lookup="1" data-value-separator="<?php echo $history->status->DisplayValueSeparatorAttribute() ?>" name="x_status" id="x_status" value="<?php echo $history->status->AdvancedSearch->SearchValue ?>"<?php echo $history->status->EditAttributes() ?>>
</span>
	</div>
<?php } ?>
</div>
...
<?php if ($history->operational_status->Visible) { // operational_status ?>
	<div id="xsc_operational_status" class="ewCell form-group">
		<label for="x_operational_status" class="ewSearchCaption ewLabel"><?php echo $history->operational_status->FldCaption() ?></label>
		<span class="ewSearchOperator"><?php echo $Language->Phrase("=") ?><input type="hidden" name="z_operational_status" id="z_operational_status" value="="></span>
		<span class="ewSearchField">
<span class="ewLookupList">
	<span onclick="jQuery(this).parent().next().click();" tabindex="-1" class="form-control ewLookupText" id="lu_x_operational_status"><?php echo (strval($history->operational_status->AdvancedSearch->ViewValue) == "" ? $Language->Phrase("PleaseSelect") : $history->operational_status->AdvancedSearch->ViewValue); ?></span>
</span>
<button type="button" title="<?php echo ew_HtmlEncode(str_replace("%s", ew_RemoveHtml($history->operational_status->FldCaption()), $Language->Phrase("LookupLink", TRUE))) ?>" onclick="ew_ModalLookupShow({lnk:this,el:'x_operational_status',m:0,n:10});" class="ewLookupBtn btn btn-default btn-sm"<?php echo (($history->operational_status->ReadOnly || $history->operational_status->Disabled) ? " disabled" : "")?>><span class="glyphicon glyphicon-search ewIcon"></span></button>
<input type="hidden" data-table="history" data-field="x_operational_status" data-multiple="0" data-lookup="1" data-value-separator="<?php echo $history->operational_status->DisplayValueSeparatorAttribute() ?>" name="x_operational_status" id="x_operational_status" value="<?php echo $history->operational_status->AdvancedSearch->SearchValue ?>"<?php echo $history->operational_status->EditAttributes() ?>>
</span>
	</div>
<?php } ?>
</div>
<div id="xsr_6" class="ewRow">
	...
	<input type="text" name="<?php echo EW_TABLE_BASIC_SEARCH ?>" id="<?php echo EW_TABLE_BASIC_SEARCH ?>" class="form-control" value="<?php echo ew_HtmlEncode($history_list->BasicSearch->getKeyword()) ?>" placeholder="<?php echo ew_HtmlEncode($Language->Phrase("Search")) ?>">
	<input type="hidden" name="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" id="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" value="<?php echo ew_HtmlEncode($history_list->BasicSearch->getType()) ?>">
	<div class="input-group-btn">
		<button type="button" data-toggle="dropdown" class="btn btn-default"><span id="searchtype"><?php echo $history_list->BasicSearch->getTypeNameShort() ?></span><span class="caret"></span></button>
		<ul class="dropdown-menu pull-right" role="menu">
			<li<?php if ($history_list->BasicSearch->getType() == "") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this)"><?php echo $Language->Phrase("QuickSearchAuto") ?></a></li>
			<li<?php if ($history_list->BasicSearch->getType() == "=") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'=')"><?php echo $Language->Phrase("QuickSearchExact") ?></a></li>
			<li<?php if ($history_list->BasicSearch->getType() == "AND") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'AND')"><?php echo $Language->Phrase("QuickSearchAll") ?></a></li>
			<li<?php if ($history_list->BasicSearch->getType() == "OR") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'OR')"><?php echo $Language->Phrase("QuickSearchAny") ?></a></li>
		</ul>
...
<?php } ?>
<?php } ?>
<?php $history_list->ShowPageHeader(); ?>
<?php
$history_list->ShowMessage();
?>
<?php if ($history_list->TotalRecs > 0 || $history->CurrentAction <> "") { ?>
<div class="box ewBox ewGrid<?php if ($history_list->IsAddOrEdit()) { ?> ewGridAddEdit<?php } ?> history">
<?php if ($history->Export == "") { ?>
<div class="box-header ewGridUpperPanel">
<?php if ($history->CurrentAction <> "gridadd" && $history->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="form-inline ewForm ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($history_list->Pager)) $history_list->Pager = new cPrevNextPager($history_list->StartRec, $history_list->DisplayRecs, $history_list->TotalRecs, $history_list->AutoHidePager) ?>
<?php if ($history_list->Pager->RecordCount > 0 && $history_list->Pager->Visible) { ?>
<div class="ewPager">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($history_list->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $history_list->PageUrl() ?>start=<?php echo $history_list->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($history_list->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $history_list->PageUrl() ?>start=<?php echo $history_list->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $history_list->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($history_list->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $history_list->PageUrl() ?>start=<?php echo $history_list->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($history_list->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $history_list->PageUrl() ?>start=<?php echo $history_list->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $history_list->Pager->PageCount ?></span>
</div>
<?php } ?>
<?php if ($history_list->Pager->RecordCount > 0) { ?>
<div class="ewPager ewRec">
	<span><?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $history_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $history_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $history_list->Pager->RecordCount ?></span>
</div>
<?php } ?>
<?php if ($history_list->TotalRecs > 0 && (!$history_list->AutoHidePageSizeSelector || $history_list->Pager->Visible)) { ?>
<div class="ewPager">
<input type="hidden" name="t" value="history">
<select name="<?php echo EW_TABLE_REC_PER_PAGE ?>" class="form-control input-sm ewTooltip" title="<?php echo $Language->Phrase("RecordsPerPage") ?>" onchange="this.form.submit();">
<option value="5"<?php if ($history_list->DisplayRecs == 5) { ?> selected<?php } ?>>5</option>
<option value="10"<?php if ($history_list->DisplayRecs == 10) { ?> selected<?php } ?>>10</option>
<option value="15"<?php if ($history_list->DisplayRecs == 15) { ?> selected<?php } ?>>15</option>
<option value="20"<?php if ($history_list->DisplayRecs == 20) { ?> selected<?php } ?>>20</option>
<option value="50"<?php if ($history_list->DisplayRecs == 50) { ?> selected<?php } ?>>50</option>
<option value="ALL"<?php if ($history->getRecordsPerPage() == -1) { ?> selected<?php } ?>><?php echo $Language->Phrase("AllRecords") ?></option>
</select>
</div>
<?php } ?>
</form>
...
<?php
	foreach ($history_list->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<div class="clearfix"></div>
...
<?php } ?>
<form name="fhistorylist" id="fhistorylist" class="form-inline ewForm ewListForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($history_list->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $history_list->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="history">
<div id="gmp_history" class="<?php if (ew_IsResponsiveLayout()) { ?>table-responsive <?php } ?>ewGridMiddlePanel">
<?php if ($history_list->TotalRecs > 0 || $history->CurrentAction == "gridedit") { ?>
<table id="tbl_historylist" class="table ewTable">
<thead>
	<tr class="ewTableHeader">
<?php

// Header row
$history_list->RowType = EW_ROWTYPE_HEADER;

// Render list options
$history_list->RenderListOptions();

// Render list options (header, left)
$history_list->ListOptions->Render("header", "left");
?>
<?php if ($history->datetime_initiated->Visible) { // datetime_initiated ?>
	<?php if ($history->SortUrl($history->datetime_initiated) == "") { ?>
		<th data-name="datetime_initiated" class="<?php echo $history->datetime_initiated->HeaderCellClass() ?>"><div id="elh_history_datetime_initiated" class="history_datetime_initiated"><div class="ewTableHeaderCaption"><?php echo $history->datetime_initiated->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="datetime_initiated" class="<?php echo $history->datetime_initiated->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $history->SortUrl($history->datetime_initiated) ?>',1);"><div id="elh_history_datetime_initiated" class="history_datetime_initiated">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $history->datetime_initiated->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($history->datetime_initiated->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($history->datetime_initiated->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($history->incident_id->Visible) { // incident_id ?>
	<?php if ($history->SortUrl($history->incident_id) == "") { ?>
		<th data-name="incident_id" class="<?php echo $history->incident_id->HeaderCellClass() ?>"><div id="elh_history_incident_id" class="history_incident_id"><div class="ewTableHeaderCaption"><?php echo $history->incident_id->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="incident_id" class="<?php echo $history->incident_id->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $history->SortUrl($history->incident_id) ?>',1);"><div id="elh_history_incident_id" class="history_incident_id">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $history->incident_id->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($history->incident_id->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($history->incident_id->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($history->staffid->Visible) { // staffid ?>
	<?php if ($history->SortUrl($history->staffid) == "") { ?>
		<th data-name="staffid" class="<?php echo $history->staffid->HeaderCellClass() ?>"><div id="elh_history_staffid" class="history_staffid"><div class="ewTableHeaderCaption"><?php echo $history->staffid->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="staffid" class="<?php echo $history->staffid->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $history->SortUrl($history->staffid) ?>',1);"><div id="elh_history_staffid" class="history_staffid">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $history->staffid->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($history->staffid->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($history->staffid->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($history->staff_id->Visible) { // staff_id ?>
	<?php if ($history->SortUrl($history->staff_id) == "") { ?>
		<th data-name="staff_id" class="<?php echo $history->staff_id->HeaderCellClass() ?>"><div id="elh_history_staff_id" class="history_staff_id"><div class="ewTableHeaderCaption"><?php echo $history->staff_id->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="staff_id" class="<?php echo $history->staff_id->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $history->SortUrl($history->staff_id) ?>',1);"><div id="elh_history_staff_id" class="history_staff_id">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $history->staff_id->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($history->staff_id->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($history->staff_id->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($history->branch->Visible) { // branch ?>
	<?php if ($history->SortUrl($history->branch) == "") { ?>
		<th data-name="branch" class="<?php echo $history->branch->HeaderCellClass() ?>"><div id="elh_history_branch" class="history_branch"><div class="ewTableHeaderCaption"><?php echo $history->branch->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="branch" class="<?php echo $history->branch->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $history->SortUrl($history->branch) ?>',1);"><div id="elh_history_branch" class="history_branch">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $history->branch->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($history->branch->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($history->branch->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($history->departments->Visible) { // departments ?>
	<?php if ($history->SortUrl($history->departments) == "") { ?>
		<th data-name="departments" class="<?php echo $history->departments->HeaderCellClass() ?>"><div id="elh_history_departments" class="history_departments"><div class="ewTableHeaderCaption"><?php echo $history->departments->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="departments" class="<?php echo $history->departments->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $history->SortUrl($history->departments) ?>',1);"><div id="elh_history_departments" class="history_departments">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $history->departments->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($history->departments->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($history->departments->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($history->category->Visible) { // category ?>
	<?php if ($history->SortUrl($history->category) == "") { ?>
		<th data-name="category" class="<?php echo $history->category->HeaderCellClass() ?>"><div id="elh_history_category" class="history_category"><div class="ewTableHeaderCaption"><?php echo $history->category->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="category" class="<?php echo $history->category->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $history->SortUrl($history->category) ?>',1);"><div id="elh_history_category" class="history_category">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $history->category->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($history->category->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($history->category->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($history->sub_category->Visible) { // sub_category ?>
	<?php if ($history->SortUrl($history->sub_category) == "") { ?>
		<th data-name="sub_category" class="<?php echo $history->sub_category->HeaderCellClass() ?>"><div id="elh_history_sub_category" class="history_sub_category"><div class="ewTableHeaderCaption"><?php echo $history->sub_category->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="sub_category" class="<?php echo $history->sub_category->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $history->SortUrl($history->sub_category) ?>',1);"><div id="elh_history_sub_category" class="history_sub_category">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $history->sub_category->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($history->sub_category->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($history->sub_category->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($history->incident_description->Visible) { // incident_description ?>
	<?php if ($history->SortUrl($history->incident_description) == "") { ?>
		<th data-name="incident_description" class="<?php echo $history->incident_description->HeaderCellClass() ?>"><div id="elh_history_incident_description" class="history_incident_description"><div class="ewTableHeaderCaption"><?php echo $history->incident_description->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="incident_description" class="<?php echo $history->incident_description->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $history->SortUrl($history->incident_description) ?>',1);"><div id="elh_history_incident_description" class="history_incident_description">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $history->incident_description->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($history->incident_description->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($history->incident_description->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($history->status->Visible) { // status ?>
	<?php if ($history->SortUrl($history->status) == "") { ?>
		<th data-name="status" class="<?php echo $history->status->HeaderCellClass() ?>"><div id="elh_history_status" class="history_status"><div class="ewTableHeaderCaption"><?php echo $history->status->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="status" class="<?php echo $history->status->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $history->SortUrl($history->status) ?>',1);"><div id="elh_history_status" class="history_status">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $history->status->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($history->status->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($history->status->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($history->operational_status->Visible) { // operational_status ?>
	<?php if ($history->SortUrl($history->operational_status) == "") { ?>
		<th data-name="operational_status" class="<?php echo $history->operational_status->HeaderCellClass() ?>"><div id="elh_history_operational_status" class="history_operational_status"><div class="ewTableHeaderCaption"><?php echo $history->operational_status->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="operational_status" class="<?php echo $history->operational_status->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $history->SortUrl($history->operational_status) ?>',1);"><div id="elh_history_operational_status" class="history_operational_status">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $history->operational_status->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($history->operational_status->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($history->operational_status->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($history->last_updated_date->Visible) { // last_updated_date ?>
	<?php if ($history->SortUrl($history->last_updated_date) == "") { ?>
		<th data-name="last_updated_date" class="<?php echo $history->last_updated_date->HeaderCellClass() ?>"><div id="elh_history_last_updated_date" class="history_last_updated_date"><div class="ewTableHeaderCaption"><?php echo $history->last_updated_date->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="last_updated_date" class="<?php echo $history->last_updated_date->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $history->SortUrl($history->last_updated_date) ?>',1);"><div id="elh_history_last_updated_date" class="history_last_updated_date">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $history->last_updated_date->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($history->last_updated_date->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($history->last_updated_date->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($history->last_updated_by->Visible) { // last_updated_by ?>
	<?php if ($history->SortUrl($history->last_updated_by) == "") { ?>
		<th data-name="last_updated_by" class="<?php echo $history->last_updated_by->HeaderCellClass() ?>"><div id="elh_history_last_updated_by" class="history_last_updated_by"><div class="ewTableHeaderCaption"><?php echo $history->last_updated_by->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="last_updated_by" class="<?php echo $history->last_updated_by->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $history->SortUrl($history->last_updated_by) ?>',1);"><div id="elh_history_last_updated_by" class="history_last_updated_by">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $history->last_updated_by->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($history->last_updated_by->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($history->last_updated_by->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php
...
$history_list->ListOptions->Render("header", "right");
?>
	</tr>
</thead>
<tbody>
<?php
if ($history->ExportAll && $history->Export <> "") {
	$history_list->StopRec = $history_list->TotalRecs;
} else {

	// Set the last record to display
	if ($history_list->TotalRecs > $history_list->StartRec + $history_list->DisplayRecs - 1)
		$history_list->StopRec = $history_list->StartRec + $history_list->DisplayRecs - 1;
	else
		$history_list->StopRec = $history_list->TotalRecs;
}
$history_list->RecCnt = $history_list->StartRec - 1;
if ($history_list->Recordset && !$history_list->Recordset->EOF) {
	$history_list->Recordset->MoveFirst();
	$bSelectLimit = $history_list->UseSelectLimit;
	if (!$bSelectLimit && $history_list->StartRec > 1)
		$history_list->Recordset->Move($history_list->StartRec - 1);
} elseif (!$history->AllowAddDeleteRow && $history_list->StopRec == 0) {
	$history_list->StopRec = $history->GridAddRowCount;
}

// Initialize aggregate
$history->RowType = EW_ROWTYPE_AGGREGATEINIT;
$history->ResetAttrs();
$history_list->RenderRow();
while ($history_list->RecCnt < $history_list->StopRec) {
	$history_list->RecCnt++;
	if (intval($history_list->RecCnt) >= intval($history_list->StartRec)) {
		$history_list->RowCnt++;

		// Set up key count
		$history_list->KeyCount = $history_list->RowIndex;

		// Init row class and style
		$history->ResetAttrs();
		$history->CssClass = "";
		if ($history->CurrentAction == "gridadd") {
		} else {
			$history_list->LoadRowValues($history_list->Recordset); // Load row values
		}
		$history->RowType = EW_ROWTYPE_VIEW; // Render view

		// Set up row id / data-rowindex
		$history->RowAttrs = array_merge($history->RowAttrs, array('data-rowindex'=>$history_list->RowCnt, 'id'=>'r' . $history_list->RowCnt . '_history', 'data-rowtype'=>$history->RowType));
...
$history_list->ListOptions->Render("body", "left", $history_list->RowCnt);
?>
	<?php if ($history->datetime_initiated->Visible) { // datetime_initiated ?>
		<td data-name="datetime_initiated"<?php echo $history->datetime_initiated->CellAttributes() ?>>
<span id="el<?php echo $history_list->RowCnt ?>_history_datetime_initiated" class="history_datetime_initiated">
...
<?php echo $history->datetime_initiated->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($history->incident_id->Visible) { // incident_id ?>
		<td data-name="incident_id"<?php echo $history->incident_id->CellAttributes() ?>>
<span id="el<?php echo $history_list->RowCnt ?>_history_incident_id" class="history_incident_id">
...
<?php echo $history->incident_id->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($history->staffid->Visible) { // staffid ?>
		<td data-name="staffid"<?php echo $history->staffid->CellAttributes() ?>>
<span id="el<?php echo $history_list->RowCnt ?>_history_staffid" class="history_staffid">
...
<?php echo $history->staffid->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($history->staff_id->Visible) { // staff_id ?>
		<td data-name="staff_id"<?php echo $history->staff_id->CellAttributes() ?>>
<span id="el<?php echo $history_list->RowCnt ?>_history_staff_id" class="history_staff_id">
...
<?php echo $history->staff_id->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($history->branch->Visible) { // branch ?>
		<td data-name="branch"<?php echo $history->branch->CellAttributes() ?>>
<span id="el<?php echo $history_list->RowCnt ?>_history_branch" class="history_branch">
...
<?php echo $history->branch->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($history->departments->Visible) { // departments ?>
		<td data-name="departments"<?php echo $history->departments->CellAttributes() ?>>
<span id="el<?php echo $history_list->RowCnt ?>_history_departments" class="history_departments">
...
<?php echo $history->departments->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($history->category->Visible) { // category ?>
		<td data-name="category"<?php echo $history->category->CellAttributes() ?>>
<span id="el<?php echo $history_list->RowCnt ?>_history_category" class="history_category">
...
<?php echo $history->category->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($history->sub_category->Visible) { // sub_category ?>
		<td data-name="sub_category"<?php echo $history->sub_category->CellAttributes() ?>>
<span id="el<?php echo $history_list->RowCnt ?>_history_sub_category" class="history_sub_category">
...
<?php echo $history->sub_category->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($history->incident_description->Visible) { // incident_description ?>
		<td data-name="incident_description"<?php echo $history->incident_description->CellAttributes() ?>>
<span id="el<?php echo $history_list->RowCnt ?>_history_incident_description" class="history_incident_description">
...
<?php echo $history->incident_description->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($history->status->Visible) { // status ?>
		<td data-name="status"<?php echo $history->status->CellAttributes() ?>>
<span id="el<?php echo $history_list->RowCnt ?>_history_status" class="history_status">
...
<?php echo $history->status->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($history->operational_status->Visible) { // operational_status ?>
		<td data-name="operational_status"<?php echo $history->operational_status->CellAttributes() ?>>
<span id="el<?php echo $history_list->RowCnt ?>_history_operational_status" class="history_operational_status">
...
<?php echo $history->operational_status->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($history->last_updated_date->Visible) { // last_updated_date ?>
		<td data-name="last_updated_date"<?php echo $history->last_updated_date->CellAttributes() ?>>
<span id="el<?php echo $history_list->RowCnt ?>_history_last_updated_date" class="history_last_updated_date">
...
<?php echo $history->last_updated_date->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($history->last_updated_by->Visible) { // last_updated_by ?>
		<td data-name="last_updated_by"<?php echo $history->last_updated_by->CellAttributes() ?>>
<span id="el<?php echo $history_list->RowCnt ?>_history_last_updated_by" class="history_last_updated_by">
...
$history_list->ListOptions->Render("body", "right", $history_list->RowCnt);
?>
	</tr>
<?php
	}
	if ($history->CurrentAction <> "gridadd")
		$history_list->Recordset->MoveNext();
}
?>
</tbody>
</table>
<?php } ?>
<?php if ($history->CurrentAction == "") { ?>
...
// Close recordset
if ($history_list->Recordset)
	$history_list->Recordset->Close();
?>
</div>
<?php } ?>
<?php if ($history_list->TotalRecs == 0 && $history->CurrentAction == "") { // Show other options ?>
<div class="ewListOtherOptions">
<?php
	foreach ($history_list->OtherOptions as &$option) {
		$option->ButtonClass = "";
...
<?php if ($history->Export == "") { ?>
<script type="text/javascript">
fhistorylistsrch.FilterList = <?php echo $history_list->GetFilterList() ?>;
fhistorylistsrch.Init();
fhistorylist.Init();
</script>
<?php } ?>
<?php
$history_list->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($history->Export == "") { ?>
