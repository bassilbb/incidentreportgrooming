...
$report_form_edit = NULL; // Initialize page object first

class creport_form_edit extends creport_form {
	...
	// Page object name
	var $PageObjName = 'report_form_edit';
	...
	function LoadFormValues() {
		...
		if (!$this->operational_status->FldIsDetailKey) {
			$this->operational_status->setFormValue($objForm->GetValue("x_operational_status"));
		}
		...
		if (!$this->closure_action->FldIsDetailKey) {
			$this->closure_action->setFormValue($objForm->GetValue("x_closure_action"));
		}
		if (!$this->closure_comment->FldIsDetailKey) {
			$this->closure_comment->setFormValue($objForm->GetValue("x_closure_comment"));
		}
	...
	function RestoreFormValues() {
		...
		$this->operational_status->CurrentValue = $this->operational_status->FormValue;
		...
		$this->closure_action->CurrentValue = $this->closure_action->FormValue;
		$this->closure_comment->CurrentValue = $this->closure_comment->FormValue;
	...
	function RenderRow() {
		...
		$this->last_updated_by->ViewCustomAttributes = "";
			...
			if (trim(strval($this->operational_status->CurrentValue)) == "") {
				$sFilterWrk = "0=1";
			} else {
				$sFilterWrk = "`code`" . ew_SearchString("=", $this->operational_status->CurrentValue, EW_DATATYPE_NUMBER, "");
			}
			...
			$this->closure_comment->EditValue = ew_HtmlEncode($this->closure_comment->CurrentValue);
	...
	function ValidateForm() {
		...
		if (!$this->operational_status->FldIsDetailKey && !is_null($this->operational_status->FormValue) && $this->operational_status->FormValue == "") {
			ew_AddMessage($gsFormError, str_replace("%s", $this->operational_status->FldCaption(), $this->operational_status->ReqErrMsg));
		}
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// datetime_initiated
			$this->datetime_initiated->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->datetime_initiated->CurrentValue, 11), ew_CurrentDate(), $this->datetime_initiated->ReadOnly);
			...
			// operational_status
			$this->operational_status->SetDbValueDef($rsnew, $this->operational_status->CurrentValue, NULL, $this->operational_status->ReadOnly);
			...
			// datetime_resolved
			$this->datetime_resolved->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->datetime_resolved->CurrentValue, 11), ew_CurrentDate(), $this->datetime_resolved->ReadOnly);
			...
			// closure_action
			$this->closure_action->SetDbValueDef($rsnew, $this->closure_action->CurrentValue, NULL, $this->closure_action->ReadOnly);

			// closure_comment
			$this->closure_comment->SetDbValueDef($rsnew, $this->closure_comment->CurrentValue, NULL, $this->closure_comment->ReadOnly);

			// last_updated_date
			$this->last_updated_date->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->last_updated_date->CurrentValue, 17), NULL, $this->last_updated_date->ReadOnly);
	...
	function Form_CustomValidate(&$CustomError) {
		...
		if(CurrentUserLevel() == 1 && $rs["initiator_action"] == '') {
			$CustomError = "Please enter required field - Initiator Action!";
			return FALSE;
		}	
		if(CurrentUserLevel() == 1 && $rs["initiator_comment"] == '') {
			$CustomError = "Please enter required field - Initiator Comment!";
			return FALSE;
		}
		if(CurrentUserLevel() == 2 && $this->status->CurrentValue == 3 && $rs["resolved_action"] == '') {
			$CustomError = "Please enter required field - Resolved Action!";
			return FALSE;
		}	
		if(CurrentUserLevel() == 2 && $this->status->CurrentValue == 3 && $rs["resolved_comment"] == '') {
			$CustomError = "Please enter required field - Resolved Comment!";
			return FALSE;
		}
		if(CurrentUserLevel() == 3 && $rs["approval_action"] == '') {
			$CustomError = "Please enter required field - Approval Action!";
			return FALSE;
		}	
		if(CurrentUserLevel() == 3 && $rs["approval_comment"] == '') {
			$CustomError = "Please enter required field - Approval Comment!";
			return FALSE;
		}
		return TRUE;
	}
...
// Create page object
if (!isset($report_form_edit)) $report_form_edit = new creport_form_edit();

// Page init
$report_form_edit->Page_Init();

// Page main
$report_form_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$report_form_edit->Page_Render();
?>
...
var CurrentForm = freport_formedit = new ew_Form("freport_formedit", "edit");

// Validate form
freport_formedit.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			elm = this.GetElements("x" + infix + "_operational_status");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $report_form->operational_status->FldCaption(), $report_form->operational_status->ReqErrMsg)) ?>");
			...
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $report_form->approval_action->FldCaption(), $report_form->approval_action->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_approval_comment");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $report_form->approval_comment->FldCaption(), $report_form->approval_comment->ReqErrMsg)) ?>");
...
freport_formedit.Form_CustomValidate = 
...
freport_formedit.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Multi-Page
freport_formedit.MultiPage = new ew_MultiPage("freport_formedit");

// Dynamic selection lists
freport_formedit.Lists["x_staffid"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_staffno","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"users"};
freport_formedit.Lists["x_staffid"].Data = "<?php echo $report_form_edit->staffid->LookupFilterQuery(FALSE, "edit") ?>";
freport_formedit.AutoSuggests["x_staffid"] = <?php echo json_encode(array("data" => "ajax=autosuggest&" . $report_form_edit->staffid->LookupFilterQuery(TRUE, "edit"))) ?>;
freport_formedit.Lists["x_staff_id"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_firstname","x_lastname","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"users"};
freport_formedit.Lists["x_staff_id"].Data = "<?php echo $report_form_edit->staff_id->LookupFilterQuery(FALSE, "edit") ?>";
freport_formedit.Lists["x_department"] = {"LinkField":"x_department_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_department_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"depertment"};
freport_formedit.Lists["x_department"].Data = "<?php echo $report_form_edit->department->LookupFilterQuery(FALSE, "edit") ?>";
freport_formedit.Lists["x_branch"] = {"LinkField":"x_branch_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_branch_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"branch"};
freport_formedit.Lists["x_branch"].Data = "<?php echo $report_form_edit->branch->LookupFilterQuery(FALSE, "edit") ?>";
freport_formedit.Lists["x_departments"] = {"LinkField":"x_code_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_description","","",""],"ParentFields":[],"ChildFields":["x_category"],"FilterFields":[],"Options":[],"Template":"","LinkTable":"departments"};
freport_formedit.Lists["x_departments"].Data = "<?php echo $report_form_edit->departments->LookupFilterQuery(FALSE, "edit") ?>";
freport_formedit.Lists["x_category"] = {"LinkField":"x_category_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_description","","",""],"ParentFields":["x_departments"],"ChildFields":["x_sub_category"],"FilterFields":["x_code_id"],"Options":[],"Template":"","LinkTable":"category"};
freport_formedit.Lists["x_category"].Data = "<?php echo $report_form_edit->category->LookupFilterQuery(FALSE, "edit") ?>";
freport_formedit.Lists["x_sub_category"] = {"LinkField":"x_sub_category_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_sub_category_name","","",""],"ParentFields":["x_category"],"ChildFields":[],"FilterFields":["x_category_id"],"Options":[],"Template":"","LinkTable":"sub_category"};
freport_formedit.Lists["x_sub_category"].Data = "<?php echo $report_form_edit->sub_category->LookupFilterQuery(FALSE, "edit") ?>";
freport_formedit.Lists["x_incident_type"] = {"LinkField":"x_code","Ajax":true,"AutoFill":false,"DisplayFields":["x_description","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"type_of_incident"};
freport_formedit.Lists["x_incident_type"].Data = "<?php echo $report_form_edit->incident_type->LookupFilterQuery(FALSE, "edit") ?>";
freport_formedit.Lists["x_incident_category"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_description","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"incident_category"};
freport_formedit.Lists["x_incident_category"].Data = "<?php echo $report_form_edit->incident_category->LookupFilterQuery(FALSE, "edit") ?>";
freport_formedit.Lists["x_incident_location"] = {"LinkField":"x_code","Ajax":true,"AutoFill":false,"DisplayFields":["x_description","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"incident_location"};
freport_formedit.Lists["x_incident_location"].Data = "<?php echo $report_form_edit->incident_location->LookupFilterQuery(FALSE, "edit") ?>";
freport_formedit.Lists["x_status"] = {"LinkField":"x_code","Ajax":true,"AutoFill":false,"DisplayFields":["x_description","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"status"};
freport_formedit.Lists["x_status"].Data = "<?php echo $report_form_edit->status->LookupFilterQuery(FALSE, "edit") ?>";
freport_formedit.Lists["x_operational_status"] = {"LinkField":"x_code","Ajax":true,"AutoFill":false,"DisplayFields":["x_description","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"operational_status"};
freport_formedit.Lists["x_operational_status"].Data = "<?php echo $report_form_edit->operational_status->LookupFilterQuery(FALSE, "edit") ?>";
freport_formedit.Lists["x_initiator_action"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
freport_formedit.Lists["x_initiator_action"].Options = <?php echo json_encode($report_form_edit->initiator_action->Options()) ?>;
freport_formedit.Lists["x_report_by"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_firstname","x_lastname","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"users"};
freport_formedit.Lists["x_report_by"].Data = "<?php echo $report_form_edit->report_by->LookupFilterQuery(FALSE, "edit") ?>";
freport_formedit.AutoSuggests["x_report_by"] = <?php echo json_encode(array("data" => "ajax=autosuggest&" . $report_form_edit->report_by->LookupFilterQuery(TRUE, "edit"))) ?>;
freport_formedit.Lists["x_resolved_action"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
freport_formedit.Lists["x_resolved_action"].Options = <?php echo json_encode($report_form_edit->resolved_action->Options()) ?>;
freport_formedit.Lists["x_resolved_by"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_firstname","x_lastname","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"users"};
freport_formedit.Lists["x_resolved_by"].Data = "<?php echo $report_form_edit->resolved_by->LookupFilterQuery(FALSE, "edit") ?>";
freport_formedit.AutoSuggests["x_resolved_by"] = <?php echo json_encode(array("data" => "ajax=autosuggest&" . $report_form_edit->resolved_by->LookupFilterQuery(TRUE, "edit"))) ?>;
freport_formedit.Lists["x_approval_action"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
freport_formedit.Lists["x_approval_action"].Options = <?php echo json_encode($report_form_edit->approval_action->Options()) ?>;
freport_formedit.Lists["x_approved_by"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_firstname","x_lastname","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"users"};
freport_formedit.Lists["x_approved_by"].Data = "<?php echo $report_form_edit->approved_by->LookupFilterQuery(FALSE, "edit") ?>";
freport_formedit.AutoSuggests["x_approved_by"] = <?php echo json_encode(array("data" => "ajax=autosuggest&" . $report_form_edit->approved_by->LookupFilterQuery(TRUE, "edit"))) ?>;
freport_formedit.Lists["x_closure_action"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
freport_formedit.Lists["x_closure_action"].Options = <?php echo json_encode($report_form_edit->closure_action->Options()) ?>;
freport_formedit.Lists["x_last_updated_by"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_firstname","x_lastname","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"users"};
freport_formedit.Lists["x_last_updated_by"].Data = "<?php echo $report_form_edit->last_updated_by->LookupFilterQuery(FALSE, "edit") ?>";
freport_formedit.AutoSuggests["x_last_updated_by"] = <?php echo json_encode(array("data" => "ajax=autosuggest&" . $report_form_edit->last_updated_by->LookupFilterQuery(TRUE, "edit"))) ?>;
...
$('#x_departments').attr('readonly',true);
...
<?php $report_form_edit->ShowPageHeader(); ?>
<?php
$report_form_edit->ShowMessage();
?>
<?php if (!$report_form_edit->IsModal) { ?>
<?php if ($report_form->CurrentAction <> "F") { // Confirm page ?>
<form name="ewPagerForm" class="form-horizontal ewForm ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($report_form_edit->Pager)) $report_form_edit->Pager = new cPrevNextPager($report_form_edit->StartRec, $report_form_edit->DisplayRecs, $report_form_edit->TotalRecs, $report_form_edit->AutoHidePager) ?>
<?php if ($report_form_edit->Pager->RecordCount > 0 && $report_form_edit->Pager->Visible) { ?>
<div class="ewPager">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($report_form_edit->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $report_form_edit->PageUrl() ?>start=<?php echo $report_form_edit->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($report_form_edit->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $report_form_edit->PageUrl() ?>start=<?php echo $report_form_edit->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $report_form_edit->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($report_form_edit->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $report_form_edit->PageUrl() ?>start=<?php echo $report_form_edit->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($report_form_edit->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $report_form_edit->PageUrl() ?>start=<?php echo $report_form_edit->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $report_form_edit->Pager->PageCount ?></span>
</div>
<?php } ?>
...
<?php } ?>
<?php } ?>
<form name="freport_formedit" id="freport_formedit" class="<?php echo $report_form_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($report_form_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $report_form_edit->Token ?>">
<?php } ?>
...
<?php } ?>
<input type="hidden" name="modal" value="<?php echo intval($report_form_edit->IsModal) ?>">
<div class="ewMultiPage"><!-- multi-page -->
<div class="nav-tabs-custom" id="report_form_edit"><!-- multi-page .nav-tabs-custom -->
	<ul class="nav<?php echo $report_form_edit->MultiPages->NavStyle() ?>">
		<li<?php echo $report_form_edit->MultiPages->TabStyle("1") ?>><a href="#tab_report_form1" data-toggle="tab"><?php echo $report_form->PageCaption(1) ?></a></li>
		<li<?php echo $report_form_edit->MultiPages->TabStyle("2") ?>><a href="#tab_report_form2" data-toggle="tab"><?php echo $report_form->PageCaption(2) ?></a></li>
	</ul>
	<div class="tab-content"><!-- multi-page .nav-tabs-custom .tab-content -->
		<div class="tab-pane<?php echo $report_form_edit->MultiPages->PageStyle("1") ?>" id="tab_report_form1"><!-- multi-page .tab-pane -->
<div class="ewEditDiv"><!-- page* -->
<?php if ($report_form->datetime_initiated->Visible) { // datetime_initiated ?>
	<div id="r_datetime_initiated" class="form-group">
		<label id="elh_report_form_datetime_initiated" for="x_datetime_initiated" class="<?php echo $report_form_edit->LeftColumnClass ?>"><?php echo $report_form->datetime_initiated->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $report_form_edit->RightColumnClass ?>"><div<?php echo $report_form->datetime_initiated->CellAttributes() ?>>
...
<?php if ($report_form->incident_id->Visible) { // incident_id ?>
	<div id="r_incident_id" class="form-group">
		<label id="elh_report_form_incident_id" for="x_incident_id" class="<?php echo $report_form_edit->LeftColumnClass ?>"><?php echo $report_form->incident_id->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $report_form_edit->RightColumnClass ?>"><div<?php echo $report_form->incident_id->CellAttributes() ?>>
<?php if ($report_form->CurrentAction <> "F") { ?>
...
<?php if ($report_form->staffid->Visible) { // staffid ?>
	<div id="r_staffid" class="form-group">
		<label id="elh_report_form_staffid" class="<?php echo $report_form_edit->LeftColumnClass ?>"><?php echo $report_form->staffid->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $report_form_edit->RightColumnClass ?>"><div<?php echo $report_form->staffid->CellAttributes() ?>>
<?php if ($report_form->CurrentAction <> "F") { ?>
...
<script type="text/javascript">
freport_formedit.CreateAutoSuggest({"id":"x_staffid","forceSelect":false});
</script>
</span>
<?php } else { ?>
<span id="el_report_form_staffid">
...
<?php if ($report_form->staff_id->Visible) { // staff_id ?>
	<div id="r_staff_id" class="form-group">
		<label id="elh_report_form_staff_id" for="x_staff_id" class="<?php echo $report_form_edit->LeftColumnClass ?>"><?php echo $report_form->staff_id->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $report_form_edit->RightColumnClass ?>"><div<?php echo $report_form->staff_id->CellAttributes() ?>>
<?php if ($report_form->CurrentAction <> "F") { ?>
<span id="el_report_form_staff_id">
<select data-table="report_form" data-field="x_staff_id" data-page="1" data-value-separator="<?php echo $report_form->staff_id->DisplayValueSeparatorAttribute() ?>" id="x_staff_id" name="x_staff_id"<?php echo $report_form->staff_id->EditAttributes() ?>>
<?php echo $report_form->staff_id->SelectOptionListHtml("x_staff_id") ?>
</select>
</span>
<?php } else { ?>
<span id="el_report_form_staff_id">
...
<?php if ($report_form->department->Visible) { // department ?>
	<div id="r_department" class="form-group">
		<label id="elh_report_form_department" for="x_department" class="<?php echo $report_form_edit->LeftColumnClass ?>"><?php echo $report_form->department->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $report_form_edit->RightColumnClass ?>"><div<?php echo $report_form->department->CellAttributes() ?>>
<?php if ($report_form->CurrentAction <> "F") { ?>
...
<?php if ($report_form->branch->Visible) { // branch ?>
	<div id="r_branch" class="form-group">
		<label id="elh_report_form_branch" for="x_branch" class="<?php echo $report_form_edit->LeftColumnClass ?>"><?php echo $report_form->branch->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $report_form_edit->RightColumnClass ?>"><div<?php echo $report_form->branch->CellAttributes() ?>>
<?php if ($report_form->CurrentAction <> "F") { ?>
...
<?php if ($report_form->departments->Visible) { // departments ?>
	<div id="r_departments" class="form-group">
		<label id="elh_report_form_departments" for="x_departments" class="<?php echo $report_form_edit->LeftColumnClass ?>"><?php echo $report_form->departments->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $report_form_edit->RightColumnClass ?>"><div<?php echo $report_form->departments->CellAttributes() ?>>
<?php if ($report_form->CurrentAction <> "F") { ?>
...
<span class="ewLookupList">
	<span onclick="jQuery(this).parent().next().click();" tabindex="-1" class="form-control ewLookupText" id="lu_x_departments"><?php echo (strval($report_form->departments->ViewValue) == "" ? $Language->Phrase("PleaseSelect") : $report_form->departments->ViewValue); ?></span>
</span>
...
<input type="hidden" data-table="report_form" data-field="x_departments" data-page="1" data-multiple="0" data-lookup="1" data-value-separator="<?php echo $report_form->departments->DisplayValueSeparatorAttribute() ?>" name="x_departments" id="x_departments" value="<?php echo $report_form->departments->CurrentValue ?>"<?php echo $report_form->departments->EditAttributes() ?>>
</span>
<?php } else { ?>
<span id="el_report_form_departments">
...
<?php if ($report_form->category->Visible) { // category ?>
	<div id="r_category" class="form-group">
		<label id="elh_report_form_category" for="x_category" class="<?php echo $report_form_edit->LeftColumnClass ?>"><?php echo $report_form->category->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $report_form_edit->RightColumnClass ?>"><div<?php echo $report_form->category->CellAttributes() ?>>
<?php if ($report_form->CurrentAction <> "F") { ?>
...
<span class="ewLookupList">
	<span onclick="jQuery(this).parent().next().click();" tabindex="-1" class="form-control ewLookupText" id="lu_x_category"><?php echo (strval($report_form->category->ViewValue) == "" ? $Language->Phrase("PleaseSelect") : $report_form->category->ViewValue); ?></span>
</span>
...
<input type="hidden" data-table="report_form" data-field="x_category" data-page="1" data-multiple="0" data-lookup="1" data-value-separator="<?php echo $report_form->category->DisplayValueSeparatorAttribute() ?>" name="x_category" id="x_category" value="<?php echo $report_form->category->CurrentValue ?>"<?php echo $report_form->category->EditAttributes() ?>>
</span>
<?php } else { ?>
<span id="el_report_form_category">
...
<?php if ($report_form->sub_category->Visible) { // sub_category ?>
	<div id="r_sub_category" class="form-group">
		<label id="elh_report_form_sub_category" for="x_sub_category" class="<?php echo $report_form_edit->LeftColumnClass ?>"><?php echo $report_form->sub_category->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $report_form_edit->RightColumnClass ?>"><div<?php echo $report_form->sub_category->CellAttributes() ?>>
<?php if ($report_form->CurrentAction <> "F") { ?>
<span id="el_report_form_sub_category">
<span class="ewLookupList">
	<span onclick="jQuery(this).parent().next().click();" tabindex="-1" class="form-control ewLookupText" id="lu_x_sub_category"><?php echo (strval($report_form->sub_category->ViewValue) == "" ? $Language->Phrase("PleaseSelect") : $report_form->sub_category->ViewValue); ?></span>
</span>
...
<input type="hidden" data-table="report_form" data-field="x_sub_category" data-page="1" data-multiple="0" data-lookup="1" data-value-separator="<?php echo $report_form->sub_category->DisplayValueSeparatorAttribute() ?>" name="x_sub_category" id="x_sub_category" value="<?php echo $report_form->sub_category->CurrentValue ?>"<?php echo $report_form->sub_category->EditAttributes() ?>>
</span>
<?php } else { ?>
<span id="el_report_form_sub_category">
...
<?php if ($report_form->start_date->Visible) { // start_date ?>
	<div id="r_start_date" class="form-group">
		<label id="elh_report_form_start_date" for="x_start_date" class="<?php echo $report_form_edit->LeftColumnClass ?>"><?php echo $report_form->start_date->FldCaption() ?></label>
		<div class="<?php echo $report_form_edit->RightColumnClass ?>"><div<?php echo $report_form->start_date->CellAttributes() ?>>
<?php if ($report_form->CurrentAction <> "F") { ?>
<span id="el_report_form_start_date">
<input type="text" data-table="report_form" data-field="x_start_date" data-page="1" data-format="2" name="x_start_date" id="x_start_date" size="18" placeholder="<?php echo ew_HtmlEncode($report_form->start_date->getPlaceHolder()) ?>" value="<?php echo $report_form->start_date->EditValue ?>"<?php echo $report_form->start_date->EditAttributes() ?>>
...
<script type="text/javascript">
ew_CreateDateTimePicker("freport_formedit", "x_start_date", {"ignoreReadonly":true,"useCurrent":false,"format":2});
</script>
<?php } ?>
</span>
<?php } else { ?>
...
<?php if ($report_form->end_date->Visible) { // end_date ?>
	<div id="r_end_date" class="form-group">
		<label id="elh_report_form_end_date" for="x_end_date" class="<?php echo $report_form_edit->LeftColumnClass ?>"><?php echo $report_form->end_date->FldCaption() ?></label>
		<div class="<?php echo $report_form_edit->RightColumnClass ?>"><div<?php echo $report_form->end_date->CellAttributes() ?>>
<?php if ($report_form->CurrentAction <> "F") { ?>
<span id="el_report_form_end_date">
<input type="text" data-table="report_form" data-field="x_end_date" data-page="1" data-format="2" name="x_end_date" id="x_end_date" size="18" placeholder="<?php echo ew_HtmlEncode($report_form->end_date->getPlaceHolder()) ?>" value="<?php echo $report_form->end_date->EditValue ?>"<?php echo $report_form->end_date->EditAttributes() ?>>
...
<script type="text/javascript">
ew_CreateDateTimePicker("freport_formedit", "x_end_date", {"ignoreReadonly":true,"useCurrent":false,"format":2});
</script>
<?php } ?>
</span>
<?php } else { ?>
...
<?php if ($report_form->duration->Visible) { // duration ?>
	<div id="r_duration" class="form-group">
		<label id="elh_report_form_duration" for="x_duration" class="<?php echo $report_form_edit->LeftColumnClass ?>"><?php echo $report_form->duration->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $report_form_edit->RightColumnClass ?>"><div<?php echo $report_form->duration->CellAttributes() ?>>
<?php if ($report_form->CurrentAction <> "F") { ?>
...
<?php if ($report_form->amount_paid->Visible) { // amount_paid ?>
	<div id="r_amount_paid" class="form-group">
		<label id="elh_report_form_amount_paid" for="x_amount_paid" class="<?php echo $report_form_edit->LeftColumnClass ?>"><?php echo $report_form->amount_paid->FldCaption() ?></label>
		<div class="<?php echo $report_form_edit->RightColumnClass ?>"><div<?php echo $report_form->amount_paid->CellAttributes() ?>>
<?php if ($report_form->CurrentAction <> "F") { ?>
...
<?php if ($report_form->no_of_people_involved->Visible) { // no_of_people_involved ?>
	<div id="r_no_of_people_involved" class="form-group">
		<label id="elh_report_form_no_of_people_involved" for="x_no_of_people_involved" class="<?php echo $report_form_edit->LeftColumnClass ?>"><?php echo $report_form->no_of_people_involved->FldCaption() ?></label>
		<div class="<?php echo $report_form_edit->RightColumnClass ?>"><div<?php echo $report_form->no_of_people_involved->CellAttributes() ?>>
<?php if ($report_form->CurrentAction <> "F") { ?>
...
<?php if ($report_form->incident_type->Visible) { // incident_type ?>
	<div id="r_incident_type" class="form-group">
		<label id="elh_report_form_incident_type" for="x_incident_type" class="<?php echo $report_form_edit->LeftColumnClass ?>"><?php echo $report_form->incident_type->FldCaption() ?></label>
		<div class="<?php echo $report_form_edit->RightColumnClass ?>"><div<?php echo $report_form->incident_type->CellAttributes() ?>>
<?php if ($report_form->CurrentAction <> "F") { ?>
...
<?php if ($report_form->incident_category->Visible) { // incident-category ?>
	<div id="r_incident_category" class="form-group">
		<label id="elh_report_form_incident_category" for="x_incident_category" class="<?php echo $report_form_edit->LeftColumnClass ?>"><?php echo $report_form->incident_category->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $report_form_edit->RightColumnClass ?>"><div<?php echo $report_form->incident_category->CellAttributes() ?>>
<?php if ($report_form->CurrentAction <> "F") { ?>
...
<?php if ($report_form->incident_location->Visible) { // incident_location ?>
	<div id="r_incident_location" class="form-group">
		<label id="elh_report_form_incident_location" for="x_incident_location" class="<?php echo $report_form_edit->LeftColumnClass ?>"><?php echo $report_form->incident_location->FldCaption() ?></label>
		<div class="<?php echo $report_form_edit->RightColumnClass ?>"><div<?php echo $report_form->incident_location->CellAttributes() ?>>
<?php if ($report_form->CurrentAction <> "F") { ?>
...
<?php if ($report_form->incident_description->Visible) { // incident_description ?>
	<div id="r_incident_description" class="form-group">
		<label id="elh_report_form_incident_description" class="<?php echo $report_form_edit->LeftColumnClass ?>"><?php echo $report_form->incident_description->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $report_form_edit->RightColumnClass ?>"><div<?php echo $report_form->incident_description->CellAttributes() ?>>
<?php if ($report_form->CurrentAction <> "F") { ?>
<span id="el_report_form_incident_description">
<?php ew_AppendClass($report_form->incident_description->EditAttrs["class"], "editor"); ?>
...
<script type="text/javascript">
ew_CreateEditor("freport_formedit", "x_incident_description", 35, 4, <?php echo ($report_form->incident_description->ReadOnly || FALSE) ? "true" : "false" ?>);
</script>
</span>
<?php } else { ?>
<span id="el_report_form_incident_description">
...
<?php if ($report_form->_upload->Visible) { // upload ?>
	<div id="r__upload" class="form-group">
		<label id="elh_report_form__upload" class="<?php echo $report_form_edit->LeftColumnClass ?>"><?php echo $report_form->_upload->FldCaption() ?></label>
		<div class="<?php echo $report_form_edit->RightColumnClass ?>"><div<?php echo $report_form->_upload->CellAttributes() ?>>
...
<?php } ?>
<input type="hidden" name="fs_x__upload" id= "fs_x__upload" value="120">
...
<?php } ?>
		...
		<div class="tab-pane<?php echo $report_form_edit->MultiPages->PageStyle("2") ?>" id="tab_report_form2"><!-- multi-page .tab-pane -->
<div class="ewEditDiv"><!-- page* -->
<?php if ($report_form->status->Visible) { // status ?>
	<div id="r_status" class="form-group">
		<label id="elh_report_form_status" for="x_status" class="<?php echo $report_form_edit->LeftColumnClass ?>"><?php echo $report_form->status->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $report_form_edit->RightColumnClass ?>"><div<?php echo $report_form->status->CellAttributes() ?>>
<?php if ($report_form->CurrentAction <> "F") { ?>
...
<?php } ?>
<?php if ($report_form->operational_status->Visible) { // operational_status ?>
	<div id="r_operational_status" class="form-group">
		<label id="elh_report_form_operational_status" for="x_operational_status" class="<?php echo $report_form_edit->LeftColumnClass ?>"><?php echo $report_form->operational_status->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $report_form_edit->RightColumnClass ?>"><div<?php echo $report_form->operational_status->CellAttributes() ?>>
<?php if ($report_form->CurrentAction <> "F") { ?>
<span id="el_report_form_operational_status">
<select data-table="report_form" data-field="x_operational_status" data-page="2" data-value-separator="<?php echo $report_form->operational_status->DisplayValueSeparatorAttribute() ?>" id="x_operational_status" name="x_operational_status"<?php echo $report_form->operational_status->EditAttributes() ?>>
<?php echo $report_form->operational_status->SelectOptionListHtml("x_operational_status") ?>
</select>
</span>
<?php } else { ?>
<span id="el_report_form_operational_status">
<span<?php echo $report_form->operational_status->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $report_form->operational_status->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="report_form" data-field="x_operational_status" data-page="2" name="x_operational_status" id="x_operational_status" value="<?php echo ew_HtmlEncode($report_form->operational_status->FormValue) ?>">
<?php } ?>
<?php echo $report_form->operational_status->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($report_form->initiator_action->Visible) { // initiator_action ?>
	<div id="r_initiator_action" class="form-group">
		<label id="elh_report_form_initiator_action" class="<?php echo $report_form_edit->LeftColumnClass ?>"><?php echo $report_form->initiator_action->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $report_form_edit->RightColumnClass ?>"><div<?php echo $report_form->initiator_action->CellAttributes() ?>>
<?php if ($report_form->CurrentAction <> "F") { ?>
...
<?php if ($report_form->initiator_comment->Visible) { // initiator_comment ?>
	<div id="r_initiator_comment" class="form-group">
		<label id="elh_report_form_initiator_comment" for="x_initiator_comment" class="<?php echo $report_form_edit->LeftColumnClass ?>"><?php echo $report_form->initiator_comment->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $report_form_edit->RightColumnClass ?>"><div<?php echo $report_form->initiator_comment->CellAttributes() ?>>
<?php if ($report_form->CurrentAction <> "F") { ?>
...
<?php if ($report_form->report_by->Visible) { // report_by ?>
	<div id="r_report_by" class="form-group">
		<label id="elh_report_form_report_by" class="<?php echo $report_form_edit->LeftColumnClass ?>"><?php echo $report_form->report_by->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $report_form_edit->RightColumnClass ?>"><div<?php echo $report_form->report_by->CellAttributes() ?>>
<?php if ($report_form->CurrentAction <> "F") { ?>
...
?>
<span id="as_x_report_by" style="white-space: nowrap; z-index: 8750">
...
<script type="text/javascript">
freport_formedit.CreateAutoSuggest({"id":"x_report_by","forceSelect":false});
</script>
</span>
<?php } else { ?>
<span id="el_report_form_report_by">
...
<?php if ($report_form->datetime_resolved->Visible) { // datetime_resolved ?>
	<div id="r_datetime_resolved" class="form-group">
		<label id="elh_report_form_datetime_resolved" for="x_datetime_resolved" class="<?php echo $report_form_edit->LeftColumnClass ?>"><?php echo $report_form->datetime_resolved->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $report_form_edit->RightColumnClass ?>"><div<?php echo $report_form->datetime_resolved->CellAttributes() ?>>
<?php if ($report_form->CurrentAction <> "F") { ?>
...
<?php if ($report_form->resolved_action->Visible) { // resolved_action ?>
	<div id="r_resolved_action" class="form-group">
		<label id="elh_report_form_resolved_action" class="<?php echo $report_form_edit->LeftColumnClass ?>"><?php echo $report_form->resolved_action->FldCaption() ?></label>
		<div class="<?php echo $report_form_edit->RightColumnClass ?>"><div<?php echo $report_form->resolved_action->CellAttributes() ?>>
<?php if ($report_form->CurrentAction <> "F") { ?>
...
<?php if ($report_form->resolved_comment->Visible) { // resolved_comment ?>
	<div id="r_resolved_comment" class="form-group">
		<label id="elh_report_form_resolved_comment" for="x_resolved_comment" class="<?php echo $report_form_edit->LeftColumnClass ?>"><?php echo $report_form->resolved_comment->FldCaption() ?></label>
		<div class="<?php echo $report_form_edit->RightColumnClass ?>"><div<?php echo $report_form->resolved_comment->CellAttributes() ?>>
<?php if ($report_form->CurrentAction <> "F") { ?>
...
<?php if ($report_form->resolved_by->Visible) { // resolved_by ?>
	<div id="r_resolved_by" class="form-group">
		<label id="elh_report_form_resolved_by" class="<?php echo $report_form_edit->LeftColumnClass ?>"><?php echo $report_form->resolved_by->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $report_form_edit->RightColumnClass ?>"><div<?php echo $report_form->resolved_by->CellAttributes() ?>>
<?php if ($report_form->CurrentAction <> "F") { ?>
...
?>
<span id="as_x_resolved_by" style="white-space: nowrap; z-index: 8710">
...
<script type="text/javascript">
freport_formedit.CreateAutoSuggest({"id":"x_resolved_by","forceSelect":false});
</script>
</span>
<?php } else { ?>
<span id="el_report_form_resolved_by">
...
<?php if ($report_form->datetime_approved->Visible) { // datetime_approved ?>
	<div id="r_datetime_approved" class="form-group">
		<label id="elh_report_form_datetime_approved" for="x_datetime_approved" class="<?php echo $report_form_edit->LeftColumnClass ?>"><?php echo $report_form->datetime_approved->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $report_form_edit->RightColumnClass ?>"><div<?php echo $report_form->datetime_approved->CellAttributes() ?>>
<?php if ($report_form->CurrentAction <> "F") { ?>
...
<?php if ($report_form->approval_action->Visible) { // approval_action ?>
	<div id="r_approval_action" class="form-group">
		<label id="elh_report_form_approval_action" class="<?php echo $report_form_edit->LeftColumnClass ?>"><?php echo $report_form->approval_action->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $report_form_edit->RightColumnClass ?>"><div<?php echo $report_form->approval_action->CellAttributes() ?>>
<?php if ($report_form->CurrentAction <> "F") { ?>
...
<?php if ($report_form->approval_comment->Visible) { // approval_comment ?>
	<div id="r_approval_comment" class="form-group">
		<label id="elh_report_form_approval_comment" for="x_approval_comment" class="<?php echo $report_form_edit->LeftColumnClass ?>"><?php echo $report_form->approval_comment->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $report_form_edit->RightColumnClass ?>"><div<?php echo $report_form->approval_comment->CellAttributes() ?>>
<?php if ($report_form->CurrentAction <> "F") { ?>
...
<?php if ($report_form->approved_by->Visible) { // approved_by ?>
	<div id="r_approved_by" class="form-group">
		<label id="elh_report_form_approved_by" class="<?php echo $report_form_edit->LeftColumnClass ?>"><?php echo $report_form->approved_by->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $report_form_edit->RightColumnClass ?>"><div<?php echo $report_form->approved_by->CellAttributes() ?>>
<?php if ($report_form->CurrentAction <> "F") { ?>
...
?>
<span id="as_x_approved_by" style="white-space: nowrap; z-index: 8670">
...
<script type="text/javascript">
freport_formedit.CreateAutoSuggest({"id":"x_approved_by","forceSelect":false});
</script>
</span>
<?php } else { ?>
<span id="el_report_form_approved_by">
...
<?php if ($report_form->closure_action->Visible) { // closure_action ?>
	<div id="r_closure_action" class="form-group">
		<label id="elh_report_form_closure_action" class="<?php echo $report_form_edit->LeftColumnClass ?>"><?php echo $report_form->closure_action->FldCaption() ?></label>
		<div class="<?php echo $report_form_edit->RightColumnClass ?>"><div<?php echo $report_form->closure_action->CellAttributes() ?>>
<?php if ($report_form->CurrentAction <> "F") { ?>
<span id="el_report_form_closure_action">
<div id="tp_x_closure_action" class="ewTemplate"><input type="radio" data-table="report_form" data-field="x_closure_action" data-page="2" data-value-separator="<?php echo $report_form->closure_action->DisplayValueSeparatorAttribute() ?>" name="x_closure_action" id="x_closure_action" value="{value}"<?php echo $report_form->closure_action->EditAttributes() ?>></div>
<div id="dsl_x_closure_action" data-repeatcolumn="5" class="ewItemList" style="display: none;"><div>
<?php echo $report_form->closure_action->RadioButtonListHtml(FALSE, "x_closure_action", 2) ?>
</div></div>
</span>
<?php } else { ?>
<span id="el_report_form_closure_action">
<span<?php echo $report_form->closure_action->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $report_form->closure_action->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="report_form" data-field="x_closure_action" data-page="2" name="x_closure_action" id="x_closure_action" value="<?php echo ew_HtmlEncode($report_form->closure_action->FormValue) ?>">
<?php } ?>
<?php echo $report_form->closure_action->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($report_form->closure_comment->Visible) { // closure_comment ?>
	<div id="r_closure_comment" class="form-group">
		<label id="elh_report_form_closure_comment" for="x_closure_comment" class="<?php echo $report_form_edit->LeftColumnClass ?>"><?php echo $report_form->closure_comment->FldCaption() ?></label>
		<div class="<?php echo $report_form_edit->RightColumnClass ?>"><div<?php echo $report_form->closure_comment->CellAttributes() ?>>
<?php if ($report_form->CurrentAction <> "F") { ?>
<span id="el_report_form_closure_comment">
<textarea data-table="report_form" data-field="x_closure_comment" data-page="2" name="x_closure_comment" id="x_closure_comment" cols="35" rows="4" placeholder="<?php echo ew_HtmlEncode($report_form->closure_comment->getPlaceHolder()) ?>"<?php echo $report_form->closure_comment->EditAttributes() ?>><?php echo $report_form->closure_comment->EditValue ?></textarea>
</span>
<?php } else { ?>
<span id="el_report_form_closure_comment">
<span<?php echo $report_form->closure_comment->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $report_form->closure_comment->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="report_form" data-field="x_closure_comment" data-page="2" name="x_closure_comment" id="x_closure_comment" value="<?php echo ew_HtmlEncode($report_form->closure_comment->FormValue) ?>">
<?php } ?>
<?php echo $report_form->closure_comment->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($report_form->last_updated_date->Visible) { // last_updated_date ?>
	<div id="r_last_updated_date" class="form-group">
		<label id="elh_report_form_last_updated_date" for="x_last_updated_date" class="<?php echo $report_form_edit->LeftColumnClass ?>"><?php echo $report_form->last_updated_date->FldCaption() ?></label>
		<div class="<?php echo $report_form_edit->RightColumnClass ?>"><div<?php echo $report_form->last_updated_date->CellAttributes() ?>>
<?php if ($report_form->CurrentAction <> "F") { ?>
...
<?php if ($report_form->last_updated_by->Visible) { // last_updated_by ?>
	<div id="r_last_updated_by" class="form-group">
		<label id="elh_report_form_last_updated_by" class="<?php echo $report_form_edit->LeftColumnClass ?>"><?php echo $report_form->last_updated_by->FldCaption() ?></label>
		<div class="<?php echo $report_form_edit->RightColumnClass ?>"><div<?php echo $report_form->last_updated_by->CellAttributes() ?>>
<?php if ($report_form->CurrentAction <> "F") { ?>
...
?>
<span id="as_x_last_updated_by" style="white-space: nowrap; z-index: 8630">
...
<script type="text/javascript">
freport_formedit.CreateAutoSuggest({"id":"x_last_updated_by","forceSelect":false});
</script>
</span>
<?php } else { ?>
<span id="el_report_form_last_updated_by">
...
<input type="hidden" data-table="report_form" data-field="x_id" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($report_form->id->CurrentValue) ?>">
<?php if (!$report_form_edit->IsModal) { ?>
<div class="form-group"><!-- buttons .form-group -->
	<div class="<?php echo $report_form_edit->OffsetColumnClass ?>"><!-- buttons offset -->
<?php if ($report_form->CurrentAction <> "F") { // Confirm page ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $report_form_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
<?php } else { ?>
...
<script type="text/javascript">
freport_formedit.Init();
</script>
<?php
$report_form_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
var departments = $("#x_departments").val()
var currlevel = "<?php echo CurrentUserLevel() ?>";
$('#x_departments').attr('readonly',true);
;

//var test = $("#x_incident_id").val()
if(currlevel == 2 && departments == 6){
	$("#x_resolved_action_2").attr('disabled',false);
}else{
...
<?php
$report_form_edit->Page_Terminate();
?>
