...
<?php include_once "resolved_issueinfo.php" ?>
...
$resolved_issue_list = NULL; // Initialize page object first

class cresolved_issue_list extends cresolved_issue {
	...
	// Table name
	var $TableName = 'resolved_issue';

	// Page object name
	var $PageObjName = 'resolved_issue_list';

	// Grid form hidden field names
	var $FormName = 'fresolved_issuelist';
	...
	function __construct() {
		...
		// Table object (resolved_issue)
		if (!isset($GLOBALS["resolved_issue"]) || get_class($GLOBALS["resolved_issue"]) == "cresolved_issue") {
			$GLOBALS["resolved_issue"] = &$this;
			$GLOBALS["Table"] = &$GLOBALS["resolved_issue"];
		}
		...
		$this->AddUrl = "resolved_issueadd.php";
		$this->InlineAddUrl = $this->PageUrl() . "a=add";
		$this->GridAddUrl = $this->PageUrl() . "a=gridadd";
		$this->GridEditUrl = $this->PageUrl() . "a=gridedit";
		$this->MultiDeleteUrl = "resolved_issuedelete.php";
		$this->MultiUpdateUrl = "resolved_issueupdate.php";
		...
		if (!defined("EW_TABLE_NAME"))
			define("EW_TABLE_NAME", 'resolved_issue', TRUE);
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fresolved_issuelistsrch";
	...
	function Page_Terminate($url = "") {
		...
		// Export
		global $EW_EXPORT, $resolved_issue;
		if ($this->CustomExport <> "" && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, $EW_EXPORT)) {
				$sContent = ob_get_contents();
			if ($gsExportFile == "") $gsExportFile = $this->TableVar;
			$class = $EW_EXPORT[$this->CustomExport];
			if (class_exists($class)) {
				$doc = new $class($resolved_issue);
				$doc->Text = $sContent;
	...
	function GetFilterList() {
		...
		if (EW_SEARCH_FILTER_OPTION == "Server" && isset($UserProfile))
			$sSavedFilterList = $UserProfile->GetSearchFilters(CurrentUserName(), "fresolved_issuelistsrch");
	...
	function ProcessFilterList() {
		...
		if (@$_POST["ajax"] == "savefilters") { // Save filter request (Ajax)
			...
			$UserProfile->SetSearchFilters(CurrentUserName(), "fresolved_issuelistsrch", $filters);
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fresolved_issuelistsrch\" href=\"#\">" . $Language->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fresolved_issuelistsrch\" href=\"#\">" . $Language->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->ListActions->Items as $listaction) {
				if ($listaction->Select == EW_ACTION_MULTIPLE) {
					...
					$item->Body = "<a class=\"ewAction ewListAction\" title=\"" . ew_HtmlEncode($caption) . "\" data-caption=\"" . ew_HtmlEncode($caption) . "\" href=\"\" onclick=\"ew_SubmitAction(event,jQuery.extend({f:document.fresolved_issuelist}," . $listaction->ToJson(TRUE) . "));return false;\">" . $icon . "</a>";
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $Language->Phrase("SearchPanel") . "\" data-caption=\"" . $Language->Phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fresolved_issuelistsrch\">" . $Language->Phrase("SearchLink") . "</button>";
		$item->Visible = TRUE;
	...
	function SetupExportOptions() {
		...
		$url = "";
		$item->Body = "<button id=\"emf_resolved_issue\" class=\"ewExportLink ewEmail\" title=\"" . $Language->Phrase("ExportToEmailText") . "\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_resolved_issue',hdr:ewLanguage.Phrase('ExportToEmailText'),f:document.fresolved_issuelist,sel:false" . $url . "});\">" . $Language->Phrase("ExportToEmail") . "</button>";
		$item->Visible = FALSE;
...
// Create page object
if (!isset($resolved_issue_list)) $resolved_issue_list = new cresolved_issue_list();

// Page init
$resolved_issue_list->Page_Init();

// Page main
$resolved_issue_list->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$resolved_issue_list->Page_Render();
?>
<?php include_once "header.php" ?>
<?php if ($resolved_issue->Export == "") { ?>
<script type="text/javascript">

// Form object
var CurrentPageID = EW_PAGE_ID = "list";
var CurrentForm = fresolved_issuelist = new ew_Form("fresolved_issuelist", "list");
fresolved_issuelist.FormKeyCountName = '<?php echo $resolved_issue_list->FormKeyCountName ?>';

// Form_CustomValidate event
fresolved_issuelist.Form_CustomValidate = 
...
fresolved_issuelist.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fresolved_issuelist.Lists["x_staffid"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_staffno","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"users"};
fresolved_issuelist.Lists["x_staffid"].Data = "<?php echo $resolved_issue_list->staffid->LookupFilterQuery(FALSE, "list") ?>";
fresolved_issuelist.AutoSuggests["x_staffid"] = <?php echo json_encode(array("data" => "ajax=autosuggest&" . $resolved_issue_list->staffid->LookupFilterQuery(TRUE, "list"))) ?>;
fresolved_issuelist.Lists["x_staff_id"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_firstname","x_lastname","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"users"};
fresolved_issuelist.Lists["x_staff_id"].Data = "<?php echo $resolved_issue_list->staff_id->LookupFilterQuery(FALSE, "list") ?>";
fresolved_issuelist.Lists["x_branch"] = {"LinkField":"x_branch_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_branch_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"branch"};
fresolved_issuelist.Lists["x_branch"].Data = "<?php echo $resolved_issue_list->branch->LookupFilterQuery(FALSE, "list") ?>";
fresolved_issuelist.Lists["x_departments"] = {"LinkField":"x_code_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_description","","",""],"ParentFields":[],"ChildFields":["x_category"],"FilterFields":[],"Options":[],"Template":"","LinkTable":"departments"};
fresolved_issuelist.Lists["x_departments"].Data = "<?php echo $resolved_issue_list->departments->LookupFilterQuery(FALSE, "list") ?>";
fresolved_issuelist.Lists["x_category"] = {"LinkField":"x_category_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_description","","",""],"ParentFields":[],"ChildFields":["x_sub_category"],"FilterFields":[],"Options":[],"Template":"","LinkTable":"category"};
fresolved_issuelist.Lists["x_category"].Data = "<?php echo $resolved_issue_list->category->LookupFilterQuery(FALSE, "list") ?>";
fresolved_issuelist.Lists["x_sub_category"] = {"LinkField":"x_sub_category_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_sub_category_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"sub_category"};
fresolved_issuelist.Lists["x_sub_category"].Data = "<?php echo $resolved_issue_list->sub_category->LookupFilterQuery(FALSE, "list") ?>";
fresolved_issuelist.Lists["x_status"] = {"LinkField":"x_code","Ajax":true,"AutoFill":false,"DisplayFields":["x_description","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"status"};
fresolved_issuelist.Lists["x_status"].Data = "<?php echo $resolved_issue_list->status->LookupFilterQuery(FALSE, "list") ?>";
fresolved_issuelist.Lists["x_last_updated_by"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_firstname","x_lastname","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"users"};
fresolved_issuelist.Lists["x_last_updated_by"].Data = "<?php echo $resolved_issue_list->last_updated_by->LookupFilterQuery(FALSE, "list") ?>";
fresolved_issuelist.AutoSuggests["x_last_updated_by"] = <?php echo json_encode(array("data" => "ajax=autosuggest&" . $resolved_issue_list->last_updated_by->LookupFilterQuery(TRUE, "list"))) ?>;

// Form object for search
var CurrentSearchForm = fresolved_issuelistsrch = new ew_Form("fresolved_issuelistsrch");

// Validate function for search
fresolved_issuelistsrch.Validate = function(fobj) {
...
// Form_CustomValidate event
fresolved_issuelistsrch.Form_CustomValidate = 
 function(fobj) { // DO NOT CHANGE THIS LINE!
...
// Use JavaScript validation or not
fresolved_issuelistsrch.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fresolved_issuelistsrch.Lists["x_staff_id"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_firstname","x_lastname","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"users"};
fresolved_issuelistsrch.Lists["x_staff_id"].Data = "<?php echo $resolved_issue_list->staff_id->LookupFilterQuery(FALSE, "extbs") ?>";
fresolved_issuelistsrch.Lists["x_status"] = {"LinkField":"x_code","Ajax":true,"AutoFill":false,"DisplayFields":["x_description","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"status"};
fresolved_issuelistsrch.Lists["x_status"].Data = "<?php echo $resolved_issue_list->status->LookupFilterQuery(FALSE, "extbs") ?>";
</script>
<script type="text/javascript">
...
<?php if ($resolved_issue->Export == "") { ?>
<div class="ewToolbar">
<?php if ($resolved_issue_list->TotalRecs > 0 && $resolved_issue_list->ExportOptions->Visible()) { ?>
<?php $resolved_issue_list->ExportOptions->Render("body") ?>
<?php } ?>
<?php if ($resolved_issue_list->SearchOptions->Visible()) { ?>
<?php $resolved_issue_list->SearchOptions->Render("body") ?>
<?php } ?>
<?php if ($resolved_issue_list->FilterOptions->Visible()) { ?>
<?php $resolved_issue_list->FilterOptions->Render("body") ?>
<?php } ?>
<div class="clearfix"></div>
</div>
<?php } ?>
<?php
	$bSelectLimit = $resolved_issue_list->UseSelectLimit;
	if ($bSelectLimit) {
		if ($resolved_issue_list->TotalRecs <= 0)
			$resolved_issue_list->TotalRecs = $resolved_issue->ListRecordCount();
	} else {
		if (!$resolved_issue_list->Recordset && ($resolved_issue_list->Recordset = $resolved_issue_list->LoadRecordset()))
			$resolved_issue_list->TotalRecs = $resolved_issue_list->Recordset->RecordCount();
	}
	$resolved_issue_list->StartRec = 1;
	if ($resolved_issue_list->DisplayRecs <= 0 || ($resolved_issue->Export <> "" && $resolved_issue->ExportAll)) // Display all records
		$resolved_issue_list->DisplayRecs = $resolved_issue_list->TotalRecs;
	if (!($resolved_issue->Export <> "" && $resolved_issue->ExportAll))
		$resolved_issue_list->SetupStartRec(); // Set up start record position
	if ($bSelectLimit)
		$resolved_issue_list->Recordset = $resolved_issue_list->LoadRecordset($resolved_issue_list->StartRec-1, $resolved_issue_list->DisplayRecs);

	// Set no record found message
	if ($resolved_issue->CurrentAction == "" && $resolved_issue_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$resolved_issue_list->setWarningMessage(ew_DeniedMsg());
		if ($resolved_issue_list->SearchWhere == "0=101")
			$resolved_issue_list->setWarningMessage($Language->Phrase("EnterSearchCriteria"));
		else
			$resolved_issue_list->setWarningMessage($Language->Phrase("NoRecord"));
	}
$resolved_issue_list->RenderOtherOptions();
?>
<?php if ($Security->CanSearch()) { ?>
<?php if ($resolved_issue->Export == "" && $resolved_issue->CurrentAction == "") { ?>
<form name="fresolved_issuelistsrch" id="fresolved_issuelistsrch" class="form-inline ewForm ewExtSearchForm" action="<?php echo ew_CurrentPage() ?>">
<?php $SearchPanelClass = ($resolved_issue_list->SearchWhere <> "") ? " in" : " in"; ?>
<div id="fresolved_issuelistsrch_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
<input type="hidden" name="cmd" value="search">
<input type="hidden" name="t" value="resolved_issue">
	<div class="ewBasicSearch">
<?php
if ($gsSearchError == "")
	$resolved_issue_list->LoadAdvancedSearch(); // Load advanced search

// Render for search
$resolved_issue->RowType = EW_ROWTYPE_SEARCH;

// Render row
$resolved_issue->ResetAttrs();
$resolved_issue_list->RenderRow();
?>
<div id="xsr_1" class="ewRow">
<?php if ($resolved_issue->incident_id->Visible) { // incident_id ?>
	<div id="xsc_incident_id" class="ewCell form-group">
		<label for="x_incident_id" class="ewSearchCaption ewLabel"><?php echo $resolved_issue->incident_id->FldCaption() ?></label>
...
<input type="text" data-table="resolved_issue" data-field="x_incident_id" name="x_incident_id" id="x_incident_id" size="18" placeholder="<?php echo ew_HtmlEncode($resolved_issue->incident_id->getPlaceHolder()) ?>" value="<?php echo $resolved_issue->incident_id->EditValue ?>"<?php echo $resolved_issue->incident_id->EditAttributes() ?>>
</span>
	</div>
<?php } ?>
</div>
<div id="xsr_2" class="ewRow">
<?php if ($resolved_issue->staff_id->Visible) { // staff_id ?>
	<div id="xsc_staff_id" class="ewCell form-group">
		<label for="x_staff_id" class="ewSearchCaption ewLabel"><?php echo $resolved_issue->staff_id->FldCaption() ?></label>
		<span class="ewSearchOperator"><?php echo $Language->Phrase("=") ?><input type="hidden" name="z_staff_id" id="z_staff_id" value="="></span>
		<span class="ewSearchField">
<select data-table="resolved_issue" data-field="x_staff_id" data-value-separator="<?php echo $resolved_issue->staff_id->DisplayValueSeparatorAttribute() ?>" id="x_staff_id" name="x_staff_id"<?php echo $resolved_issue->staff_id->EditAttributes() ?>>
<?php echo $resolved_issue->staff_id->SelectOptionListHtml("x_staff_id") ?>
</select>
</span>
	</div>
<?php } ?>
...
<?php if ($resolved_issue->status->Visible) { // status ?>
	<div id="xsc_status" class="ewCell form-group">
		<label for="x_status" class="ewSearchCaption ewLabel"><?php echo $resolved_issue->status->FldCaption() ?></label>
		<span class="ewSearchOperator"><?php echo $Language->Phrase("=") ?><input type="hidden" name="z_status" id="z_status" value="="></span>
		<span class="ewSearchField">
<select data-table="resolved_issue" data-field="x_status" data-value-separator="<?php echo $resolved_issue->status->DisplayValueSeparatorAttribute() ?>" id="x_status" name="x_status"<?php echo $resolved_issue->status->EditAttributes() ?>>
<?php echo $resolved_issue->status->SelectOptionListHtml("x_status") ?>
</select>
</span>
	</div>
<?php } ?>
...
<div id="xsr_4" class="ewRow">
	...
	<input type="text" name="<?php echo EW_TABLE_BASIC_SEARCH ?>" id="<?php echo EW_TABLE_BASIC_SEARCH ?>" class="form-control" value="<?php echo ew_HtmlEncode($resolved_issue_list->BasicSearch->getKeyword()) ?>" placeholder="<?php echo ew_HtmlEncode($Language->Phrase("Search")) ?>">
	<input type="hidden" name="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" id="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" value="<?php echo ew_HtmlEncode($resolved_issue_list->BasicSearch->getType()) ?>">
	<div class="input-group-btn">
		<button type="button" data-toggle="dropdown" class="btn btn-default"><span id="searchtype"><?php echo $resolved_issue_list->BasicSearch->getTypeNameShort() ?></span><span class="caret"></span></button>
		<ul class="dropdown-menu pull-right" role="menu">
			<li<?php if ($resolved_issue_list->BasicSearch->getType() == "") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this)"><?php echo $Language->Phrase("QuickSearchAuto") ?></a></li>
			<li<?php if ($resolved_issue_list->BasicSearch->getType() == "=") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'=')"><?php echo $Language->Phrase("QuickSearchExact") ?></a></li>
			<li<?php if ($resolved_issue_list->BasicSearch->getType() == "AND") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'AND')"><?php echo $Language->Phrase("QuickSearchAll") ?></a></li>
			<li<?php if ($resolved_issue_list->BasicSearch->getType() == "OR") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'OR')"><?php echo $Language->Phrase("QuickSearchAny") ?></a></li>
		</ul>
...
<?php } ?>
<?php } ?>
<?php $resolved_issue_list->ShowPageHeader(); ?>
<?php
$resolved_issue_list->ShowMessage();
?>
<?php if ($resolved_issue_list->TotalRecs > 0 || $resolved_issue->CurrentAction <> "") { ?>
<div class="box ewBox ewGrid<?php if ($resolved_issue_list->IsAddOrEdit()) { ?> ewGridAddEdit<?php } ?> resolved_issue">
<?php if ($resolved_issue->Export == "") { ?>
<div class="box-header ewGridUpperPanel">
<?php if ($resolved_issue->CurrentAction <> "gridadd" && $resolved_issue->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="form-inline ewForm ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($resolved_issue_list->Pager)) $resolved_issue_list->Pager = new cPrevNextPager($resolved_issue_list->StartRec, $resolved_issue_list->DisplayRecs, $resolved_issue_list->TotalRecs, $resolved_issue_list->AutoHidePager) ?>
<?php if ($resolved_issue_list->Pager->RecordCount > 0 && $resolved_issue_list->Pager->Visible) { ?>
<div class="ewPager">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($resolved_issue_list->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $resolved_issue_list->PageUrl() ?>start=<?php echo $resolved_issue_list->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($resolved_issue_list->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $resolved_issue_list->PageUrl() ?>start=<?php echo $resolved_issue_list->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $resolved_issue_list->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($resolved_issue_list->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $resolved_issue_list->PageUrl() ?>start=<?php echo $resolved_issue_list->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($resolved_issue_list->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $resolved_issue_list->PageUrl() ?>start=<?php echo $resolved_issue_list->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $resolved_issue_list->Pager->PageCount ?></span>
</div>
<?php } ?>
<?php if ($resolved_issue_list->Pager->RecordCount > 0) { ?>
<div class="ewPager ewRec">
	<span><?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $resolved_issue_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $resolved_issue_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $resolved_issue_list->Pager->RecordCount ?></span>
</div>
<?php } ?>
<?php if ($resolved_issue_list->TotalRecs > 0 && (!$resolved_issue_list->AutoHidePageSizeSelector || $resolved_issue_list->Pager->Visible)) { ?>
<div class="ewPager">
<input type="hidden" name="t" value="resolved_issue">
<select name="<?php echo EW_TABLE_REC_PER_PAGE ?>" class="form-control input-sm ewTooltip" title="<?php echo $Language->Phrase("RecordsPerPage") ?>" onchange="this.form.submit();">
<option value="5"<?php if ($resolved_issue_list->DisplayRecs == 5) { ?> selected<?php } ?>>5</option>
<option value="10"<?php if ($resolved_issue_list->DisplayRecs == 10) { ?> selected<?php } ?>>10</option>
<option value="15"<?php if ($resolved_issue_list->DisplayRecs == 15) { ?> selected<?php } ?>>15</option>
<option value="20"<?php if ($resolved_issue_list->DisplayRecs == 20) { ?> selected<?php } ?>>20</option>
<option value="50"<?php if ($resolved_issue_list->DisplayRecs == 50) { ?> selected<?php } ?>>50</option>
<option value="ALL"<?php if ($resolved_issue->getRecordsPerPage() == -1) { ?> selected<?php } ?>><?php echo $Language->Phrase("AllRecords") ?></option>
</select>
</div>
<?php } ?>
</form>
...
<?php
	foreach ($resolved_issue_list->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<div class="clearfix"></div>
...
<?php } ?>
<form name="fresolved_issuelist" id="fresolved_issuelist" class="form-inline ewForm ewListForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($resolved_issue_list->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $resolved_issue_list->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="resolved_issue">
<div id="gmp_resolved_issue" class="<?php if (ew_IsResponsiveLayout()) { ?>table-responsive <?php } ?>ewGridMiddlePanel">
<?php if ($resolved_issue_list->TotalRecs > 0 || $resolved_issue->CurrentAction == "gridedit") { ?>
<table id="tbl_resolved_issuelist" class="table ewTable">
<thead>
	<tr class="ewTableHeader">
<?php

// Header row
$resolved_issue_list->RowType = EW_ROWTYPE_HEADER;

// Render list options
$resolved_issue_list->RenderListOptions();

// Render list options (header, left)
$resolved_issue_list->ListOptions->Render("header", "left");
?>
<?php if ($resolved_issue->datetime_initiated->Visible) { // datetime_initiated ?>
	<?php if ($resolved_issue->SortUrl($resolved_issue->datetime_initiated) == "") { ?>
		<th data-name="datetime_initiated" class="<?php echo $resolved_issue->datetime_initiated->HeaderCellClass() ?>"><div id="elh_resolved_issue_datetime_initiated" class="resolved_issue_datetime_initiated"><div class="ewTableHeaderCaption"><?php echo $resolved_issue->datetime_initiated->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="datetime_initiated" class="<?php echo $resolved_issue->datetime_initiated->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $resolved_issue->SortUrl($resolved_issue->datetime_initiated) ?>',1);"><div id="elh_resolved_issue_datetime_initiated" class="resolved_issue_datetime_initiated">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $resolved_issue->datetime_initiated->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($resolved_issue->datetime_initiated->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($resolved_issue->datetime_initiated->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($resolved_issue->incident_id->Visible) { // incident_id ?>
	<?php if ($resolved_issue->SortUrl($resolved_issue->incident_id) == "") { ?>
		<th data-name="incident_id" class="<?php echo $resolved_issue->incident_id->HeaderCellClass() ?>"><div id="elh_resolved_issue_incident_id" class="resolved_issue_incident_id"><div class="ewTableHeaderCaption"><?php echo $resolved_issue->incident_id->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="incident_id" class="<?php echo $resolved_issue->incident_id->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $resolved_issue->SortUrl($resolved_issue->incident_id) ?>',1);"><div id="elh_resolved_issue_incident_id" class="resolved_issue_incident_id">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $resolved_issue->incident_id->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($resolved_issue->incident_id->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($resolved_issue->incident_id->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($resolved_issue->staffid->Visible) { // staffid ?>
	<?php if ($resolved_issue->SortUrl($resolved_issue->staffid) == "") { ?>
		<th data-name="staffid" class="<?php echo $resolved_issue->staffid->HeaderCellClass() ?>"><div id="elh_resolved_issue_staffid" class="resolved_issue_staffid"><div class="ewTableHeaderCaption"><?php echo $resolved_issue->staffid->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="staffid" class="<?php echo $resolved_issue->staffid->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $resolved_issue->SortUrl($resolved_issue->staffid) ?>',1);"><div id="elh_resolved_issue_staffid" class="resolved_issue_staffid">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $resolved_issue->staffid->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($resolved_issue->staffid->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($resolved_issue->staffid->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($resolved_issue->staff_id->Visible) { // staff_id ?>
	<?php if ($resolved_issue->SortUrl($resolved_issue->staff_id) == "") { ?>
		<th data-name="staff_id" class="<?php echo $resolved_issue->staff_id->HeaderCellClass() ?>"><div id="elh_resolved_issue_staff_id" class="resolved_issue_staff_id"><div class="ewTableHeaderCaption"><?php echo $resolved_issue->staff_id->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="staff_id" class="<?php echo $resolved_issue->staff_id->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $resolved_issue->SortUrl($resolved_issue->staff_id) ?>',1);"><div id="elh_resolved_issue_staff_id" class="resolved_issue_staff_id">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $resolved_issue->staff_id->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($resolved_issue->staff_id->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($resolved_issue->staff_id->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($resolved_issue->branch->Visible) { // branch ?>
	<?php if ($resolved_issue->SortUrl($resolved_issue->branch) == "") { ?>
		<th data-name="branch" class="<?php echo $resolved_issue->branch->HeaderCellClass() ?>"><div id="elh_resolved_issue_branch" class="resolved_issue_branch"><div class="ewTableHeaderCaption"><?php echo $resolved_issue->branch->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="branch" class="<?php echo $resolved_issue->branch->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $resolved_issue->SortUrl($resolved_issue->branch) ?>',1);"><div id="elh_resolved_issue_branch" class="resolved_issue_branch">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $resolved_issue->branch->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($resolved_issue->branch->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($resolved_issue->branch->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($resolved_issue->departments->Visible) { // departments ?>
	<?php if ($resolved_issue->SortUrl($resolved_issue->departments) == "") { ?>
		<th data-name="departments" class="<?php echo $resolved_issue->departments->HeaderCellClass() ?>"><div id="elh_resolved_issue_departments" class="resolved_issue_departments"><div class="ewTableHeaderCaption"><?php echo $resolved_issue->departments->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="departments" class="<?php echo $resolved_issue->departments->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $resolved_issue->SortUrl($resolved_issue->departments) ?>',1);"><div id="elh_resolved_issue_departments" class="resolved_issue_departments">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $resolved_issue->departments->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($resolved_issue->departments->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($resolved_issue->departments->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($resolved_issue->category->Visible) { // category ?>
	<?php if ($resolved_issue->SortUrl($resolved_issue->category) == "") { ?>
		<th data-name="category" class="<?php echo $resolved_issue->category->HeaderCellClass() ?>"><div id="elh_resolved_issue_category" class="resolved_issue_category"><div class="ewTableHeaderCaption"><?php echo $resolved_issue->category->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="category" class="<?php echo $resolved_issue->category->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $resolved_issue->SortUrl($resolved_issue->category) ?>',1);"><div id="elh_resolved_issue_category" class="resolved_issue_category">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $resolved_issue->category->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($resolved_issue->category->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($resolved_issue->category->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($resolved_issue->sub_category->Visible) { // sub_category ?>
	<?php if ($resolved_issue->SortUrl($resolved_issue->sub_category) == "") { ?>
		<th data-name="sub_category" class="<?php echo $resolved_issue->sub_category->HeaderCellClass() ?>"><div id="elh_resolved_issue_sub_category" class="resolved_issue_sub_category"><div class="ewTableHeaderCaption"><?php echo $resolved_issue->sub_category->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="sub_category" class="<?php echo $resolved_issue->sub_category->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $resolved_issue->SortUrl($resolved_issue->sub_category) ?>',1);"><div id="elh_resolved_issue_sub_category" class="resolved_issue_sub_category">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $resolved_issue->sub_category->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($resolved_issue->sub_category->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($resolved_issue->sub_category->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($resolved_issue->incident_description->Visible) { // incident_description ?>
	<?php if ($resolved_issue->SortUrl($resolved_issue->incident_description) == "") { ?>
		<th data-name="incident_description" class="<?php echo $resolved_issue->incident_description->HeaderCellClass() ?>"><div id="elh_resolved_issue_incident_description" class="resolved_issue_incident_description"><div class="ewTableHeaderCaption"><?php echo $resolved_issue->incident_description->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="incident_description" class="<?php echo $resolved_issue->incident_description->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $resolved_issue->SortUrl($resolved_issue->incident_description) ?>',1);"><div id="elh_resolved_issue_incident_description" class="resolved_issue_incident_description">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $resolved_issue->incident_description->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($resolved_issue->incident_description->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($resolved_issue->incident_description->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($resolved_issue->status->Visible) { // status ?>
	<?php if ($resolved_issue->SortUrl($resolved_issue->status) == "") { ?>
		<th data-name="status" class="<?php echo $resolved_issue->status->HeaderCellClass() ?>"><div id="elh_resolved_issue_status" class="resolved_issue_status"><div class="ewTableHeaderCaption"><?php echo $resolved_issue->status->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="status" class="<?php echo $resolved_issue->status->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $resolved_issue->SortUrl($resolved_issue->status) ?>',1);"><div id="elh_resolved_issue_status" class="resolved_issue_status">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $resolved_issue->status->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($resolved_issue->status->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($resolved_issue->status->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($resolved_issue->last_updated_date->Visible) { // last_updated_date ?>
	<?php if ($resolved_issue->SortUrl($resolved_issue->last_updated_date) == "") { ?>
		<th data-name="last_updated_date" class="<?php echo $resolved_issue->last_updated_date->HeaderCellClass() ?>"><div id="elh_resolved_issue_last_updated_date" class="resolved_issue_last_updated_date"><div class="ewTableHeaderCaption"><?php echo $resolved_issue->last_updated_date->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="last_updated_date" class="<?php echo $resolved_issue->last_updated_date->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $resolved_issue->SortUrl($resolved_issue->last_updated_date) ?>',1);"><div id="elh_resolved_issue_last_updated_date" class="resolved_issue_last_updated_date">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $resolved_issue->last_updated_date->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($resolved_issue->last_updated_date->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($resolved_issue->last_updated_date->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($resolved_issue->last_updated_by->Visible) { // last_updated_by ?>
	<?php if ($resolved_issue->SortUrl($resolved_issue->last_updated_by) == "") { ?>
		<th data-name="last_updated_by" class="<?php echo $resolved_issue->last_updated_by->HeaderCellClass() ?>"><div id="elh_resolved_issue_last_updated_by" class="resolved_issue_last_updated_by"><div class="ewTableHeaderCaption"><?php echo $resolved_issue->last_updated_by->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="last_updated_by" class="<?php echo $resolved_issue->last_updated_by->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $resolved_issue->SortUrl($resolved_issue->last_updated_by) ?>',1);"><div id="elh_resolved_issue_last_updated_by" class="resolved_issue_last_updated_by">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $resolved_issue->last_updated_by->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($resolved_issue->last_updated_by->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($resolved_issue->last_updated_by->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php
...
$resolved_issue_list->ListOptions->Render("header", "right");
?>
	</tr>
</thead>
<tbody>
<?php
if ($resolved_issue->ExportAll && $resolved_issue->Export <> "") {
	$resolved_issue_list->StopRec = $resolved_issue_list->TotalRecs;
} else {

	// Set the last record to display
	if ($resolved_issue_list->TotalRecs > $resolved_issue_list->StartRec + $resolved_issue_list->DisplayRecs - 1)
		$resolved_issue_list->StopRec = $resolved_issue_list->StartRec + $resolved_issue_list->DisplayRecs - 1;
	else
		$resolved_issue_list->StopRec = $resolved_issue_list->TotalRecs;
}
$resolved_issue_list->RecCnt = $resolved_issue_list->StartRec - 1;
if ($resolved_issue_list->Recordset && !$resolved_issue_list->Recordset->EOF) {
	$resolved_issue_list->Recordset->MoveFirst();
	$bSelectLimit = $resolved_issue_list->UseSelectLimit;
	if (!$bSelectLimit && $resolved_issue_list->StartRec > 1)
		$resolved_issue_list->Recordset->Move($resolved_issue_list->StartRec - 1);
} elseif (!$resolved_issue->AllowAddDeleteRow && $resolved_issue_list->StopRec == 0) {
	$resolved_issue_list->StopRec = $resolved_issue->GridAddRowCount;
}

// Initialize aggregate
$resolved_issue->RowType = EW_ROWTYPE_AGGREGATEINIT;
$resolved_issue->ResetAttrs();
$resolved_issue_list->RenderRow();
while ($resolved_issue_list->RecCnt < $resolved_issue_list->StopRec) {
	$resolved_issue_list->RecCnt++;
	if (intval($resolved_issue_list->RecCnt) >= intval($resolved_issue_list->StartRec)) {
		$resolved_issue_list->RowCnt++;

		// Set up key count
		$resolved_issue_list->KeyCount = $resolved_issue_list->RowIndex;

		// Init row class and style
		$resolved_issue->ResetAttrs();
		$resolved_issue->CssClass = "";
		if ($resolved_issue->CurrentAction == "gridadd") {
		} else {
			$resolved_issue_list->LoadRowValues($resolved_issue_list->Recordset); // Load row values
		}
		$resolved_issue->RowType = EW_ROWTYPE_VIEW; // Render view

		// Set up row id / data-rowindex
		$resolved_issue->RowAttrs = array_merge($resolved_issue->RowAttrs, array('data-rowindex'=>$resolved_issue_list->RowCnt, 'id'=>'r' . $resolved_issue_list->RowCnt . '_resolved_issue', 'data-rowtype'=>$resolved_issue->RowType));
		...
		$resolved_issue_list->RenderListOptions();
?>
	<tr<?php echo $resolved_issue->RowAttributes() ?>>
<?php

// Render list options (body, left)
$resolved_issue_list->ListOptions->Render("body", "left", $resolved_issue_list->RowCnt);
?>
	<?php if ($resolved_issue->datetime_initiated->Visible) { // datetime_initiated ?>
		<td data-name="datetime_initiated"<?php echo $resolved_issue->datetime_initiated->CellAttributes() ?>>
<span id="el<?php echo $resolved_issue_list->RowCnt ?>_resolved_issue_datetime_initiated" class="resolved_issue_datetime_initiated">
<span<?php echo $resolved_issue->datetime_initiated->ViewAttributes() ?>>
<?php echo $resolved_issue->datetime_initiated->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($resolved_issue->incident_id->Visible) { // incident_id ?>
		<td data-name="incident_id"<?php echo $resolved_issue->incident_id->CellAttributes() ?>>
<span id="el<?php echo $resolved_issue_list->RowCnt ?>_resolved_issue_incident_id" class="resolved_issue_incident_id">
<span<?php echo $resolved_issue->incident_id->ViewAttributes() ?>>
<?php echo $resolved_issue->incident_id->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($resolved_issue->staffid->Visible) { // staffid ?>
		<td data-name="staffid"<?php echo $resolved_issue->staffid->CellAttributes() ?>>
<span id="el<?php echo $resolved_issue_list->RowCnt ?>_resolved_issue_staffid" class="resolved_issue_staffid">
<span<?php echo $resolved_issue->staffid->ViewAttributes() ?>>
<?php echo $resolved_issue->staffid->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($resolved_issue->staff_id->Visible) { // staff_id ?>
		<td data-name="staff_id"<?php echo $resolved_issue->staff_id->CellAttributes() ?>>
<span id="el<?php echo $resolved_issue_list->RowCnt ?>_resolved_issue_staff_id" class="resolved_issue_staff_id">
<span<?php echo $resolved_issue->staff_id->ViewAttributes() ?>>
<?php echo $resolved_issue->staff_id->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($resolved_issue->branch->Visible) { // branch ?>
		<td data-name="branch"<?php echo $resolved_issue->branch->CellAttributes() ?>>
<span id="el<?php echo $resolved_issue_list->RowCnt ?>_resolved_issue_branch" class="resolved_issue_branch">
<span<?php echo $resolved_issue->branch->ViewAttributes() ?>>
<?php echo $resolved_issue->branch->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($resolved_issue->departments->Visible) { // departments ?>
		<td data-name="departments"<?php echo $resolved_issue->departments->CellAttributes() ?>>
<span id="el<?php echo $resolved_issue_list->RowCnt ?>_resolved_issue_departments" class="resolved_issue_departments">
<span<?php echo $resolved_issue->departments->ViewAttributes() ?>>
<?php echo $resolved_issue->departments->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($resolved_issue->category->Visible) { // category ?>
		<td data-name="category"<?php echo $resolved_issue->category->CellAttributes() ?>>
<span id="el<?php echo $resolved_issue_list->RowCnt ?>_resolved_issue_category" class="resolved_issue_category">
<span<?php echo $resolved_issue->category->ViewAttributes() ?>>
<?php echo $resolved_issue->category->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($resolved_issue->sub_category->Visible) { // sub_category ?>
		<td data-name="sub_category"<?php echo $resolved_issue->sub_category->CellAttributes() ?>>
<span id="el<?php echo $resolved_issue_list->RowCnt ?>_resolved_issue_sub_category" class="resolved_issue_sub_category">
<span<?php echo $resolved_issue->sub_category->ViewAttributes() ?>>
<?php echo $resolved_issue->sub_category->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($resolved_issue->incident_description->Visible) { // incident_description ?>
		<td data-name="incident_description"<?php echo $resolved_issue->incident_description->CellAttributes() ?>>
<span id="el<?php echo $resolved_issue_list->RowCnt ?>_resolved_issue_incident_description" class="resolved_issue_incident_description">
<span<?php echo $resolved_issue->incident_description->ViewAttributes() ?>>
<?php echo $resolved_issue->incident_description->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($resolved_issue->status->Visible) { // status ?>
		<td data-name="status"<?php echo $resolved_issue->status->CellAttributes() ?>>
<span id="el<?php echo $resolved_issue_list->RowCnt ?>_resolved_issue_status" class="resolved_issue_status">
<span<?php echo $resolved_issue->status->ViewAttributes() ?>>
<?php echo $resolved_issue->status->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($resolved_issue->last_updated_date->Visible) { // last_updated_date ?>
		<td data-name="last_updated_date"<?php echo $resolved_issue->last_updated_date->CellAttributes() ?>>
<span id="el<?php echo $resolved_issue_list->RowCnt ?>_resolved_issue_last_updated_date" class="resolved_issue_last_updated_date">
<span<?php echo $resolved_issue->last_updated_date->ViewAttributes() ?>>
<?php echo $resolved_issue->last_updated_date->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($resolved_issue->last_updated_by->Visible) { // last_updated_by ?>
		<td data-name="last_updated_by"<?php echo $resolved_issue->last_updated_by->CellAttributes() ?>>
<span id="el<?php echo $resolved_issue_list->RowCnt ?>_resolved_issue_last_updated_by" class="resolved_issue_last_updated_by">
<span<?php echo $resolved_issue->last_updated_by->ViewAttributes() ?>>
<?php echo $resolved_issue->last_updated_by->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
<?php
...
$resolved_issue_list->ListOptions->Render("body", "right", $resolved_issue_list->RowCnt);
?>
	</tr>
<?php
	}
	if ($resolved_issue->CurrentAction <> "gridadd")
		$resolved_issue_list->Recordset->MoveNext();
}
?>
</tbody>
</table>
<?php } ?>
<?php if ($resolved_issue->CurrentAction == "") { ?>
...
// Close recordset
if ($resolved_issue_list->Recordset)
	$resolved_issue_list->Recordset->Close();
?>
</div>
<?php } ?>
<?php if ($resolved_issue_list->TotalRecs == 0 && $resolved_issue->CurrentAction == "") { // Show other options ?>
<div class="ewListOtherOptions">
<?php
	foreach ($resolved_issue_list->OtherOptions as &$option) {
		$option->ButtonClass = "";
...
<?php if ($resolved_issue->Export == "") { ?>
<script type="text/javascript">
fresolved_issuelistsrch.FilterList = <?php echo $resolved_issue_list->GetFilterList() ?>;
fresolved_issuelistsrch.Init();
fresolved_issuelist.Init();
</script>
<?php } ?>
<?php
$resolved_issue_list->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($resolved_issue->Export == "") { ?>
...
<?php
$resolved_issue_list->Page_Terminate();
?>
