...
class csub_category_list extends csub_category {
	...
	function __construct() {
		...
		if (!defined("EW_TABLE_NAME"))
			define("EW_TABLE_NAME", 'sub-category', TRUE);
	...
	function Page_Init() {
		...
		$this->sub_category_id->SetVisibility();
		if ($this->IsAdd() || $this->IsCopy() || $this->IsGridAdd())
			$this->sub_category_id->Visible = FALSE;
		$this->sub_category_name->SetVisibility();
	...
	function SetupKeyValues($key) {
		...
		if (count($arrKeyFlds) >= 1) {
			$this->sub_category_id->setFormValue($arrKeyFlds[0]);
			if (!is_numeric($this->sub_category_id->FormValue))
				return FALSE;
		}
		return TRUE;
	}
	...
	function GetFilterList() {
		...
		if (EW_SEARCH_FILTER_OPTION == "Server" && isset($UserProfile))
			$sSavedFilterList = $UserProfile->GetSearchFilters(CurrentUserName(), "fsub_categorylistsrch");
		$sFilterList = ew_Concat($sFilterList, $this->sub_category_id->AdvancedSearch->ToJson(), ","); // Field sub-category_id
		$sFilterList = ew_Concat($sFilterList, $this->sub_category_name->AdvancedSearch->ToJson(), ","); // Field sub-category_name
	...
	function ProcessFilterList() {
		...
		if (@$_POST["ajax"] == "savefilters") { // Save filter request (Ajax)
			...
			$UserProfile->SetSearchFilters(CurrentUserName(), "fsub_categorylistsrch", $filters);
	...
	function RestoreFilterList() {
		...
		// Field sub-category_id
		$this->sub_category_id->AdvancedSearch->SearchValue = @$filter["x_sub_category_id"];
		$this->sub_category_id->AdvancedSearch->SearchOperator = @$filter["z_sub_category_id"];
		$this->sub_category_id->AdvancedSearch->SearchCondition = @$filter["v_sub_category_id"];
		$this->sub_category_id->AdvancedSearch->SearchValue2 = @$filter["y_sub_category_id"];
		$this->sub_category_id->AdvancedSearch->SearchOperator2 = @$filter["w_sub_category_id"];
		$this->sub_category_id->AdvancedSearch->Save();

		// Field sub-category_name
		$this->sub_category_name->AdvancedSearch->SearchValue = @$filter["x_sub_category_name"];
		$this->sub_category_name->AdvancedSearch->SearchOperator = @$filter["z_sub_category_name"];
		$this->sub_category_name->AdvancedSearch->SearchCondition = @$filter["v_sub_category_name"];
		$this->sub_category_name->AdvancedSearch->SearchValue2 = @$filter["y_sub_category_name"];
		$this->sub_category_name->AdvancedSearch->SearchOperator2 = @$filter["w_sub_category_name"];
		$this->sub_category_name->AdvancedSearch->Save();
	...
	function BasicSearchSQL($arKeywords, $type) {
		$sWhere = "";
		$this->BuildBasicSearchSQL($sWhere, $this->sub_category_name, $arKeywords, $type);
		return $sWhere;
	}
	...
	function SetupSortOrder() {
		...
		if (@$_GET["order"] <> "") {
			...
			$this->UpdateSort($this->sub_category_id); // sub-category_id
			$this->UpdateSort($this->sub_category_name); // sub-category_name
	...
	function ResetCmd() {
		...
		if (substr($this->Command,0,5) == "reset") {
			...
			if ($this->Command == "resetsort") {
				$sOrderBy = "";
				$this->setSessionOrderBy($sOrderBy);
				$this->sub_category_id->setSort("");
				$this->sub_category_name->setSort("");
	...
	function RenderListOptions() {
		...
		$oListOpt->Body = "<input type=\"checkbox\" name=\"key_m[]\" class=\"ewMultiSelect\" value=\"" . ew_HtmlEncode($this->sub_category_id->CurrentValue) . "\" onclick=\"ew_ClickMultiCheckbox(event);\">";
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $Language->Phrase("SearchPanel") . "\" data-caption=\"" . $Language->Phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fsub_categorylistsrch\">" . $Language->Phrase("SearchLink") . "</button>";
		$item->Visible = TRUE;
	...
	function LoadRowValues($rs = NULL) {
		...
		if (!$rs || $rs->EOF)
			return;
		$this->sub_category_id->setDbValue($row['sub-category_id']);
		$this->sub_category_name->setDbValue($row['sub-category_name']);
	...
	function NewRow() {
		$row = array();
		$row['sub-category_id'] = NULL;
		$row['sub-category_name'] = NULL;
	...
	function LoadDbValues(&$rs) {
		...
		$this->sub_category_id->DbValue = $row['sub-category_id'];
		$this->sub_category_name->DbValue = $row['sub-category_name'];
	...
	function LoadOldRecord() {
		...
		$bValidKey = TRUE;
		if (strval($this->getKey("sub_category_id")) <> "")
			$this->sub_category_id->CurrentValue = $this->getKey("sub_category_id"); // sub-category_id
		else
			$bValidKey = FALSE;
	...
	function RenderRow() {
		...
		// sub-category_id
		$this->sub_category_id->ViewValue = $this->sub_category_id->CurrentValue;
		$this->sub_category_id->ViewCustomAttributes = "";

		// sub-category_name
		$this->sub_category_name->ViewValue = $this->sub_category_name->CurrentValue;
		$this->sub_category_name->ViewCustomAttributes = "";
		...
		$this->category_id->ViewCustomAttributes = "";
			...
			// sub-category_id
			$this->sub_category_id->LinkCustomAttributes = "";
			$this->sub_category_id->HrefValue = "";
			$this->sub_category_id->TooltipValue = "";

			// sub-category_name
			$this->sub_category_name->LinkCustomAttributes = "";
			$this->sub_category_name->HrefValue = "";
			$this->sub_category_name->TooltipValue = "";
	...
	function SetupExportOptions() {
		...
		$url = "";
		$item->Body = "<button id=\"emf_sub_category\" class=\"ewExportLink ewEmail\" title=\"" . $Language->Phrase("ExportToEmailText") . "\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_sub_category',hdr:ewLanguage.Phrase('ExportToEmailText'),f:document.fsub_categorylist,sel:false" . $url . "});\">" . $Language->Phrase("ExportToEmail") . "</button>";
		$item->Visible = FALSE;
...
fsub_categorylist.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;
...
<?php
	...
	if ($bSelectLimit) {
		if ($sub_category_list->TotalRecs <= 0)
			$sub_category_list->TotalRecs = $sub_category->ListRecordCount();
	} else {
	...
	if ($sub_category->CurrentAction == "" && $sub_category_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$sub_category_list->setWarningMessage(ew_DeniedMsg());
...
<form name="fsub_categorylistsrch" id="fsub_categorylistsrch" class="form-inline ewForm ewExtSearchForm" action="<?php echo ew_CurrentPage() ?>">
...
<div class="box ewBox ewGrid<?php if ($sub_category_list->IsAddOrEdit()) { ?> ewGridAddEdit<?php } ?> sub_category">
<?php if ($sub_category->Export == "") { ?>
<div class="box-header ewGridUpperPanel">
<?php if ($sub_category->CurrentAction <> "gridadd" && $sub_category->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="form-inline ewForm ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($sub_category_list->Pager)) $sub_category_list->Pager = new cPrevNextPager($sub_category_list->StartRec, $sub_category_list->DisplayRecs, $sub_category_list->TotalRecs, $sub_category_list->AutoHidePager) ?>
<?php if ($sub_category_list->Pager->RecordCount > 0 && $sub_category_list->Pager->Visible) { ?>
<div class="ewPager">
...
<?php } ?>
<?php if ($sub_category_list->TotalRecs > 0 && (!$sub_category_list->AutoHidePageSizeSelector || $sub_category_list->Pager->Visible)) { ?>
<div class="ewPager">
<input type="hidden" name="t" value="sub_category">
<select name="<?php echo EW_TABLE_REC_PER_PAGE ?>" class="form-control input-sm ewTooltip" title="<?php echo $Language->Phrase("RecordsPerPage") ?>" onchange="this.form.submit();">
<option value="5"<?php if ($sub_category_list->DisplayRecs == 5) { ?> selected<?php } ?>>5</option>
<option value="10"<?php if ($sub_category_list->DisplayRecs == 10) { ?> selected<?php } ?>>10</option>
<option value="15"<?php if ($sub_category_list->DisplayRecs == 15) { ?> selected<?php } ?>>15</option>
<option value="20"<?php if ($sub_category_list->DisplayRecs == 20) { ?> selected<?php } ?>>20</option>
<option value="50"<?php if ($sub_category_list->DisplayRecs == 50) { ?> selected<?php } ?>>50</option>
<option value="ALL"<?php if ($sub_category->getRecordsPerPage() == -1) { ?> selected<?php } ?>><?php echo $Language->Phrase("AllRecords") ?></option>
</select>
</div>
<?php } ?>
</form>
...
<?php } ?>
<input type="hidden" name="t" value="sub_category">
<div id="gmp_sub_category" class="<?php if (ew_IsResponsiveLayout()) { ?>table-responsive <?php } ?>ewGridMiddlePanel">
<?php if ($sub_category_list->TotalRecs > 0 || $sub_category->CurrentAction == "gridedit") { ?>
...
?>
<?php if ($sub_category->sub_category_id->Visible) { // sub-category_id ?>
	<?php if ($sub_category->SortUrl($sub_category->sub_category_id) == "") { ?>
		<th data-name="sub_category_id" class="<?php echo $sub_category->sub_category_id->HeaderCellClass() ?>"><div id="elh_sub_category_sub_category_id" class="sub_category_sub_category_id"><div class="ewTableHeaderCaption"><?php echo $sub_category->sub_category_id->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="sub_category_id" class="<?php echo $sub_category->sub_category_id->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $sub_category->SortUrl($sub_category->sub_category_id) ?>',1);"><div id="elh_sub_category_sub_category_id" class="sub_category_sub_category_id">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $sub_category->sub_category_id->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($sub_category->sub_category_id->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($sub_category->sub_category_id->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($sub_category->sub_category_name->Visible) { // sub-category_name ?>
	<?php if ($sub_category->SortUrl($sub_category->sub_category_name) == "") { ?>
		<th data-name="sub_category_name" class="<?php echo $sub_category->sub_category_name->HeaderCellClass() ?>"><div id="elh_sub_category_sub_category_name" class="sub_category_sub_category_name"><div class="ewTableHeaderCaption"><?php echo $sub_category->sub_category_name->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="sub_category_name" class="<?php echo $sub_category->sub_category_name->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $sub_category->SortUrl($sub_category->sub_category_name) ?>',1);"><div id="elh_sub_category_sub_category_name" class="sub_category_sub_category_name">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $sub_category->sub_category_name->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($sub_category->sub_category_name->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($sub_category->sub_category_name->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($sub_category->category_id->Visible) { // category_id ?>
	<?php if ($sub_category->SortUrl($sub_category->category_id) == "") { ?>
		<th data-name="category_id" class="<?php echo $sub_category->category_id->HeaderCellClass() ?>"><div id="elh_sub_category_category_id" class="sub_category_category_id"><div class="ewTableHeaderCaption"><?php echo $sub_category->category_id->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="category_id" class="<?php echo $sub_category->category_id->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $sub_category->SortUrl($sub_category->category_id) ?>',1);"><div id="elh_sub_category_category_id" class="sub_category_category_id">
...
?>
	<?php if ($sub_category->sub_category_id->Visible) { // sub-category_id ?>
		<td data-name="sub_category_id"<?php echo $sub_category->sub_category_id->CellAttributes() ?>>
<span id="el<?php echo $sub_category_list->RowCnt ?>_sub_category_sub_category_id" class="sub_category_sub_category_id">
<span<?php echo $sub_category->sub_category_id->ViewAttributes() ?>>
<?php echo $sub_category->sub_category_id->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($sub_category->sub_category_name->Visible) { // sub-category_name ?>
		<td data-name="sub_category_name"<?php echo $sub_category->sub_category_name->CellAttributes() ?>>
<span id="el<?php echo $sub_category_list->RowCnt ?>_sub_category_sub_category_name" class="sub_category_sub_category_name">
<span<?php echo $sub_category->sub_category_name->ViewAttributes() ?>>
<?php echo $sub_category->sub_category_name->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($sub_category->category_id->Visible) { // category_id ?>
