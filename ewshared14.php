...
if (IsLoggedIn()) {
		$MyReportCount = 0;
			...
			if (CurrentUserLevel() == 2) {
				$_SESSION['MyReportCount'] = ew_ExecuteScalar("SELECT COUNT(id) FROM `report_form` WHERE (`status` in (3) AND `departments` = '".$_SESSION['Department']."')");
			}
			/*if (CurrentUserLevel() == 2) {
...
function Menu_Rendering(&$Menu) {
	...
	if ($Menu->IsRoot) { // Root menu
		...
		if(date('D') == 'Sat' || date('D') == 'Sun') { 
			...
			//$Menu->Clear();
			//$Menu->AddMenuItem(1, "MyName1", "Testing", "testing.php");
			//$Menu->AddMenuItem(2, "MyName2", "News", "newsview.php");
