...
function CurrentProjectID() {
	...
	return "{DD9080C0-D1CA-431F-831F-CAC8FA61260C}";
}
			...
			if ($rs = $UserTableConn->Execute($sSql)) {
				if (!$rs->EOF) {
					...
					if ($valid) {
						...
						if (is_null($rs->fields('accesslevel'))) {
							$this->setSessionUserLevelID(0);
						} else {
							$this->setSessionUserLevelID(intval($rs->fields('accesslevel'))); // Load User Level
						}
						$this->SetupUserLevel();
