...
$report_form_list = NULL; // Initialize page object first

class creport_form_list extends creport_form {
	...
	// Page object name
	var $PageObjName = 'report_form_list';

	// Grid form hidden field names
	var $FormName = 'freport_formlist';
	...
	function __construct() {
		...
		$this->AddUrl = "report_formadd.php";
		$this->InlineAddUrl = $this->PageUrl() . "a=add";
		$this->GridAddUrl = $this->PageUrl() . "a=gridadd";
		$this->GridEditUrl = $this->PageUrl() . "a=gridedit";
		$this->MultiDeleteUrl = "report_formdelete.php";
		$this->MultiUpdateUrl = "report_formupdate.php";
		...
		$this->FilterOptions->TagClassName = "ewFilterOption freport_formlistsrch";
	...
	function GetFilterList() {
		...
		if (EW_SEARCH_FILTER_OPTION == "Server" && isset($UserProfile))
			$sSavedFilterList = $UserProfile->GetSearchFilters(CurrentUserName(), "freport_formlistsrch");
	...
	function ProcessFilterList() {
		...
		if (@$_POST["ajax"] == "savefilters") { // Save filter request (Ajax)
			...
			$UserProfile->SetSearchFilters(CurrentUserName(), "freport_formlistsrch", $filters);
	...
	function RenderListOptions() {
		...
		if ($Security->CanView()) {
			if (ew_IsMobile())
				...
				$oListOpt->Body = "<a class=\"ewRowLink ewView\" title=\"" . $viewcaption . "\" data-table=\"report_form\" data-caption=\"" . $viewcaption . "\" href=\"javascript:void(0);\" onclick=\"ew_ModalDialogShow({lnk:this,url:'" . ew_HtmlEncode($this->ViewUrl) . "',btn:null});\">" . $Language->Phrase("ViewLink") . "</a>";
		} else {
			$oListOpt->Body = "";
		}
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"freport_formlistsrch\" href=\"#\">" . $Language->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"freport_formlistsrch\" href=\"#\">" . $Language->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->ListActions->Items as $listaction) {
				if ($listaction->Select == EW_ACTION_MULTIPLE) {
					...
					$item->Body = "<a class=\"ewAction ewListAction\" title=\"" . ew_HtmlEncode($caption) . "\" data-caption=\"" . ew_HtmlEncode($caption) . "\" href=\"\" onclick=\"ew_SubmitAction(event,jQuery.extend({f:document.freport_formlist}," . $listaction->ToJson(TRUE) . "));return false;\">" . $icon . "</a>";
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $Language->Phrase("SearchPanel") . "\" data-caption=\"" . $Language->Phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"freport_formlistsrch\">" . $Language->Phrase("SearchLink") . "</button>";
		$item->Visible = TRUE;
	...
	function Page_Load() {
		...
		if (CurrentPageID() == "list"){
			 $_SESSION['Inc_ID'] = generateINCKey();

			 ///$_SESSION['StffNO'] = generateSIDKey();
		 }
	}
...
// Create page object
if (!isset($report_form_list)) $report_form_list = new creport_form_list();

// Page init
$report_form_list->Page_Init();

// Page main
$report_form_list->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$report_form_list->Page_Render();
?>
...
var CurrentForm = freport_formlist = new ew_Form("freport_formlist", "list");
freport_formlist.FormKeyCountName = '<?php echo $report_form_list->FormKeyCountName ?>';

// Form_CustomValidate event
freport_formlist.Form_CustomValidate = 
...
freport_formlist.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
freport_formlist.Lists["x_staffid"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_staffno","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"users"};
freport_formlist.Lists["x_staffid"].Data = "<?php echo $report_form_list->staffid->LookupFilterQuery(FALSE, "list") ?>";
freport_formlist.AutoSuggests["x_staffid"] = <?php echo json_encode(array("data" => "ajax=autosuggest&" . $report_form_list->staffid->LookupFilterQuery(TRUE, "list"))) ?>;
freport_formlist.Lists["x_staff_id"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_firstname","x_lastname","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"users"};
freport_formlist.Lists["x_staff_id"].Data = "<?php echo $report_form_list->staff_id->LookupFilterQuery(FALSE, "list") ?>";
freport_formlist.Lists["x_branch"] = {"LinkField":"x_branch_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_branch_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"branch"};
freport_formlist.Lists["x_branch"].Data = "<?php echo $report_form_list->branch->LookupFilterQuery(FALSE, "list") ?>";
freport_formlist.Lists["x_departments"] = {"LinkField":"x_code_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_description","","",""],"ParentFields":[],"ChildFields":["x_category"],"FilterFields":[],"Options":[],"Template":"","LinkTable":"departments"};
freport_formlist.Lists["x_departments"].Data = "<?php echo $report_form_list->departments->LookupFilterQuery(FALSE, "list") ?>";
freport_formlist.Lists["x_category"] = {"LinkField":"x_category_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_description","","",""],"ParentFields":[],"ChildFields":["x_sub_category"],"FilterFields":[],"Options":[],"Template":"","LinkTable":"category"};
freport_formlist.Lists["x_category"].Data = "<?php echo $report_form_list->category->LookupFilterQuery(FALSE, "list") ?>";
freport_formlist.Lists["x_sub_category"] = {"LinkField":"x_sub_category_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_sub_category_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"sub_category"};
freport_formlist.Lists["x_sub_category"].Data = "<?php echo $report_form_list->sub_category->LookupFilterQuery(FALSE, "list") ?>";
...
freport_formlist.Lists["x_operational_status"] = {"LinkField":"x_code","Ajax":true,"AutoFill":false,"DisplayFields":["x_description","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"operational_status"};
freport_formlist.Lists["x_operational_status"].Data = "<?php echo $report_form_list->operational_status->LookupFilterQuery(FALSE, "list") ?>";
freport_formlist.Lists["x_last_updated_by"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_firstname","x_lastname","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"users"};
freport_formlist.Lists["x_last_updated_by"].Data = "<?php echo $report_form_list->last_updated_by->LookupFilterQuery(FALSE, "list") ?>";
freport_formlist.AutoSuggests["x_last_updated_by"] = <?php echo json_encode(array("data" => "ajax=autosuggest&" . $report_form_list->last_updated_by->LookupFilterQuery(TRUE, "list"))) ?>;

// Form object for search
var CurrentSearchForm = freport_formlistsrch = new ew_Form("freport_formlistsrch");

// Validate function for search
freport_formlistsrch.Validate = function(fobj) {
...
// Form_CustomValidate event
freport_formlistsrch.Form_CustomValidate = 
 function(fobj) { // DO NOT CHANGE THIS LINE!
...
// Use JavaScript validation or not
freport_formlistsrch.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
freport_formlistsrch.Lists["x_staff_id"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_firstname","x_lastname","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"users"};
freport_formlistsrch.Lists["x_staff_id"].Data = "<?php echo $report_form_list->staff_id->LookupFilterQuery(FALSE, "extbs") ?>";
freport_formlistsrch.Lists["x_status"] = {"LinkField":"x_code","Ajax":true,"AutoFill":false,"DisplayFields":["x_description","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"status"};
freport_formlistsrch.Lists["x_status"].Data = "<?php echo $report_form_list->status->LookupFilterQuery(FALSE, "extbs") ?>";
</script>
<script type="text/javascript">
...
<?php if ($report_form->Export == "") { ?>
<div class="ewToolbar">
<?php if ($report_form_list->TotalRecs > 0 && $report_form_list->ExportOptions->Visible()) { ?>
<?php $report_form_list->ExportOptions->Render("body") ?>
<?php } ?>
<?php if ($report_form_list->SearchOptions->Visible()) { ?>
<?php $report_form_list->SearchOptions->Render("body") ?>
<?php } ?>
<?php if ($report_form_list->FilterOptions->Visible()) { ?>
<?php $report_form_list->FilterOptions->Render("body") ?>
<?php } ?>
<div class="clearfix"></div>
</div>
<?php } ?>
<?php
	$bSelectLimit = $report_form_list->UseSelectLimit;
	if ($bSelectLimit) {
		if ($report_form_list->TotalRecs <= 0)
			$report_form_list->TotalRecs = $report_form->ListRecordCount();
	} else {
		if (!$report_form_list->Recordset && ($report_form_list->Recordset = $report_form_list->LoadRecordset()))
			$report_form_list->TotalRecs = $report_form_list->Recordset->RecordCount();
	}
	$report_form_list->StartRec = 1;
	if ($report_form_list->DisplayRecs <= 0 || ($report_form->Export <> "" && $report_form->ExportAll)) // Display all records
		$report_form_list->DisplayRecs = $report_form_list->TotalRecs;
	if (!($report_form->Export <> "" && $report_form->ExportAll))
		$report_form_list->SetupStartRec(); // Set up start record position
	if ($bSelectLimit)
		$report_form_list->Recordset = $report_form_list->LoadRecordset($report_form_list->StartRec-1, $report_form_list->DisplayRecs);

	// Set no record found message
	if ($report_form->CurrentAction == "" && $report_form_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$report_form_list->setWarningMessage(ew_DeniedMsg());
		if ($report_form_list->SearchWhere == "0=101")
			$report_form_list->setWarningMessage($Language->Phrase("EnterSearchCriteria"));
		else
			$report_form_list->setWarningMessage($Language->Phrase("NoRecord"));
	}

	// Audit trail on search
	if ($report_form_list->AuditTrailOnSearch && $report_form_list->Command == "search" && !$report_form_list->RestoreSearch) {
		$searchparm = ew_ServerVar("QUERY_STRING");
		$searchsql = $report_form_list->getSessionWhere();
		$report_form_list->WriteAuditTrailOnSearch($searchparm, $searchsql);
	}
$report_form_list->RenderOtherOptions();
?>
<?php if ($Security->CanSearch()) { ?>
<?php if ($report_form->Export == "" && $report_form->CurrentAction == "") { ?>
<form name="freport_formlistsrch" id="freport_formlistsrch" class="form-inline ewForm ewExtSearchForm" action="<?php echo ew_CurrentPage() ?>">
<?php $SearchPanelClass = ($report_form_list->SearchWhere <> "") ? " in" : " in"; ?>
<div id="freport_formlistsrch_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
<input type="hidden" name="cmd" value="search">
<input type="hidden" name="t" value="report_form">
	<div class="ewBasicSearch">
<?php
if ($gsSearchError == "")
	$report_form_list->LoadAdvancedSearch(); // Load advanced search

// Render for search
$report_form->RowType = EW_ROWTYPE_SEARCH;

// Render row
$report_form->ResetAttrs();
$report_form_list->RenderRow();
?>
<div id="xsr_1" class="ewRow">
<?php if ($report_form->incident_id->Visible) { // incident_id ?>
	<div id="xsc_incident_id" class="ewCell form-group">
		<label for="x_incident_id" class="ewSearchCaption ewLabel"><?php echo $report_form->incident_id->FldCaption() ?></label>
...
<?php if ($report_form->staff_id->Visible) { // staff_id ?>
	<div id="xsc_staff_id" class="ewCell form-group">
		<label for="x_staff_id" class="ewSearchCaption ewLabel"><?php echo $report_form->staff_id->FldCaption() ?></label>
		<span class="ewSearchOperator"><?php echo $Language->Phrase("=") ?><input type="hidden" name="z_staff_id" id="z_staff_id" value="="></span>
		<span class="ewSearchField">
<select data-table="report_form" data-field="x_staff_id" data-value-separator="<?php echo $report_form->staff_id->DisplayValueSeparatorAttribute() ?>" id="x_staff_id" name="x_staff_id"<?php echo $report_form->staff_id->EditAttributes() ?>>
...
<?php if ($report_form->status->Visible) { // status ?>
	<div id="xsc_status" class="ewCell form-group">
		<label for="x_status" class="ewSearchCaption ewLabel"><?php echo $report_form->status->FldCaption() ?></label>
...
<div id="xsr_4" class="ewRow">
	...
	<input type="text" name="<?php echo EW_TABLE_BASIC_SEARCH ?>" id="<?php echo EW_TABLE_BASIC_SEARCH ?>" class="form-control" value="<?php echo ew_HtmlEncode($report_form_list->BasicSearch->getKeyword()) ?>" placeholder="<?php echo ew_HtmlEncode($Language->Phrase("Search")) ?>">
	<input type="hidden" name="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" id="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" value="<?php echo ew_HtmlEncode($report_form_list->BasicSearch->getType()) ?>">
	<div class="input-group-btn">
		<button type="button" data-toggle="dropdown" class="btn btn-default"><span id="searchtype"><?php echo $report_form_list->BasicSearch->getTypeNameShort() ?></span><span class="caret"></span></button>
		<ul class="dropdown-menu pull-right" role="menu">
			<li<?php if ($report_form_list->BasicSearch->getType() == "") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this)"><?php echo $Language->Phrase("QuickSearchAuto") ?></a></li>
			<li<?php if ($report_form_list->BasicSearch->getType() == "=") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'=')"><?php echo $Language->Phrase("QuickSearchExact") ?></a></li>
			<li<?php if ($report_form_list->BasicSearch->getType() == "AND") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'AND')"><?php echo $Language->Phrase("QuickSearchAll") ?></a></li>
			<li<?php if ($report_form_list->BasicSearch->getType() == "OR") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'OR')"><?php echo $Language->Phrase("QuickSearchAny") ?></a></li>
		</ul>
...
<?php } ?>
<?php } ?>
<?php $report_form_list->ShowPageHeader(); ?>
<?php
$report_form_list->ShowMessage();
?>
<?php if ($report_form_list->TotalRecs > 0 || $report_form->CurrentAction <> "") { ?>
<div class="box ewBox ewGrid<?php if ($report_form_list->IsAddOrEdit()) { ?> ewGridAddEdit<?php } ?> report_form">
<?php if ($report_form->Export == "") { ?>
<div class="box-header ewGridUpperPanel">
<?php if ($report_form->CurrentAction <> "gridadd" && $report_form->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="form-inline ewForm ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($report_form_list->Pager)) $report_form_list->Pager = new cPrevNextPager($report_form_list->StartRec, $report_form_list->DisplayRecs, $report_form_list->TotalRecs, $report_form_list->AutoHidePager) ?>
<?php if ($report_form_list->Pager->RecordCount > 0 && $report_form_list->Pager->Visible) { ?>
<div class="ewPager">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($report_form_list->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $report_form_list->PageUrl() ?>start=<?php echo $report_form_list->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($report_form_list->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $report_form_list->PageUrl() ?>start=<?php echo $report_form_list->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $report_form_list->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($report_form_list->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $report_form_list->PageUrl() ?>start=<?php echo $report_form_list->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($report_form_list->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $report_form_list->PageUrl() ?>start=<?php echo $report_form_list->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $report_form_list->Pager->PageCount ?></span>
</div>
<?php } ?>
<?php if ($report_form_list->Pager->RecordCount > 0) { ?>
<div class="ewPager ewRec">
	<span><?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $report_form_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $report_form_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $report_form_list->Pager->RecordCount ?></span>
</div>
<?php } ?>
<?php if ($report_form_list->TotalRecs > 0 && (!$report_form_list->AutoHidePageSizeSelector || $report_form_list->Pager->Visible)) { ?>
<div class="ewPager">
<input type="hidden" name="t" value="report_form">
<select name="<?php echo EW_TABLE_REC_PER_PAGE ?>" class="form-control input-sm ewTooltip" title="<?php echo $Language->Phrase("RecordsPerPage") ?>" onchange="this.form.submit();">
<option value="5"<?php if ($report_form_list->DisplayRecs == 5) { ?> selected<?php } ?>>5</option>
<option value="10"<?php if ($report_form_list->DisplayRecs == 10) { ?> selected<?php } ?>>10</option>
<option value="15"<?php if ($report_form_list->DisplayRecs == 15) { ?> selected<?php } ?>>15</option>
<option value="20"<?php if ($report_form_list->DisplayRecs == 20) { ?> selected<?php } ?>>20</option>
<option value="50"<?php if ($report_form_list->DisplayRecs == 50) { ?> selected<?php } ?>>50</option>
<option value="ALL"<?php if ($report_form->getRecordsPerPage() == -1) { ?> selected<?php } ?>><?php echo $Language->Phrase("AllRecords") ?></option>
</select>
</div>
<?php } ?>
</form>
...
<?php
	foreach ($report_form_list->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<div class="clearfix"></div>
...
<?php } ?>
<form name="freport_formlist" id="freport_formlist" class="form-inline ewForm ewListForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($report_form_list->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $report_form_list->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="report_form">
<div id="gmp_report_form" class="<?php if (ew_IsResponsiveLayout()) { ?>table-responsive <?php } ?>ewGridMiddlePanel">
<?php if ($report_form_list->TotalRecs > 0 || $report_form->CurrentAction == "gridedit") { ?>
<table id="tbl_report_formlist" class="table ewTable">
<thead>
	<tr class="ewTableHeader">
<?php

// Header row
$report_form_list->RowType = EW_ROWTYPE_HEADER;

// Render list options
$report_form_list->RenderListOptions();

// Render list options (header, left)
$report_form_list->ListOptions->Render("header", "left");
?>
<?php if ($report_form->datetime_initiated->Visible) { // datetime_initiated ?>
	<?php if ($report_form->SortUrl($report_form->datetime_initiated) == "") { ?>
		<th data-name="datetime_initiated" class="<?php echo $report_form->datetime_initiated->HeaderCellClass() ?>"><div id="elh_report_form_datetime_initiated" class="report_form_datetime_initiated"><div class="ewTableHeaderCaption"><?php echo $report_form->datetime_initiated->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="datetime_initiated" class="<?php echo $report_form->datetime_initiated->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $report_form->SortUrl($report_form->datetime_initiated) ?>',1);"><div id="elh_report_form_datetime_initiated" class="report_form_datetime_initiated">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $report_form->datetime_initiated->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($report_form->datetime_initiated->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($report_form->datetime_initiated->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($report_form->incident_id->Visible) { // incident_id ?>
	<?php if ($report_form->SortUrl($report_form->incident_id) == "") { ?>
		<th data-name="incident_id" class="<?php echo $report_form->incident_id->HeaderCellClass() ?>"><div id="elh_report_form_incident_id" class="report_form_incident_id"><div class="ewTableHeaderCaption"><?php echo $report_form->incident_id->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="incident_id" class="<?php echo $report_form->incident_id->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $report_form->SortUrl($report_form->incident_id) ?>',1);"><div id="elh_report_form_incident_id" class="report_form_incident_id">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $report_form->incident_id->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($report_form->incident_id->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($report_form->incident_id->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($report_form->staffid->Visible) { // staffid ?>
	<?php if ($report_form->SortUrl($report_form->staffid) == "") { ?>
		<th data-name="staffid" class="<?php echo $report_form->staffid->HeaderCellClass() ?>"><div id="elh_report_form_staffid" class="report_form_staffid"><div class="ewTableHeaderCaption"><?php echo $report_form->staffid->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="staffid" class="<?php echo $report_form->staffid->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $report_form->SortUrl($report_form->staffid) ?>',1);"><div id="elh_report_form_staffid" class="report_form_staffid">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $report_form->staffid->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($report_form->staffid->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($report_form->staffid->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($report_form->staff_id->Visible) { // staff_id ?>
	<?php if ($report_form->SortUrl($report_form->staff_id) == "") { ?>
		<th data-name="staff_id" class="<?php echo $report_form->staff_id->HeaderCellClass() ?>"><div id="elh_report_form_staff_id" class="report_form_staff_id"><div class="ewTableHeaderCaption"><?php echo $report_form->staff_id->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="staff_id" class="<?php echo $report_form->staff_id->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $report_form->SortUrl($report_form->staff_id) ?>',1);"><div id="elh_report_form_staff_id" class="report_form_staff_id">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $report_form->staff_id->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($report_form->staff_id->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($report_form->staff_id->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($report_form->branch->Visible) { // branch ?>
	<?php if ($report_form->SortUrl($report_form->branch) == "") { ?>
		<th data-name="branch" class="<?php echo $report_form->branch->HeaderCellClass() ?>"><div id="elh_report_form_branch" class="report_form_branch"><div class="ewTableHeaderCaption"><?php echo $report_form->branch->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="branch" class="<?php echo $report_form->branch->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $report_form->SortUrl($report_form->branch) ?>',1);"><div id="elh_report_form_branch" class="report_form_branch">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $report_form->branch->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($report_form->branch->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($report_form->branch->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($report_form->departments->Visible) { // departments ?>
	<?php if ($report_form->SortUrl($report_form->departments) == "") { ?>
		<th data-name="departments" class="<?php echo $report_form->departments->HeaderCellClass() ?>"><div id="elh_report_form_departments" class="report_form_departments"><div class="ewTableHeaderCaption"><?php echo $report_form->departments->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="departments" class="<?php echo $report_form->departments->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $report_form->SortUrl($report_form->departments) ?>',1);"><div id="elh_report_form_departments" class="report_form_departments">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $report_form->departments->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($report_form->departments->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($report_form->departments->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($report_form->category->Visible) { // category ?>
	<?php if ($report_form->SortUrl($report_form->category) == "") { ?>
		<th data-name="category" class="<?php echo $report_form->category->HeaderCellClass() ?>"><div id="elh_report_form_category" class="report_form_category"><div class="ewTableHeaderCaption"><?php echo $report_form->category->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="category" class="<?php echo $report_form->category->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $report_form->SortUrl($report_form->category) ?>',1);"><div id="elh_report_form_category" class="report_form_category">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $report_form->category->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($report_form->category->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($report_form->category->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($report_form->sub_category->Visible) { // sub_category ?>
	<?php if ($report_form->SortUrl($report_form->sub_category) == "") { ?>
		<th data-name="sub_category" class="<?php echo $report_form->sub_category->HeaderCellClass() ?>"><div id="elh_report_form_sub_category" class="report_form_sub_category"><div class="ewTableHeaderCaption"><?php echo $report_form->sub_category->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="sub_category" class="<?php echo $report_form->sub_category->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $report_form->SortUrl($report_form->sub_category) ?>',1);"><div id="elh_report_form_sub_category" class="report_form_sub_category">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $report_form->sub_category->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($report_form->sub_category->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($report_form->sub_category->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($report_form->incident_description->Visible) { // incident_description ?>
	<?php if ($report_form->SortUrl($report_form->incident_description) == "") { ?>
		<th data-name="incident_description" class="<?php echo $report_form->incident_description->HeaderCellClass() ?>"><div id="elh_report_form_incident_description" class="report_form_incident_description"><div class="ewTableHeaderCaption"><?php echo $report_form->incident_description->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="incident_description" class="<?php echo $report_form->incident_description->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $report_form->SortUrl($report_form->incident_description) ?>',1);"><div id="elh_report_form_incident_description" class="report_form_incident_description">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $report_form->incident_description->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($report_form->incident_description->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($report_form->incident_description->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($report_form->status->Visible) { // status ?>
	<?php if ($report_form->SortUrl($report_form->status) == "") { ?>
		<th data-name="status" class="<?php echo $report_form->status->HeaderCellClass() ?>"><div id="elh_report_form_status" class="report_form_status"><div class="ewTableHeaderCaption"><?php echo $report_form->status->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="status" class="<?php echo $report_form->status->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $report_form->SortUrl($report_form->status) ?>',1);"><div id="elh_report_form_status" class="report_form_status">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $report_form->status->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($report_form->status->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($report_form->status->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($report_form->operational_status->Visible) { // operational_status ?>
	<?php if ($report_form->SortUrl($report_form->operational_status) == "") { ?>
		<th data-name="operational_status" class="<?php echo $report_form->operational_status->HeaderCellClass() ?>"><div id="elh_report_form_operational_status" class="report_form_operational_status"><div class="ewTableHeaderCaption"><?php echo $report_form->operational_status->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="operational_status" class="<?php echo $report_form->operational_status->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $report_form->SortUrl($report_form->operational_status) ?>',1);"><div id="elh_report_form_operational_status" class="report_form_operational_status">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $report_form->operational_status->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($report_form->operational_status->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($report_form->operational_status->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($report_form->last_updated_date->Visible) { // last_updated_date ?>
	<?php if ($report_form->SortUrl($report_form->last_updated_date) == "") { ?>
		<th data-name="last_updated_date" class="<?php echo $report_form->last_updated_date->HeaderCellClass() ?>"><div id="elh_report_form_last_updated_date" class="report_form_last_updated_date"><div class="ewTableHeaderCaption"><?php echo $report_form->last_updated_date->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="last_updated_date" class="<?php echo $report_form->last_updated_date->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $report_form->SortUrl($report_form->last_updated_date) ?>',1);"><div id="elh_report_form_last_updated_date" class="report_form_last_updated_date">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $report_form->last_updated_date->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($report_form->last_updated_date->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($report_form->last_updated_date->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($report_form->last_updated_by->Visible) { // last_updated_by ?>
	<?php if ($report_form->SortUrl($report_form->last_updated_by) == "") { ?>
		<th data-name="last_updated_by" class="<?php echo $report_form->last_updated_by->HeaderCellClass() ?>"><div id="elh_report_form_last_updated_by" class="report_form_last_updated_by"><div class="ewTableHeaderCaption"><?php echo $report_form->last_updated_by->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="last_updated_by" class="<?php echo $report_form->last_updated_by->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $report_form->SortUrl($report_form->last_updated_by) ?>',1);"><div id="elh_report_form_last_updated_by" class="report_form_last_updated_by">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $report_form->last_updated_by->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($report_form->last_updated_by->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($report_form->last_updated_by->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php
...
$report_form_list->ListOptions->Render("header", "right");
?>
	</tr>
</thead>
<tbody>
<?php
if ($report_form->ExportAll && $report_form->Export <> "") {
	$report_form_list->StopRec = $report_form_list->TotalRecs;
} else {

	// Set the last record to display
	if ($report_form_list->TotalRecs > $report_form_list->StartRec + $report_form_list->DisplayRecs - 1)
		$report_form_list->StopRec = $report_form_list->StartRec + $report_form_list->DisplayRecs - 1;
	else
		$report_form_list->StopRec = $report_form_list->TotalRecs;
}
$report_form_list->RecCnt = $report_form_list->StartRec - 1;
if ($report_form_list->Recordset && !$report_form_list->Recordset->EOF) {
	$report_form_list->Recordset->MoveFirst();
	$bSelectLimit = $report_form_list->UseSelectLimit;
	if (!$bSelectLimit && $report_form_list->StartRec > 1)
		$report_form_list->Recordset->Move($report_form_list->StartRec - 1);
} elseif (!$report_form->AllowAddDeleteRow && $report_form_list->StopRec == 0) {
	$report_form_list->StopRec = $report_form->GridAddRowCount;
}

// Initialize aggregate
$report_form->RowType = EW_ROWTYPE_AGGREGATEINIT;
$report_form->ResetAttrs();
$report_form_list->RenderRow();
while ($report_form_list->RecCnt < $report_form_list->StopRec) {
	$report_form_list->RecCnt++;
	if (intval($report_form_list->RecCnt) >= intval($report_form_list->StartRec)) {
		$report_form_list->RowCnt++;

		// Set up key count
		$report_form_list->KeyCount = $report_form_list->RowIndex;

		// Init row class and style
		$report_form->ResetAttrs();
		$report_form->CssClass = "";
		if ($report_form->CurrentAction == "gridadd") {
		} else {
			$report_form_list->LoadRowValues($report_form_list->Recordset); // Load row values
		}
		$report_form->RowType = EW_ROWTYPE_VIEW; // Render view

		// Set up row id / data-rowindex
		$report_form->RowAttrs = array_merge($report_form->RowAttrs, array('data-rowindex'=>$report_form_list->RowCnt, 'id'=>'r' . $report_form_list->RowCnt . '_report_form', 'data-rowtype'=>$report_form->RowType));
...
$report_form_list->ListOptions->Render("body", "left", $report_form_list->RowCnt);
?>
	<?php if ($report_form->datetime_initiated->Visible) { // datetime_initiated ?>
		<td data-name="datetime_initiated"<?php echo $report_form->datetime_initiated->CellAttributes() ?>>
<span id="el<?php echo $report_form_list->RowCnt ?>_report_form_datetime_initiated" class="report_form_datetime_initiated">
...
<span id="el<?php echo $report_form_list->RowCnt ?>_report_form_incident_id" class="report_form_incident_id">
...
<span id="el<?php echo $report_form_list->RowCnt ?>_report_form_staffid" class="report_form_staffid">
...
<span id="el<?php echo $report_form_list->RowCnt ?>_report_form_staff_id" class="report_form_staff_id">
...
<span id="el<?php echo $report_form_list->RowCnt ?>_report_form_branch" class="report_form_branch">
...
<span id="el<?php echo $report_form_list->RowCnt ?>_report_form_departments" class="report_form_departments">
<span<?php echo $report_form->departments->ViewAttributes() ?>>
<?php echo $report_form->departments->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($report_form->category->Visible) { // category ?>
		<td data-name="category"<?php echo $report_form->category->CellAttributes() ?>>
<span id="el<?php echo $report_form_list->RowCnt ?>_report_form_category" class="report_form_category">
...
<span id="el<?php echo $report_form_list->RowCnt ?>_report_form_sub_category" class="report_form_sub_category">
...
<span id="el<?php echo $report_form_list->RowCnt ?>_report_form_incident_description" class="report_form_incident_description">
...
<span id="el<?php echo $report_form_list->RowCnt ?>_report_form_status" class="report_form_status">
...
<?php echo $report_form->status->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($report_form->operational_status->Visible) { // operational_status ?>
		<td data-name="operational_status"<?php echo $report_form->operational_status->CellAttributes() ?>>
<span id="el<?php echo $report_form_list->RowCnt ?>_report_form_operational_status" class="report_form_operational_status">
<span<?php echo $report_form->operational_status->ViewAttributes() ?>>
<?php echo $report_form->operational_status->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($report_form->last_updated_date->Visible) { // last_updated_date ?>
		<td data-name="last_updated_date"<?php echo $report_form->last_updated_date->CellAttributes() ?>>
<span id="el<?php echo $report_form_list->RowCnt ?>_report_form_last_updated_date" class="report_form_last_updated_date">
...
<span id="el<?php echo $report_form_list->RowCnt ?>_report_form_last_updated_by" class="report_form_last_updated_by">
...
$report_form_list->ListOptions->Render("body", "right", $report_form_list->RowCnt);
?>
	</tr>
<?php
	}
	if ($report_form->CurrentAction <> "gridadd")
		$report_form_list->Recordset->MoveNext();
}
?>
</tbody>
</table>
<?php } ?>
<?php if ($report_form->CurrentAction == "") { ?>
...
// Close recordset
if ($report_form_list->Recordset)
	$report_form_list->Recordset->Close();
?>
</div>
<?php } ?>
<?php if ($report_form_list->TotalRecs == 0 && $report_form->CurrentAction == "") { // Show other options ?>
<div class="ewListOtherOptions">
<?php
	foreach ($report_form_list->OtherOptions as &$option) {
		$option->ButtonClass = "";
...
<?php if ($report_form->Export == "") { ?>
<script type="text/javascript">
freport_formlistsrch.FilterList = <?php echo $report_form_list->GetFilterList() ?>;
freport_formlistsrch.Init();
freport_formlist.Init();
</script>
<?php } ?>
<?php
$report_form_list->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($report_form->Export == "") { ?>
...
<?php
$report_form_list->Page_Terminate();
?>
