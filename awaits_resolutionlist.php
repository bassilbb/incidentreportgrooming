...
<?php include_once "awaits_resolutioninfo.php" ?>
...
$awaits_resolution_list = NULL; // Initialize page object first

class cawaits_resolution_list extends cawaits_resolution {
	...
	// Table name
	var $TableName = 'awaits_resolution';

	// Page object name
	var $PageObjName = 'awaits_resolution_list';

	// Grid form hidden field names
	var $FormName = 'fawaits_resolutionlist';
	...
	function __construct() {
		...
		// Table object (awaits_resolution)
		if (!isset($GLOBALS["awaits_resolution"]) || get_class($GLOBALS["awaits_resolution"]) == "cawaits_resolution") {
			$GLOBALS["awaits_resolution"] = &$this;
			$GLOBALS["Table"] = &$GLOBALS["awaits_resolution"];
		}
		...
		$this->AddUrl = "awaits_resolutionadd.php";
		$this->InlineAddUrl = $this->PageUrl() . "a=add";
		$this->GridAddUrl = $this->PageUrl() . "a=gridadd";
		$this->GridEditUrl = $this->PageUrl() . "a=gridedit";
		$this->MultiDeleteUrl = "awaits_resolutiondelete.php";
		$this->MultiUpdateUrl = "awaits_resolutionupdate.php";
		...
		if (!defined("EW_TABLE_NAME"))
			define("EW_TABLE_NAME", 'awaits_resolution', TRUE);
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fawaits_resolutionlistsrch";
	...
	function Page_Terminate($url = "") {
		...
		// Export
		global $EW_EXPORT, $awaits_resolution;
		if ($this->CustomExport <> "" && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, $EW_EXPORT)) {
				$sContent = ob_get_contents();
			if ($gsExportFile == "") $gsExportFile = $this->TableVar;
			$class = $EW_EXPORT[$this->CustomExport];
			if (class_exists($class)) {
				$doc = new $class($awaits_resolution);
				$doc->Text = $sContent;
	...
	function GetFilterList() {
		...
		if (EW_SEARCH_FILTER_OPTION == "Server" && isset($UserProfile))
			$sSavedFilterList = $UserProfile->GetSearchFilters(CurrentUserName(), "fawaits_resolutionlistsrch");
	...
	function ProcessFilterList() {
		...
		if (@$_POST["ajax"] == "savefilters") { // Save filter request (Ajax)
			...
			$UserProfile->SetSearchFilters(CurrentUserName(), "fawaits_resolutionlistsrch", $filters);
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fawaits_resolutionlistsrch\" href=\"#\">" . $Language->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fawaits_resolutionlistsrch\" href=\"#\">" . $Language->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->ListActions->Items as $listaction) {
				if ($listaction->Select == EW_ACTION_MULTIPLE) {
					...
					$item->Body = "<a class=\"ewAction ewListAction\" title=\"" . ew_HtmlEncode($caption) . "\" data-caption=\"" . ew_HtmlEncode($caption) . "\" href=\"\" onclick=\"ew_SubmitAction(event,jQuery.extend({f:document.fawaits_resolutionlist}," . $listaction->ToJson(TRUE) . "));return false;\">" . $icon . "</a>";
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $Language->Phrase("SearchPanel") . "\" data-caption=\"" . $Language->Phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fawaits_resolutionlistsrch\">" . $Language->Phrase("SearchLink") . "</button>";
		$item->Visible = TRUE;
	...
	function SetupExportOptions() {
		...
		$url = "";
		$item->Body = "<button id=\"emf_awaits_resolution\" class=\"ewExportLink ewEmail\" title=\"" . $Language->Phrase("ExportToEmailText") . "\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_awaits_resolution',hdr:ewLanguage.Phrase('ExportToEmailText'),f:document.fawaits_resolutionlist,sel:false" . $url . "});\">" . $Language->Phrase("ExportToEmail") . "</button>";
		$item->Visible = FALSE;
...
// Create page object
if (!isset($awaits_resolution_list)) $awaits_resolution_list = new cawaits_resolution_list();

// Page init
$awaits_resolution_list->Page_Init();

// Page main
$awaits_resolution_list->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$awaits_resolution_list->Page_Render();
?>
<?php include_once "header.php" ?>
<?php if ($awaits_resolution->Export == "") { ?>
<script type="text/javascript">

// Form object
var CurrentPageID = EW_PAGE_ID = "list";
var CurrentForm = fawaits_resolutionlist = new ew_Form("fawaits_resolutionlist", "list");
fawaits_resolutionlist.FormKeyCountName = '<?php echo $awaits_resolution_list->FormKeyCountName ?>';

// Form_CustomValidate event
fawaits_resolutionlist.Form_CustomValidate = 
...
fawaits_resolutionlist.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fawaits_resolutionlist.Lists["x_staffid"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_staffno","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"users"};
fawaits_resolutionlist.Lists["x_staffid"].Data = "<?php echo $awaits_resolution_list->staffid->LookupFilterQuery(FALSE, "list") ?>";
fawaits_resolutionlist.AutoSuggests["x_staffid"] = <?php echo json_encode(array("data" => "ajax=autosuggest&" . $awaits_resolution_list->staffid->LookupFilterQuery(TRUE, "list"))) ?>;
fawaits_resolutionlist.Lists["x_staff_id"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_firstname","x_lastname","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"users"};
fawaits_resolutionlist.Lists["x_staff_id"].Data = "<?php echo $awaits_resolution_list->staff_id->LookupFilterQuery(FALSE, "list") ?>";
fawaits_resolutionlist.Lists["x_branch"] = {"LinkField":"x_branch_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_branch_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"branch"};
fawaits_resolutionlist.Lists["x_branch"].Data = "<?php echo $awaits_resolution_list->branch->LookupFilterQuery(FALSE, "list") ?>";
fawaits_resolutionlist.Lists["x_departments"] = {"LinkField":"x_code_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_description","","",""],"ParentFields":[],"ChildFields":["x_category"],"FilterFields":[],"Options":[],"Template":"","LinkTable":"departments"};
fawaits_resolutionlist.Lists["x_departments"].Data = "<?php echo $awaits_resolution_list->departments->LookupFilterQuery(FALSE, "list") ?>";
fawaits_resolutionlist.Lists["x_category"] = {"LinkField":"x_category_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_description","","",""],"ParentFields":[],"ChildFields":["x_sub_category"],"FilterFields":[],"Options":[],"Template":"","LinkTable":"category"};
fawaits_resolutionlist.Lists["x_category"].Data = "<?php echo $awaits_resolution_list->category->LookupFilterQuery(FALSE, "list") ?>";
fawaits_resolutionlist.Lists["x_sub_category"] = {"LinkField":"x_sub_category_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_sub_category_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"sub_category"};
fawaits_resolutionlist.Lists["x_sub_category"].Data = "<?php echo $awaits_resolution_list->sub_category->LookupFilterQuery(FALSE, "list") ?>";
fawaits_resolutionlist.Lists["x_status"] = {"LinkField":"x_code","Ajax":true,"AutoFill":false,"DisplayFields":["x_description","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"status"};
fawaits_resolutionlist.Lists["x_status"].Data = "<?php echo $awaits_resolution_list->status->LookupFilterQuery(FALSE, "list") ?>";
fawaits_resolutionlist.Lists["x_last_updated_by"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_firstname","x_lastname","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"users"};
fawaits_resolutionlist.Lists["x_last_updated_by"].Data = "<?php echo $awaits_resolution_list->last_updated_by->LookupFilterQuery(FALSE, "list") ?>";
fawaits_resolutionlist.AutoSuggests["x_last_updated_by"] = <?php echo json_encode(array("data" => "ajax=autosuggest&" . $awaits_resolution_list->last_updated_by->LookupFilterQuery(TRUE, "list"))) ?>;

// Form object for search
var CurrentSearchForm = fawaits_resolutionlistsrch = new ew_Form("fawaits_resolutionlistsrch");

// Validate function for search
fawaits_resolutionlistsrch.Validate = function(fobj) {
...
// Form_CustomValidate event
fawaits_resolutionlistsrch.Form_CustomValidate = 
 function(fobj) { // DO NOT CHANGE THIS LINE!
...
// Use JavaScript validation or not
fawaits_resolutionlistsrch.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fawaits_resolutionlistsrch.Lists["x_staff_id"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_firstname","x_lastname","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"users"};
fawaits_resolutionlistsrch.Lists["x_staff_id"].Data = "<?php echo $awaits_resolution_list->staff_id->LookupFilterQuery(FALSE, "extbs") ?>";
fawaits_resolutionlistsrch.Lists["x_status"] = {"LinkField":"x_code","Ajax":true,"AutoFill":false,"DisplayFields":["x_description","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"status"};
fawaits_resolutionlistsrch.Lists["x_status"].Data = "<?php echo $awaits_resolution_list->status->LookupFilterQuery(FALSE, "extbs") ?>";
</script>
<script type="text/javascript">
...
<?php if ($awaits_resolution->Export == "") { ?>
<div class="ewToolbar">
<?php if ($awaits_resolution_list->TotalRecs > 0 && $awaits_resolution_list->ExportOptions->Visible()) { ?>
<?php $awaits_resolution_list->ExportOptions->Render("body") ?>
<?php } ?>
<?php if ($awaits_resolution_list->SearchOptions->Visible()) { ?>
<?php $awaits_resolution_list->SearchOptions->Render("body") ?>
<?php } ?>
<?php if ($awaits_resolution_list->FilterOptions->Visible()) { ?>
<?php $awaits_resolution_list->FilterOptions->Render("body") ?>
<?php } ?>
<div class="clearfix"></div>
</div>
<?php } ?>
<?php
	$bSelectLimit = $awaits_resolution_list->UseSelectLimit;
	if ($bSelectLimit) {
		if ($awaits_resolution_list->TotalRecs <= 0)
			$awaits_resolution_list->TotalRecs = $awaits_resolution->ListRecordCount();
	} else {
		if (!$awaits_resolution_list->Recordset && ($awaits_resolution_list->Recordset = $awaits_resolution_list->LoadRecordset()))
			$awaits_resolution_list->TotalRecs = $awaits_resolution_list->Recordset->RecordCount();
	}
	$awaits_resolution_list->StartRec = 1;
	if ($awaits_resolution_list->DisplayRecs <= 0 || ($awaits_resolution->Export <> "" && $awaits_resolution->ExportAll)) // Display all records
		$awaits_resolution_list->DisplayRecs = $awaits_resolution_list->TotalRecs;
	if (!($awaits_resolution->Export <> "" && $awaits_resolution->ExportAll))
		$awaits_resolution_list->SetupStartRec(); // Set up start record position
	if ($bSelectLimit)
		$awaits_resolution_list->Recordset = $awaits_resolution_list->LoadRecordset($awaits_resolution_list->StartRec-1, $awaits_resolution_list->DisplayRecs);

	// Set no record found message
	if ($awaits_resolution->CurrentAction == "" && $awaits_resolution_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$awaits_resolution_list->setWarningMessage(ew_DeniedMsg());
		if ($awaits_resolution_list->SearchWhere == "0=101")
			$awaits_resolution_list->setWarningMessage($Language->Phrase("EnterSearchCriteria"));
		else
			$awaits_resolution_list->setWarningMessage($Language->Phrase("NoRecord"));
	}
$awaits_resolution_list->RenderOtherOptions();
?>
<?php if ($Security->CanSearch()) { ?>
<?php if ($awaits_resolution->Export == "" && $awaits_resolution->CurrentAction == "") { ?>
<form name="fawaits_resolutionlistsrch" id="fawaits_resolutionlistsrch" class="form-inline ewForm ewExtSearchForm" action="<?php echo ew_CurrentPage() ?>">
<?php $SearchPanelClass = ($awaits_resolution_list->SearchWhere <> "") ? " in" : " in"; ?>
<div id="fawaits_resolutionlistsrch_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
<input type="hidden" name="cmd" value="search">
<input type="hidden" name="t" value="awaits_resolution">
	<div class="ewBasicSearch">
<?php
if ($gsSearchError == "")
	$awaits_resolution_list->LoadAdvancedSearch(); // Load advanced search

// Render for search
$awaits_resolution->RowType = EW_ROWTYPE_SEARCH;

// Render row
$awaits_resolution->ResetAttrs();
$awaits_resolution_list->RenderRow();
?>
<div id="xsr_1" class="ewRow">
<?php if ($awaits_resolution->incident_id->Visible) { // incident_id ?>
	<div id="xsc_incident_id" class="ewCell form-group">
		<label for="x_incident_id" class="ewSearchCaption ewLabel"><?php echo $awaits_resolution->incident_id->FldCaption() ?></label>
...
<input type="text" data-table="awaits_resolution" data-field="x_incident_id" name="x_incident_id" id="x_incident_id" size="18" placeholder="<?php echo ew_HtmlEncode($awaits_resolution->incident_id->getPlaceHolder()) ?>" value="<?php echo $awaits_resolution->incident_id->EditValue ?>"<?php echo $awaits_resolution->incident_id->EditAttributes() ?>>
</span>
	</div>
<?php } ?>
</div>
<div id="xsr_2" class="ewRow">
<?php if ($awaits_resolution->staff_id->Visible) { // staff_id ?>
	<div id="xsc_staff_id" class="ewCell form-group">
		<label for="x_staff_id" class="ewSearchCaption ewLabel"><?php echo $awaits_resolution->staff_id->FldCaption() ?></label>
		<span class="ewSearchOperator"><?php echo $Language->Phrase("=") ?><input type="hidden" name="z_staff_id" id="z_staff_id" value="="></span>
		<span class="ewSearchField">
<select data-table="awaits_resolution" data-field="x_staff_id" data-value-separator="<?php echo $awaits_resolution->staff_id->DisplayValueSeparatorAttribute() ?>" id="x_staff_id" name="x_staff_id"<?php echo $awaits_resolution->staff_id->EditAttributes() ?>>
<?php echo $awaits_resolution->staff_id->SelectOptionListHtml("x_staff_id") ?>
</select>
</span>
	</div>
<?php } ?>
...
<?php if ($awaits_resolution->status->Visible) { // status ?>
	<div id="xsc_status" class="ewCell form-group">
		<label for="x_status" class="ewSearchCaption ewLabel"><?php echo $awaits_resolution->status->FldCaption() ?></label>
		<span class="ewSearchOperator"><?php echo $Language->Phrase("=") ?><input type="hidden" name="z_status" id="z_status" value="="></span>
		<span class="ewSearchField">
<select data-table="awaits_resolution" data-field="x_status" data-value-separator="<?php echo $awaits_resolution->status->DisplayValueSeparatorAttribute() ?>" id="x_status" name="x_status"<?php echo $awaits_resolution->status->EditAttributes() ?>>
<?php echo $awaits_resolution->status->SelectOptionListHtml("x_status") ?>
</select>
</span>
	</div>
<?php } ?>
...
<div id="xsr_4" class="ewRow">
	...
	<input type="text" name="<?php echo EW_TABLE_BASIC_SEARCH ?>" id="<?php echo EW_TABLE_BASIC_SEARCH ?>" class="form-control" value="<?php echo ew_HtmlEncode($awaits_resolution_list->BasicSearch->getKeyword()) ?>" placeholder="<?php echo ew_HtmlEncode($Language->Phrase("Search")) ?>">
	<input type="hidden" name="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" id="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" value="<?php echo ew_HtmlEncode($awaits_resolution_list->BasicSearch->getType()) ?>">
	<div class="input-group-btn">
		<button type="button" data-toggle="dropdown" class="btn btn-default"><span id="searchtype"><?php echo $awaits_resolution_list->BasicSearch->getTypeNameShort() ?></span><span class="caret"></span></button>
		<ul class="dropdown-menu pull-right" role="menu">
			<li<?php if ($awaits_resolution_list->BasicSearch->getType() == "") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this)"><?php echo $Language->Phrase("QuickSearchAuto") ?></a></li>
			<li<?php if ($awaits_resolution_list->BasicSearch->getType() == "=") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'=')"><?php echo $Language->Phrase("QuickSearchExact") ?></a></li>
			<li<?php if ($awaits_resolution_list->BasicSearch->getType() == "AND") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'AND')"><?php echo $Language->Phrase("QuickSearchAll") ?></a></li>
			<li<?php if ($awaits_resolution_list->BasicSearch->getType() == "OR") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'OR')"><?php echo $Language->Phrase("QuickSearchAny") ?></a></li>
		</ul>
...
<?php } ?>
<?php } ?>
<?php $awaits_resolution_list->ShowPageHeader(); ?>
<?php
$awaits_resolution_list->ShowMessage();
?>
<?php if ($awaits_resolution_list->TotalRecs > 0 || $awaits_resolution->CurrentAction <> "") { ?>
<div class="box ewBox ewGrid<?php if ($awaits_resolution_list->IsAddOrEdit()) { ?> ewGridAddEdit<?php } ?> awaits_resolution">
<?php if ($awaits_resolution->Export == "") { ?>
<div class="box-header ewGridUpperPanel">
<?php if ($awaits_resolution->CurrentAction <> "gridadd" && $awaits_resolution->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="form-inline ewForm ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($awaits_resolution_list->Pager)) $awaits_resolution_list->Pager = new cPrevNextPager($awaits_resolution_list->StartRec, $awaits_resolution_list->DisplayRecs, $awaits_resolution_list->TotalRecs, $awaits_resolution_list->AutoHidePager) ?>
<?php if ($awaits_resolution_list->Pager->RecordCount > 0 && $awaits_resolution_list->Pager->Visible) { ?>
<div class="ewPager">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($awaits_resolution_list->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $awaits_resolution_list->PageUrl() ?>start=<?php echo $awaits_resolution_list->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($awaits_resolution_list->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $awaits_resolution_list->PageUrl() ?>start=<?php echo $awaits_resolution_list->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $awaits_resolution_list->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($awaits_resolution_list->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $awaits_resolution_list->PageUrl() ?>start=<?php echo $awaits_resolution_list->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($awaits_resolution_list->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $awaits_resolution_list->PageUrl() ?>start=<?php echo $awaits_resolution_list->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $awaits_resolution_list->Pager->PageCount ?></span>
</div>
<?php } ?>
<?php if ($awaits_resolution_list->Pager->RecordCount > 0) { ?>
<div class="ewPager ewRec">
	<span><?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $awaits_resolution_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $awaits_resolution_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $awaits_resolution_list->Pager->RecordCount ?></span>
</div>
<?php } ?>
<?php if ($awaits_resolution_list->TotalRecs > 0 && (!$awaits_resolution_list->AutoHidePageSizeSelector || $awaits_resolution_list->Pager->Visible)) { ?>
<div class="ewPager">
<input type="hidden" name="t" value="awaits_resolution">
<select name="<?php echo EW_TABLE_REC_PER_PAGE ?>" class="form-control input-sm ewTooltip" title="<?php echo $Language->Phrase("RecordsPerPage") ?>" onchange="this.form.submit();">
<option value="5"<?php if ($awaits_resolution_list->DisplayRecs == 5) { ?> selected<?php } ?>>5</option>
<option value="10"<?php if ($awaits_resolution_list->DisplayRecs == 10) { ?> selected<?php } ?>>10</option>
<option value="15"<?php if ($awaits_resolution_list->DisplayRecs == 15) { ?> selected<?php } ?>>15</option>
<option value="20"<?php if ($awaits_resolution_list->DisplayRecs == 20) { ?> selected<?php } ?>>20</option>
<option value="50"<?php if ($awaits_resolution_list->DisplayRecs == 50) { ?> selected<?php } ?>>50</option>
<option value="ALL"<?php if ($awaits_resolution->getRecordsPerPage() == -1) { ?> selected<?php } ?>><?php echo $Language->Phrase("AllRecords") ?></option>
</select>
</div>
<?php } ?>
</form>
...
<?php
	foreach ($awaits_resolution_list->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<div class="clearfix"></div>
...
<?php } ?>
<form name="fawaits_resolutionlist" id="fawaits_resolutionlist" class="form-inline ewForm ewListForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($awaits_resolution_list->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $awaits_resolution_list->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="awaits_resolution">
<div id="gmp_awaits_resolution" class="<?php if (ew_IsResponsiveLayout()) { ?>table-responsive <?php } ?>ewGridMiddlePanel">
<?php if ($awaits_resolution_list->TotalRecs > 0 || $awaits_resolution->CurrentAction == "gridedit") { ?>
<table id="tbl_awaits_resolutionlist" class="table ewTable">
<thead>
	<tr class="ewTableHeader">
<?php

// Header row
$awaits_resolution_list->RowType = EW_ROWTYPE_HEADER;

// Render list options
$awaits_resolution_list->RenderListOptions();

// Render list options (header, left)
$awaits_resolution_list->ListOptions->Render("header", "left");
?>
<?php if ($awaits_resolution->datetime_initiated->Visible) { // datetime_initiated ?>
	<?php if ($awaits_resolution->SortUrl($awaits_resolution->datetime_initiated) == "") { ?>
		<th data-name="datetime_initiated" class="<?php echo $awaits_resolution->datetime_initiated->HeaderCellClass() ?>"><div id="elh_awaits_resolution_datetime_initiated" class="awaits_resolution_datetime_initiated"><div class="ewTableHeaderCaption"><?php echo $awaits_resolution->datetime_initiated->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="datetime_initiated" class="<?php echo $awaits_resolution->datetime_initiated->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $awaits_resolution->SortUrl($awaits_resolution->datetime_initiated) ?>',1);"><div id="elh_awaits_resolution_datetime_initiated" class="awaits_resolution_datetime_initiated">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $awaits_resolution->datetime_initiated->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($awaits_resolution->datetime_initiated->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($awaits_resolution->datetime_initiated->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($awaits_resolution->incident_id->Visible) { // incident_id ?>
	<?php if ($awaits_resolution->SortUrl($awaits_resolution->incident_id) == "") { ?>
		<th data-name="incident_id" class="<?php echo $awaits_resolution->incident_id->HeaderCellClass() ?>"><div id="elh_awaits_resolution_incident_id" class="awaits_resolution_incident_id"><div class="ewTableHeaderCaption"><?php echo $awaits_resolution->incident_id->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="incident_id" class="<?php echo $awaits_resolution->incident_id->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $awaits_resolution->SortUrl($awaits_resolution->incident_id) ?>',1);"><div id="elh_awaits_resolution_incident_id" class="awaits_resolution_incident_id">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $awaits_resolution->incident_id->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($awaits_resolution->incident_id->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($awaits_resolution->incident_id->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($awaits_resolution->staffid->Visible) { // staffid ?>
	<?php if ($awaits_resolution->SortUrl($awaits_resolution->staffid) == "") { ?>
		<th data-name="staffid" class="<?php echo $awaits_resolution->staffid->HeaderCellClass() ?>"><div id="elh_awaits_resolution_staffid" class="awaits_resolution_staffid"><div class="ewTableHeaderCaption"><?php echo $awaits_resolution->staffid->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="staffid" class="<?php echo $awaits_resolution->staffid->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $awaits_resolution->SortUrl($awaits_resolution->staffid) ?>',1);"><div id="elh_awaits_resolution_staffid" class="awaits_resolution_staffid">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $awaits_resolution->staffid->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($awaits_resolution->staffid->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($awaits_resolution->staffid->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($awaits_resolution->staff_id->Visible) { // staff_id ?>
	<?php if ($awaits_resolution->SortUrl($awaits_resolution->staff_id) == "") { ?>
		<th data-name="staff_id" class="<?php echo $awaits_resolution->staff_id->HeaderCellClass() ?>"><div id="elh_awaits_resolution_staff_id" class="awaits_resolution_staff_id"><div class="ewTableHeaderCaption"><?php echo $awaits_resolution->staff_id->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="staff_id" class="<?php echo $awaits_resolution->staff_id->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $awaits_resolution->SortUrl($awaits_resolution->staff_id) ?>',1);"><div id="elh_awaits_resolution_staff_id" class="awaits_resolution_staff_id">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $awaits_resolution->staff_id->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($awaits_resolution->staff_id->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($awaits_resolution->staff_id->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($awaits_resolution->branch->Visible) { // branch ?>
	<?php if ($awaits_resolution->SortUrl($awaits_resolution->branch) == "") { ?>
		<th data-name="branch" class="<?php echo $awaits_resolution->branch->HeaderCellClass() ?>"><div id="elh_awaits_resolution_branch" class="awaits_resolution_branch"><div class="ewTableHeaderCaption"><?php echo $awaits_resolution->branch->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="branch" class="<?php echo $awaits_resolution->branch->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $awaits_resolution->SortUrl($awaits_resolution->branch) ?>',1);"><div id="elh_awaits_resolution_branch" class="awaits_resolution_branch">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $awaits_resolution->branch->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($awaits_resolution->branch->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($awaits_resolution->branch->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($awaits_resolution->departments->Visible) { // departments ?>
	<?php if ($awaits_resolution->SortUrl($awaits_resolution->departments) == "") { ?>
		<th data-name="departments" class="<?php echo $awaits_resolution->departments->HeaderCellClass() ?>"><div id="elh_awaits_resolution_departments" class="awaits_resolution_departments"><div class="ewTableHeaderCaption"><?php echo $awaits_resolution->departments->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="departments" class="<?php echo $awaits_resolution->departments->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $awaits_resolution->SortUrl($awaits_resolution->departments) ?>',1);"><div id="elh_awaits_resolution_departments" class="awaits_resolution_departments">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $awaits_resolution->departments->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($awaits_resolution->departments->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($awaits_resolution->departments->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($awaits_resolution->category->Visible) { // category ?>
	<?php if ($awaits_resolution->SortUrl($awaits_resolution->category) == "") { ?>
		<th data-name="category" class="<?php echo $awaits_resolution->category->HeaderCellClass() ?>"><div id="elh_awaits_resolution_category" class="awaits_resolution_category"><div class="ewTableHeaderCaption"><?php echo $awaits_resolution->category->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="category" class="<?php echo $awaits_resolution->category->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $awaits_resolution->SortUrl($awaits_resolution->category) ?>',1);"><div id="elh_awaits_resolution_category" class="awaits_resolution_category">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $awaits_resolution->category->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($awaits_resolution->category->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($awaits_resolution->category->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($awaits_resolution->sub_category->Visible) { // sub_category ?>
	<?php if ($awaits_resolution->SortUrl($awaits_resolution->sub_category) == "") { ?>
		<th data-name="sub_category" class="<?php echo $awaits_resolution->sub_category->HeaderCellClass() ?>"><div id="elh_awaits_resolution_sub_category" class="awaits_resolution_sub_category"><div class="ewTableHeaderCaption"><?php echo $awaits_resolution->sub_category->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="sub_category" class="<?php echo $awaits_resolution->sub_category->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $awaits_resolution->SortUrl($awaits_resolution->sub_category) ?>',1);"><div id="elh_awaits_resolution_sub_category" class="awaits_resolution_sub_category">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $awaits_resolution->sub_category->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($awaits_resolution->sub_category->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($awaits_resolution->sub_category->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($awaits_resolution->incident_description->Visible) { // incident_description ?>
	<?php if ($awaits_resolution->SortUrl($awaits_resolution->incident_description) == "") { ?>
		<th data-name="incident_description" class="<?php echo $awaits_resolution->incident_description->HeaderCellClass() ?>"><div id="elh_awaits_resolution_incident_description" class="awaits_resolution_incident_description"><div class="ewTableHeaderCaption"><?php echo $awaits_resolution->incident_description->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="incident_description" class="<?php echo $awaits_resolution->incident_description->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $awaits_resolution->SortUrl($awaits_resolution->incident_description) ?>',1);"><div id="elh_awaits_resolution_incident_description" class="awaits_resolution_incident_description">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $awaits_resolution->incident_description->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($awaits_resolution->incident_description->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($awaits_resolution->incident_description->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($awaits_resolution->status->Visible) { // status ?>
	<?php if ($awaits_resolution->SortUrl($awaits_resolution->status) == "") { ?>
		<th data-name="status" class="<?php echo $awaits_resolution->status->HeaderCellClass() ?>"><div id="elh_awaits_resolution_status" class="awaits_resolution_status"><div class="ewTableHeaderCaption"><?php echo $awaits_resolution->status->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="status" class="<?php echo $awaits_resolution->status->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $awaits_resolution->SortUrl($awaits_resolution->status) ?>',1);"><div id="elh_awaits_resolution_status" class="awaits_resolution_status">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $awaits_resolution->status->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($awaits_resolution->status->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($awaits_resolution->status->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($awaits_resolution->last_updated_date->Visible) { // last_updated_date ?>
	<?php if ($awaits_resolution->SortUrl($awaits_resolution->last_updated_date) == "") { ?>
		<th data-name="last_updated_date" class="<?php echo $awaits_resolution->last_updated_date->HeaderCellClass() ?>"><div id="elh_awaits_resolution_last_updated_date" class="awaits_resolution_last_updated_date"><div class="ewTableHeaderCaption"><?php echo $awaits_resolution->last_updated_date->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="last_updated_date" class="<?php echo $awaits_resolution->last_updated_date->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $awaits_resolution->SortUrl($awaits_resolution->last_updated_date) ?>',1);"><div id="elh_awaits_resolution_last_updated_date" class="awaits_resolution_last_updated_date">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $awaits_resolution->last_updated_date->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($awaits_resolution->last_updated_date->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($awaits_resolution->last_updated_date->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($awaits_resolution->last_updated_by->Visible) { // last_updated_by ?>
	<?php if ($awaits_resolution->SortUrl($awaits_resolution->last_updated_by) == "") { ?>
		<th data-name="last_updated_by" class="<?php echo $awaits_resolution->last_updated_by->HeaderCellClass() ?>"><div id="elh_awaits_resolution_last_updated_by" class="awaits_resolution_last_updated_by"><div class="ewTableHeaderCaption"><?php echo $awaits_resolution->last_updated_by->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="last_updated_by" class="<?php echo $awaits_resolution->last_updated_by->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $awaits_resolution->SortUrl($awaits_resolution->last_updated_by) ?>',1);"><div id="elh_awaits_resolution_last_updated_by" class="awaits_resolution_last_updated_by">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $awaits_resolution->last_updated_by->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($awaits_resolution->last_updated_by->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($awaits_resolution->last_updated_by->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php
...
$awaits_resolution_list->ListOptions->Render("header", "right");
?>
	</tr>
</thead>
<tbody>
<?php
if ($awaits_resolution->ExportAll && $awaits_resolution->Export <> "") {
	$awaits_resolution_list->StopRec = $awaits_resolution_list->TotalRecs;
} else {

	// Set the last record to display
	if ($awaits_resolution_list->TotalRecs > $awaits_resolution_list->StartRec + $awaits_resolution_list->DisplayRecs - 1)
		$awaits_resolution_list->StopRec = $awaits_resolution_list->StartRec + $awaits_resolution_list->DisplayRecs - 1;
	else
		$awaits_resolution_list->StopRec = $awaits_resolution_list->TotalRecs;
}
$awaits_resolution_list->RecCnt = $awaits_resolution_list->StartRec - 1;
if ($awaits_resolution_list->Recordset && !$awaits_resolution_list->Recordset->EOF) {
	$awaits_resolution_list->Recordset->MoveFirst();
	$bSelectLimit = $awaits_resolution_list->UseSelectLimit;
	if (!$bSelectLimit && $awaits_resolution_list->StartRec > 1)
		$awaits_resolution_list->Recordset->Move($awaits_resolution_list->StartRec - 1);
} elseif (!$awaits_resolution->AllowAddDeleteRow && $awaits_resolution_list->StopRec == 0) {
	$awaits_resolution_list->StopRec = $awaits_resolution->GridAddRowCount;
}

// Initialize aggregate
$awaits_resolution->RowType = EW_ROWTYPE_AGGREGATEINIT;
$awaits_resolution->ResetAttrs();
$awaits_resolution_list->RenderRow();
while ($awaits_resolution_list->RecCnt < $awaits_resolution_list->StopRec) {
	$awaits_resolution_list->RecCnt++;
	if (intval($awaits_resolution_list->RecCnt) >= intval($awaits_resolution_list->StartRec)) {
		$awaits_resolution_list->RowCnt++;

		// Set up key count
		$awaits_resolution_list->KeyCount = $awaits_resolution_list->RowIndex;

		// Init row class and style
		$awaits_resolution->ResetAttrs();
		$awaits_resolution->CssClass = "";
		if ($awaits_resolution->CurrentAction == "gridadd") {
		} else {
			$awaits_resolution_list->LoadRowValues($awaits_resolution_list->Recordset); // Load row values
		}
		$awaits_resolution->RowType = EW_ROWTYPE_VIEW; // Render view

		// Set up row id / data-rowindex
		$awaits_resolution->RowAttrs = array_merge($awaits_resolution->RowAttrs, array('data-rowindex'=>$awaits_resolution_list->RowCnt, 'id'=>'r' . $awaits_resolution_list->RowCnt . '_awaits_resolution', 'data-rowtype'=>$awaits_resolution->RowType));
		...
		$awaits_resolution_list->RenderListOptions();
?>
	<tr<?php echo $awaits_resolution->RowAttributes() ?>>
<?php

// Render list options (body, left)
$awaits_resolution_list->ListOptions->Render("body", "left", $awaits_resolution_list->RowCnt);
?>
	<?php if ($awaits_resolution->datetime_initiated->Visible) { // datetime_initiated ?>
		<td data-name="datetime_initiated"<?php echo $awaits_resolution->datetime_initiated->CellAttributes() ?>>
<span id="el<?php echo $awaits_resolution_list->RowCnt ?>_awaits_resolution_datetime_initiated" class="awaits_resolution_datetime_initiated">
<span<?php echo $awaits_resolution->datetime_initiated->ViewAttributes() ?>>
<?php echo $awaits_resolution->datetime_initiated->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($awaits_resolution->incident_id->Visible) { // incident_id ?>
		<td data-name="incident_id"<?php echo $awaits_resolution->incident_id->CellAttributes() ?>>
<span id="el<?php echo $awaits_resolution_list->RowCnt ?>_awaits_resolution_incident_id" class="awaits_resolution_incident_id">
<span<?php echo $awaits_resolution->incident_id->ViewAttributes() ?>>
<?php echo $awaits_resolution->incident_id->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($awaits_resolution->staffid->Visible) { // staffid ?>
		<td data-name="staffid"<?php echo $awaits_resolution->staffid->CellAttributes() ?>>
<span id="el<?php echo $awaits_resolution_list->RowCnt ?>_awaits_resolution_staffid" class="awaits_resolution_staffid">
<span<?php echo $awaits_resolution->staffid->ViewAttributes() ?>>
<?php echo $awaits_resolution->staffid->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($awaits_resolution->staff_id->Visible) { // staff_id ?>
		<td data-name="staff_id"<?php echo $awaits_resolution->staff_id->CellAttributes() ?>>
<span id="el<?php echo $awaits_resolution_list->RowCnt ?>_awaits_resolution_staff_id" class="awaits_resolution_staff_id">
<span<?php echo $awaits_resolution->staff_id->ViewAttributes() ?>>
<?php echo $awaits_resolution->staff_id->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($awaits_resolution->branch->Visible) { // branch ?>
		<td data-name="branch"<?php echo $awaits_resolution->branch->CellAttributes() ?>>
<span id="el<?php echo $awaits_resolution_list->RowCnt ?>_awaits_resolution_branch" class="awaits_resolution_branch">
<span<?php echo $awaits_resolution->branch->ViewAttributes() ?>>
<?php echo $awaits_resolution->branch->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($awaits_resolution->departments->Visible) { // departments ?>
		<td data-name="departments"<?php echo $awaits_resolution->departments->CellAttributes() ?>>
<span id="el<?php echo $awaits_resolution_list->RowCnt ?>_awaits_resolution_departments" class="awaits_resolution_departments">
<span<?php echo $awaits_resolution->departments->ViewAttributes() ?>>
<?php echo $awaits_resolution->departments->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($awaits_resolution->category->Visible) { // category ?>
		<td data-name="category"<?php echo $awaits_resolution->category->CellAttributes() ?>>
<span id="el<?php echo $awaits_resolution_list->RowCnt ?>_awaits_resolution_category" class="awaits_resolution_category">
<span<?php echo $awaits_resolution->category->ViewAttributes() ?>>
<?php echo $awaits_resolution->category->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($awaits_resolution->sub_category->Visible) { // sub_category ?>
		<td data-name="sub_category"<?php echo $awaits_resolution->sub_category->CellAttributes() ?>>
<span id="el<?php echo $awaits_resolution_list->RowCnt ?>_awaits_resolution_sub_category" class="awaits_resolution_sub_category">
<span<?php echo $awaits_resolution->sub_category->ViewAttributes() ?>>
<?php echo $awaits_resolution->sub_category->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($awaits_resolution->incident_description->Visible) { // incident_description ?>
		<td data-name="incident_description"<?php echo $awaits_resolution->incident_description->CellAttributes() ?>>
<span id="el<?php echo $awaits_resolution_list->RowCnt ?>_awaits_resolution_incident_description" class="awaits_resolution_incident_description">
<span<?php echo $awaits_resolution->incident_description->ViewAttributes() ?>>
<?php echo $awaits_resolution->incident_description->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($awaits_resolution->status->Visible) { // status ?>
		<td data-name="status"<?php echo $awaits_resolution->status->CellAttributes() ?>>
<span id="el<?php echo $awaits_resolution_list->RowCnt ?>_awaits_resolution_status" class="awaits_resolution_status">
<span<?php echo $awaits_resolution->status->ViewAttributes() ?>>
<?php echo $awaits_resolution->status->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($awaits_resolution->last_updated_date->Visible) { // last_updated_date ?>
		<td data-name="last_updated_date"<?php echo $awaits_resolution->last_updated_date->CellAttributes() ?>>
<span id="el<?php echo $awaits_resolution_list->RowCnt ?>_awaits_resolution_last_updated_date" class="awaits_resolution_last_updated_date">
<span<?php echo $awaits_resolution->last_updated_date->ViewAttributes() ?>>
<?php echo $awaits_resolution->last_updated_date->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($awaits_resolution->last_updated_by->Visible) { // last_updated_by ?>
		<td data-name="last_updated_by"<?php echo $awaits_resolution->last_updated_by->CellAttributes() ?>>
<span id="el<?php echo $awaits_resolution_list->RowCnt ?>_awaits_resolution_last_updated_by" class="awaits_resolution_last_updated_by">
<span<?php echo $awaits_resolution->last_updated_by->ViewAttributes() ?>>
<?php echo $awaits_resolution->last_updated_by->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
<?php
...
$awaits_resolution_list->ListOptions->Render("body", "right", $awaits_resolution_list->RowCnt);
?>
	</tr>
<?php
	}
	if ($awaits_resolution->CurrentAction <> "gridadd")
		$awaits_resolution_list->Recordset->MoveNext();
}
?>
</tbody>
</table>
<?php } ?>
<?php if ($awaits_resolution->CurrentAction == "") { ?>
...
// Close recordset
if ($awaits_resolution_list->Recordset)
	$awaits_resolution_list->Recordset->Close();
?>
</div>
<?php } ?>
<?php if ($awaits_resolution_list->TotalRecs == 0 && $awaits_resolution->CurrentAction == "") { // Show other options ?>
<div class="ewListOtherOptions">
<?php
	foreach ($awaits_resolution_list->OtherOptions as &$option) {
		$option->ButtonClass = "";
...
<?php if ($awaits_resolution->Export == "") { ?>
<script type="text/javascript">
fawaits_resolutionlistsrch.FilterList = <?php echo $awaits_resolution_list->GetFilterList() ?>;
fawaits_resolutionlistsrch.Init();
fawaits_resolutionlist.Init();
</script>
<?php } ?>
<?php
$awaits_resolution_list->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($awaits_resolution->Export == "") { ?>
...
<?php
$awaits_resolution_list->Page_Terminate();
?>
